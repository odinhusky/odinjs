...
import { modifyXdfsDetail, getModifyXdfsDetail } from 'utils/api';
...
function ModifyXdfsDetailModal({ isOpen, onClose }) {
  const { t } = useTranslation();
  const classes = useStyles();
  const { getData, userList, updateName } = useContext(Context);
  const globalData = useContext(GlobalContext);

  const [isXdfsDataUpdating, setXdfsDataUpdating] = useState(false);
  ...
  const getModifyData = () => {
    if (updateName !== undefined) {
      getModifyXdfsDetail(updateName)
        .then(res => {
          setSpace(res.size);
  ...
  const onSubmit = async() => {
    ...
    modifyXdfsDetail(formData)
      ...
      .finally(() => setXdfsDataUpdating(false))
  };
...
ModifyXdfsDetailModal.propTypes = {
  isOpen: PropTypes.bool,
  onClose: PropTypes.func
};

export default ModifyXdfsDetailModal;
