...
import { createXdfsVolume } from 'utils/api';
  ...
  const onSubmit = async() => {
    if (createGlusterfsData.name && createGlusterfsData.source && createGlusterfsData.target) {
      try {
        ...
        await createXdfsVolume(createGlusterfsData);
      ...
      <div className={styles.wrapper} >
        ...
        <div className={styles.volumeDetail}>
          ...
          <BaseTextField
            ...
            required
            type="text"
            value={choiceGroupData.size === undefined ? '' : ((choiceGroupData.size) / MB).toFixed(2)}
          />
          <BaseTextField
            classes={{ root: classes.marginBottom10 }}
            disabled
            ...
            required
            type="text"
            value={choiceGroupData.used === undefined ? '' : ((choiceGroupData.used) / MB).toFixed(2)}
          />
          <BaseTextField
            classes={{ root: classes.marginBottom10 }}
            disabled
            ...
            required
            type="text"
            value={choiceGroupData.available === undefined ? '' : ((choiceGroupData.available) / MB).toFixed(2)}
          />
        </div>
      </div>
