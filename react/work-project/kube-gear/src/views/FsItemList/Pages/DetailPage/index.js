...
const DetailPage = ({ match }) => {
  ...
  function useFetch(rootUrl, token, name) {
    let loading = false;
    let progress = 0;
            ...
            }),
            {
              ...
              onTableCellRender: (fsItemNode) => {
                ...
                const downloadFileTest = async () => {
                  ...
                  const { getData } = useFetch(`${window.ENV.restServerUri}/api/v2${`/${isXdfsEnabled ? 'xdfs' : isNFS ? 'nfs' : 'glusterfs'}/${path}/file${fsItemNodePath}?download=true`}`, token, name)
                  await getData()
                }
