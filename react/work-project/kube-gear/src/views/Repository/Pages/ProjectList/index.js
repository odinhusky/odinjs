...
const index = () => {
  ...
  // ? context
  const { isLoading, setIsLoading, projectList, getProjectList, classes, userInfo, isAdmin } = useContext(Context)
  ...
  return (
    <>
      ...
      <div className={`${classes.d_flex} ${classes.mb_16}`}>
        <DefaultButton
          children={t('refresh')}
          classNameProps={`${classes.mr_20} ${!isAdmin && classes.mr_auto}`}
          disabled={isLoading}
          onClick={() => {
            getProjectInfo();
            getProjectList();
