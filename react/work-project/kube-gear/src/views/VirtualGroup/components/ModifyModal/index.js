...
function ModifyGroupModal({ isOpen, onClose, resourceUnits }) {
  ...
  const { getData, modifyGroupData, originModifyResourceSelectedData } = useContext(Context);
  ...
  useEffect(() => {
    if (!isEmpty(modifyGroupData) && !isEmpty(originModifyResourceSelectedData)) {
  ...
  }, [modifyGroupData, originModifyResourceSelectedData])
