...
import { objToArr, arrToObj } from 'common/commonMethods'
 ...
 * @prop {function} setXdfsMountsState -- 設定 xdfsMounts setState function
 * @prop {function} setParameters -- 設定 parameters setState function
 * @prop {boolean} isXdfsEnabled -- 是否是使用 大道 API
...
export const Step2 = ({
  ...
  */
  const buildTaskRolesObj = (jobConfig) => {
    ...
    const { taskRoles, prerequisites, defaults, extras } = jobConfig
    const { virtualCluster: vg_1 } = defaults
    ...
    return Object.entries(taskRoles).reduce((acc, [taskName, taskObj]) => {
      ...
      const { pinnedCellId, skuType } = extras.hivedScheduler.taskRoles[taskName]

      // 找到該模板中針對不同 taskRole 選擇的資源資源名稱(等同於 resOptions 中的resourceName)
      const skuType_2 = !isNil(pinnedCellId) ? 'pinned' : !isNil(skuType) ? 'virtual' : null
      const sku_3 = !isNil(pinnedCellId) ? pinnedCellId : !isNil(skuType) ? skuType : null
      const vgInfos = arrToObj(selfCanUseVgList, false)
      const resourceCountObj = getResourceUnitCount(jobConfig, vgInfos)
      ...
      return {
        ...
        [taskName]: {
          ...
          taskRetryCount,
          hivedScheduler: !isEmpty(resourceCountObj)
            ? {
              vg: vg_1,
              skuType: skuType_2,
              sku: sku_3,
              skuNum: resourceCountObj[taskName].count
            }
            : {
              vg: '',
              skuType: '',
  ...
  */
  const replaceMounts = (jobConfig) => {
    ...
    if(isXdfsEnabled) {
      // 是大道 API 的場合，就只更新大道的存儲列表
      if(!isNil(xdfsList)) setXdfsMountsState(xdfsList)
    } else {
      // 不是的場合就更新集中式 | 分散式存儲列表
