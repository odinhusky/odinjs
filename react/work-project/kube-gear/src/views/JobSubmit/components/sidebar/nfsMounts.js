...
export const NFSMounts = React.memo(
  ({
    ...
    // ? context
    const { nfsNames, glusterfsNames, xdfsNames, isXdfsEnabled, classes } = useContext(Context);
    ...
    const xdfsOptions = useMemo(
      () => {
        return xdfsNames.map(xdfs => ({
          key: xdfs,
          text: xdfs
        }))
      },
          ...
          <>
            {
              isXdfsEnabled
                ...
                <GlusterfsMountLists
                  ...
                  hint={t('toolTipsNFS')}
                  keyOptions={xdfsOptions}
                  name="XDFS List"
                  nfsMounts={xdfsMounts}
                  onChange={onXdfsMountChange}
                  title={t('xdfs')}
                />
                :
                <>
...
NFSMounts.propTypes = {
  ...
  onXdfsMountChange: PropTypes.func.isRequired
};
