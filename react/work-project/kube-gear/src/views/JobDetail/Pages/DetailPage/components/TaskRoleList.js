      ...
      <BasePaper
        columns={[
          ...
          },
          {
            ...
            onTableCellRender: item => {
              ...
              const PortTable = ({ columns, data }) => {
                return (
                  <div style={{ display: 'flex', flexDirection: 'column', padding: 10 }}>
                    ...
                    {Object.entries(data).map(([name, port], index) => (
                      <div style={{ display: 'flex', justifyContent: 'space-around', marginBottom: Object.entries(data).length - 1 === index ? 0 : 10 }}>
                        <div style={{ width: 100 }}>{name}</div>
                        <div style={{ width: 100 }}>{port}</div>
                      </div>
                    ))}
                  </div>
                )
              ...
              return (
                ...
                >
                  <div style={{ cursor: 'help', maxWidth: 100, textOverflow: 'ellipsis', overflow: 'hidden', whiteSpace: 'nowrap' }}>
          ...
          },
          {
            ...
            onTableCellRender: (item) => {
              const { containerIp, containerId, containerPorts, containerLog, taskIndex } = item;
              const jobState = jobInfo.jobStatus.state;
              return (
                <div style={{ display: 'flex' }}>
                  {/* Our elegant web ssh*/}
                  {getWebSsh(jobState, taskIndex)}
                  <SplitButton
                    ButtonGroupProps={{
                      classes: { root: classes.mr_10 }
                    }}
                    ...
                    text={`${t('package')}${t('enSpace')}${t('container')}`}
                  />
                  <SplitButton
