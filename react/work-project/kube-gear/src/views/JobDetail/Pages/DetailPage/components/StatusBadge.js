...
export const IconBadge = ({ children, style, icon, containerStyle }) => {
  return (
    <div style={{ width: '100%', fontSize: '14px', color: '#fff', display: 'flex', ...style }}>
      <div
        style={
...
const bgYellow = ({ backgroundColor: statusColorMapping.waiting  });
const bgRed = ({ backgroundColor: statusColorMapping.failed  });
const bgBlue = ({ backgroundColor: statusColorMapping.running });
const bgGreen = ({ backgroundColor: statusColorMapping.succeeded  });
const bgGray = ({ backgroundColor: statusColorMapping.unknown  });
