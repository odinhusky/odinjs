<template>
	<div class="__ag__follow-main__">
		...
		<scroller class="__ag__follow-main-content__" show-scrollbar="false" :class="[ipx ? 'about-ipx' : '']" v-if="followList.length>0">

			<div class="__ag__follow-item__" v-for="item in followList" :key="item.id">
				<div class="__ag__avatar-box__" :class="[item.joinMap.streamer.avatar?'':'grayground']" >
					<image :src="__ag__url__(item.joinMap.streamer.avatar)" class="__ag__avatar-img__" v-if="item.joinMap.streamer.avatar"></image>
					<text class="__ag__firststr__" v-else>{{item.joinMap.streamer.userNicename.slice(0,1)}}</text>
				</div>
				<div class="__ag__info-box__">
					<text class="__ag__upname__">{{item.joinMap.streamer.userNicename}}</text>
				...
				<div class="__ag__follow-button__"  @click="cancelFollow(item)" :class="[item.status==1?'__ag__followed__':'']">
					<text class="__ag__folowedtext__" v-if="item.status==1">已关注</text>
		...
		<div class="__ag__follow-main-content__" :class="[ipx ? 'about-ipx' : '']"  v-else>
	...
	methods: {
		async cancelFollow(item){
			if(item.status==1){
				...
				try{
					let result = await this.__ag__editFollow__({streamerId:item.streamerId})
					if(result.success==true){
							this.followList.find(obj=>{
								if(obj.id == item.id){
									obj.status = 1
...
.__ag__follow-item__ {
	...
	border-bottom-color: rgba(253, 197, 1, 0.3);
}
.__ag__about-logo__ {
	flex: 1 1 280px;
	width: 280px;
...
.__ag__follow-button__{
	...
	width: 140px;
	height: 52px;
	background-image: linear-gradient(to bottom, #FDC401, #F8922C);
	border-radius: 10px;
	margin-right:48px;
}
