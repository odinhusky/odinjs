...
import OnPayPage from "../../components/fund/OnPayPage";
...
const FiatCurrency = () => {
  ...
  useEffect(() => {
    setAd(JSON.parse(localStorage.getItem("buy")!));
    // setInterval(()=>{
    //   api.get(`/otc/api/otcOrder/${JSON.parse(localStorage.getItem("buy")!).id }`).then(x => {
    //       console.log(x)
    //   });
    // },2000)
  }, []);
      ...
      {progressOrder === 0 ? (
        <>
          ...
          <DescriptionContainer>
            ...
            <DescriptionItem>
              <DescriptionText>{t("sellerName")}</DescriptionText>
