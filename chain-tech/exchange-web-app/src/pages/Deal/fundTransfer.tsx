...
import TransferHeader from "../../components/header/transferTradHeader";
...
const SelectInputTextTransfer = styled.p`
  ...
  color: #383743;
`;
const InputTextSplit = styled.div`
  display: flex;
  flex-direction: row;
  align-items: center;
  border:none;
...
const Transfer = () => {
  ...
  function getFund(){
    api.get("/investor/property").then(x=>{
      if(x.status !== 400){
        setFutureBalance(x.data.futures.balance)
      ...
      >
        ...
        {/* {selectStatus === "source" ? (
          <DepthItem
            isSelect={sources}
            index={2}
            onClick={selectHandler.bind(null, 2, selectStatus)}
          >
            法幣資金
          </DepthItem>
        ) : selectStatus === "transfer" ? (
