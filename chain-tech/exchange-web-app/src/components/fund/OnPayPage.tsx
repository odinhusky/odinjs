...
const Contract: React.FC<{
  ...
  payments: any[];
  id: string;
}> = ({ progressOrder, setProgressOrder, setClickButton, payments, id }) => {
  ...
  return (
    <>
      ...
      <PayContentContainer>
        <TitleContainer>
          <PayTitle>{orderStatus === 4 && "等待賣家確認交易"}</PayTitle>
          <PayTitle>{orderStatus === 0 && "請付款"}</PayTitle>
          <PayTitle>{orderStatus === 1 && t("waitSellerTransfer")}</PayTitle>
        ...
        >
          <CancelButton
            ...
            onClick={() => {
              api
                .postData(`/otc/api/otcOrder/${ad.id}/cancel`, { id: ad.id })
                .then(x => {
                ...
                <CheckboxConfirm
                  onClick={() => {
                    ...
                    api
                      ...
                      .then(x => {
                        if (x.status !== 400 && x.status !== 403) {
                          // Modalhandler(orderStatus, 1);
                          // navigation("/deal/fiatBuyComplete")
