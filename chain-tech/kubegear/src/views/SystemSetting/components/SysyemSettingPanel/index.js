...
*/
export default function SystemSettingPanel({
  currentTabIndex,
  systemSetting,
  setSystemSetting,
  history
  ...
  const emptySystemSetting = {
    ...
    isJobNeedVerify: false, // 作業是否要審批 預設不用認證(資料面跟顯示面的結果是相反的)
  ...
  const [isNeedVerifyReverse, setIsNeedVerifyReverse] = useState(!get(systemSettingformData, 'isJobNeedVerify', false));
    ...
    try {
      ...
      if(!isEmpty(sysResReq)) {
        ...
        Object.entries(cells).map(([key, val]) => {
          ...
          // 設定 showData
          const keyName = key === 'gpuMemoryPercentage' ? t('GPU Percentage') : key;
          ...
          >
            ...
            <RadioGroup
              ...
              onChange={(e) => {
                ...
                // 更新 state
                setIsNeedVerifyReverse(isJobNeedVerify);
                setSystemSettingFormData(systemSettingformData => ({ ...systemSettingformData, isJobNeedVerify: !isJobNeedVerify }));
              }}
              row
              value={isNeedVerifyReverse}
            >
              {
