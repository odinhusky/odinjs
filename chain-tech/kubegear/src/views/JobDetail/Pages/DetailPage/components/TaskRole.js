...
const TaskRole = ({
  ...
  const renderTaskRoleCount = () => {
    ...
    return (
      <div className={`${classes.jobDetailTaskRoleCount}`}>
  ...
  return (
    <div
      className={`${classes.jobDetailTaskRoleContainer}`}
      key={key}
    >
      <div className={`${classes.jobDetailTaskRoleHeader}`}>
        <span>{t('task')}{t('enSpace')}{t('role')}:</span>
        <span className={`${classes.ml_20}`}>{data.taskRoleStatus.name}</span>
        {
          taskConfig &&
          <MonacoCallout
            language="json"
      ...
      {
        isExpand &&
          <div className={`${classes.jobDetailTaskRoleContent}`}>
            <TaskRoleList
