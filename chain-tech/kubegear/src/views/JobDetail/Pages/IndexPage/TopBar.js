      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', paddingBottom: '10px' }}>
        <div>
          <PrimaryButton
            children={
              !selectedJobs.length
                ? `${t('submit')}${t('enSpace')}${t('job')}`
                : selectedJobs.some(item => item.state === 'RUNNING' || item.state === 'WAITING') ? t('Stop') : `${t('submit')}${t('enSpace')}${t('job')}`
            }
            ...
            startIcon={<BaseMuiIcon>{
              !selectedJobs.length
                ? 'add_circle'
                : selectedJobs.some(item => item.state === 'RUNNING' || item.state === 'WAITING') ? 'stop' : 'add_circle'
            }</BaseMuiIcon>}
          />
          <DefaultButton
            children={t('refresh')}
