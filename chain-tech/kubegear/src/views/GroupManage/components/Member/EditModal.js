...
/**
 * @author odin
 * @level views/GroupManage/Member/EditModal
 * @component EditModal
...
const EditModal = ({
  ...
  return (
    ...
    >
      <MuiDropdown
        classNameObj={{ container: classes.mt_10 }}
        list={options}
        maxWidth={'100%'}
        multiple
        onChange={(e) => {
        ...
        value={selectedKey}
      />
      <div className={`${classes.groupManageMemberEditModalUserBox}`}>
        <Toolbar
          className={classes.groupManageMemberEditModalTopBar}
          disableGutters
        >
          <Tabs
            aria-label="tabs"
                      ...
                      <Grid item>
                        {
                          isEmpty(resourceCells)
                            ...
                            resourceCells.map(({ key, option, value }, idx) => {
                              ...
                              return (
                                <div
                                  className={`${classes.groupManageMemberEditModalRow}`}
                                  key={key}
                                >
                                  <div>{keyName} - {cells[key]?.number}</div>
                                  <FormControl>
                                    <RadioGroup
                                      aria-label={key}
                                      name={key}
