...
import { computeDayRange, parseGBFormat, parseMBFormat, parseNormalFormat } from '../../../../utils';
...
const SingleNodeDetail = (props) => {
      ...
      return new Promise((reslove) => {
        ...
        Promise.all([
          ...
          taskMemoryTotal = taskMemoryTotal.data.result.map(item => {
            const name = `${item.metric.job_name} - ${item.metric.role_name} - ${item.metric.task_index}`
            const data = item.values.map(item => item.map(parseGBFormat))
            return (
              { name, data }
            )
          })
  ...
  return (
    <div>
      <div>
        <div className={`${classes.clusterReportTitle}`}>
