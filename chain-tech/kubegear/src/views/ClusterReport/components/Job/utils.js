...
import { parseNormalFormat, parseGBFormat, parseMBFormat, createCsvFile } from '../../utils';
...
export const getJobLayerData = async({ username, job = 'job', type, ...params }) => {
  ...
  memory = !isEmpty(memory.data.result[0]) ? memory.data.result[0].values.map(value => value.map(parseGBFormat)) : []
...
export const computeTotalResourceData = (data) => {
  const { cpuTotal, memoryTotal, gpuTotal } = data.reduce((acc, { cells }) => {
    const { cpu, memory, gpu } = Object.entries(cells).reduce((acc, [key, details]) => {
