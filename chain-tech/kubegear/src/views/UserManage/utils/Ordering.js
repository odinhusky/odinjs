...
export default class Ordering {
  ...
  apply(userInfos) {
    ...
    if (field === 'username') {
      ...
      comparator = descending
        ? (a, b) => String(b.roles).localeCompare(a.name)
        : (a, b) => String(a.roles).localeCompare(b.name);
    } else if (field === 'jobNumber') {
      comparator = descending
        ? (a, b) => String(b.roles).localeCompare(a.jobNumber)
        : (a, b) => String(a.roles).localeCompare(b.jobNumber);
    } else if (field === 'email') {
      comparator = descending
        ? (a, b) => String(b.roles).localeCompare(a.email)
        : (a, b) => String(a.roles).localeCompare(b.email);
    } else if (field === 'mobileNumber') {
      comparator = descending
        ? (a, b) => String(b.roles).localeCompare(a.phone)
        : (a, b) => String(a.roles).localeCompare(b.phone);
    } else if (field === 'roles') {
      comparator = descending
