...
import ColumnAdjustmentModal from './components/ColumnAdjustmentModal';
...
import userManageStyle from './userManageStyle';

const useStyles = makeStyles((theme) => ({
  ...commonStyle(theme),
  ...userManageStyle(theme)
}))
...
const UserManage = () => {
  ...
  // ? context
  const { userInfo: currentUser } = useContext(GlobalContext);
  ...
  const [whichStorageSetting, setWhichStorageSetting] = useState({ nfs: false, glusterfs: false });

  // 欄位調整
  const [isColumnAdjustmentModalOpen, setIsColumnAdjustmentModalOpen] = useState(false);
  ...
  const formatTimeString = mseconds => {
    if(isNil(mseconds) || mseconds === 0) return '00:00:00';
  ...
  useEffect(() => {
    if (isUserCreateSuc) {
      if (whichStorageSetting.nfs || whichStorageSetting.glusterfs) {
  ...
  const importUserOptions = [
    ...
    //   icon: <BaseMuiIcon style={{ marginRight: 10 }}>group_add</BaseMuiIcon>,
    //   handleItemclick: importFromCSV
    // },
    {
      id: 'usersTemplate',
      label: `${t('download')}${t('enSpace')}${t('template')}`,
      icon: <BaseMuiIcon style={{ marginRight: 10 }}>download</BaseMuiIcon>,
  ...
  return (
    <UserManageContext.Provider value={context}>
      <div
        className={`
          ${classes.d_flex}
      ...
      >
        <BreadCrumbs />
        <div className={`${classes.flex_justify_between} ${classes.alignItemsStretch}`}>
          <div className={`${classes.d_flex} ${classes.flex_wrap}`}>
            {
              ...
              <>
                ...
                <PrimaryButton
                  ...
                  classNameProps={classes.mr_10}
                  disabled={isDataLoading}
                  onClick={() => setShowCreateUserModal(true)}
                  startIcon={<BaseMuiIcon>person_add_alt</BaseMuiIcon>}
                />

                {/* 批次匯入 */}
                <SplitButton
                  classNameObj={{
                    btnGroup: classes.mr_10
                  }}
                  disabled={isDataLoading}
                  onClick={importFromCSV}
                  options={importUserOptions}
                  startIcon={<BaseMuiIcon>group_add</BaseMuiIcon>}
            ...
            {/* 調整顯示欄位 */}
            <DefaultButton
              children={t('columnAdjust')}
              onClick={() => {setIsColumnAdjustmentModalOpen(true)}}
              startIcon={<BaseMuiIcon children="checkbox" />}
            />
          </div>
        ...
        {/* 表單 */}
        <div className={`${classes.flexGrow1} ${classes.overflowHidden} ${classes.mt_10}`}>
          <Table />
        </div>
      ...
      <ColumnAdjustmentModal
        isOpen={isColumnAdjustmentModalOpen}
        onClose={() => {setIsColumnAdjustmentModalOpen(false)}}
      />
