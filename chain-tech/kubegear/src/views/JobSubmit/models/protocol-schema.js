...
export const taskRoleSchema = Joi.object().keys({
  ...
  shmMB: Joi.number().default(64).min(0).max(Joi.ref('resourcePerInstance.memoryMB')),
  completion: Joi.object().keys({
    minFailedInstances: Joi.number().min(0).max(1).allow(null).default(1),
    minSucceededInstances: Joi.number().min(0).max(1).allow(null).default(null)
  }),
  ...
  resourcePerInstance: Joi.object().required().keys({
    ...
    gpuMemoryPercentage: Joi.number().required(),
...
export const taskRolesSchema = Joi.object().pattern(/^[a-z][a-z0-9]*$/, taskRoleSchema.required());
