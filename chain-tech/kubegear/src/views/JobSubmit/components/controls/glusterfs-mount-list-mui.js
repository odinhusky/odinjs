...
export const GlusterfsMountLists = ({
  name,
  nfsMounts,
  onChange,
  keyOptions,
  ...
  const onKeyChange = useCallback((idx, val) => {
    onChange([...nfsMounts.slice(0, idx), { name: val, mountPoint: `/root/${val}` }, ...nfsMounts.slice(idx + 1)]);
  }, [onChange, nfsMounts]);
                  ...
                  >
                    ...
                    >
                      <DebouncedTextField
                        ...
                        error={isError}
                        helperText={
                          dupList.includes(item.mountPoint)
                            ? t('sameMountPoint')
