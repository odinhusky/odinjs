  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          {
            !isEmpty(limitedVg) && Object.entries(limitedVg).map(([vgName, { resourceCells, jobLifeHour }]) => (
              <div
                className={`${classes.vgUnit}`}
                key={vgName}
              >
                <div className={classes.w_full}>{vgName}</div>

                {
                  !isEmpty(resourceCells) &&
                  Object.entries(resourceCells)
                    .sort((a, b) => (String(a[0]).localeCompare(b[0])))
                    .map(([cellName, limitedNumber]) => {
                      const cellNameStr = cellName === 'gpuMemoryPercentage' ? t('GPU Percentage') : cellName;
