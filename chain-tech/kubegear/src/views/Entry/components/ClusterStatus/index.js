...
import { handlePercentage } from 'common/commonMethods'
...
export default function ClusterStatus() {
  ...
  */
  ...
  /**
   * @author odin
   * @param {array} arr -- 資源的陣列
   * @description 計算出總共用了多少的 cpu gpu memory，並回傳一個物件
   * @returns {object}
  */
  const calculateTotal = (arr) => {
    const totalUsed = arr.reduce((acc, { usedCpu, usedGpu, usedMemory }) => {
      return {
        ...acc,
        totalUsedCPU: acc['totalUsedCPU'] + usedCpu,
        totalUsedGPU: acc['totalUsedGPU'] + usedGpu,
        totalUsedMemory: acc['totalUsedMemory'] + usedMemory
      }
    }, { totalUsedCPU: 0, totalUsedGPU: 0, totalUsedMemory: 0 })
  ...
  */
  useEffect(() => {
    ...
    if(!isNil(selfCanUseVgList) && !isEmpty(selfCanUseVgList)) {
      ...
      subResourceArray = selfCanUseVgList.map(item => {
        ...
        // 子資源分配到的總資源 -- 個別物件
        const distributedObj = Object.entries(cells).reduce((acc, [key, { number }]) => ({ ...acc, [key]: number }), { cpu: 0, gpu: 0, memory: 0 });

        // 子資源已使用的資源 -- 個別物件
        const usedObj = Object.entries(usedCells).reduce((acc, [key, number]) => ({ ...acc, [key]: number }), { cpu: 0, gpu: 0, memory: 0 });
  ...
  */
  useEffect(() => {
    if(!isEmpty(rootResource)) {
      ...
      const systemObj = Object.entries(cells).reduce((acc, [key, { number }]) => ({ ...acc, [key]: number }), { cpu: 0, gpu: 0, memory: 0 });
  ...
  */
  useEffect(() => {
    if(!isEmpty(systemResource) && !isEmpty(subResourceArr)) {
      const { totalUsedCPU, totalUsedGPU, totalUsedMemory } = calculateTotal(subResourceArr);
  ...
  */
  ...
  return (
    <div
      className={`
      ${classes.clusterStatusContainer}
      ${(isShowGuide === true
          && (isAdmin ? currentStep === 6 : currentStep === 5)
          && isSmallTablet === false) && classes.guideShow}
      `}
    >
