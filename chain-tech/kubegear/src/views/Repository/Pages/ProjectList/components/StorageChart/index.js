import React, { useContext } from 'react';

// ? context
import RepositoryContext from 'views/Repository/RepositoryContext';

// ? Self-packed Components || Functions
import { percentColors } from 'constant';
import { GB } from 'constant';

// ^ Plugins
import PropTypes from 'prop-types';
import { useTranslation } from 'react-i18next';

/**
 * @author odin
 * @level views/Repository/ProjectList/StorageChart
 * @component StorageChart
 * @description StorageChart to chart with high chart
*/
const StorageChart = ({ info }) => {

  // $ init data
  const { t } = useTranslation();
  let max = 0;
  let used = 0;

  // ? context
  const { classes } = useContext(RepositoryContext);

  // & handled data
  if (info.storage) {
    const { total, free } = info.storage;

    max = total / GB;
    used = (total - free) / GB;
  }

  // - methods
  const computeColor = (num) => {
    if (num < 0.6) return percentColors[0]
    if (num < 0.8) return percentColors[1]
    return percentColors[2]
  }

  return (
    <div className={`${classes.storageChartContainer}`}>
      <p className={`${classes.storageChartTitle}`}>{t('storage')}</p>
      <div className={`${classes.storageChartChart}`}>
        <span style={{ borderColor: computeColor(used / max), transform: `rotate(${used / max * 180}deg)` }} />
        <b>{used.toFixed(0)}GB</b>
      </div>
      <div className={`${classes.storageChartRange}`}>
        <p>0</p>
        <p>{max.toFixed(0)}</p>
      </div>
    </div>
  );
};

StorageChart.propTypes = {
  info: PropTypes.object
}

export default StorageChart;