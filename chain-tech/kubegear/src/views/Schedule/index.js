...
const ScheduleRoute = () => {
  ...
  */
  const eventContent = arg => {
    ...
    // 套件的參數
    const { isStart, isEnd } = arg;

    // 決定是否為開頭的 event
    const isHideText = (isStart === false && isEnd === true);
  ...
  */
  useEffect(() => {
    ...
    const {
      ...
      gpuMemoryPercentage: limitedGPUMemoryPercentage
    ...
    const {
      ...
      gpuMemoryPercentage: filteredGPUMemoryPercentage
    } = filteredResourceObj;
    ...
    const resultObj = {
      ...
      gpuMemoryPercentage: Math.min(filteredGPUMemoryPercentage, (limitedGPUMemoryPercentage === -1 ? maxSafeNumber : limitedGPUMemoryPercentage))
    };
