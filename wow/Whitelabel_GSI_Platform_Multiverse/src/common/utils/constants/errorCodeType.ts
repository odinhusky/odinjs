export enum Enums {
  ...
  /** 錢包類型停用 */
  P_LAUNCH_GAME_WALLET_TYPE_DISABLE = 404003,
  /** OTP 已過期 */
  ...
  /** 銀行卡身份已綁定至其他會員（銀行） */
  P_BANK_CARD_IDENTITY_ALREADY_BINDED_TO_ANOTHER_MEMBER_FOR_BANK = 407029,
  /** 銀行卡身份已綁定至其他會員（加密貨幣） */
  P_BANK_CARD_IDENTITY_ALREADY_BINDED_TO_ANOTHER_MEMBER_FOR_CRYPTO = 407030,
  /** Email已存在 */
  ...
  /** Session 閒置逾時 */
  CORE_SESSION_IDLE_TIMEOUT = 901011,
  /** 代理 ID 不匹配 */
  ...
  /** 該遊戲商不支援查看詳情明細（第三方注單詳細查詢失敗 get game history failed） */
  P_GET_GAME_HISTORY_FAILED = 332001,
   ...
   */
  /** 利息寶申請錯誤 */
  P_INTEREST_APPLY_ERROR = 400003001
}
...
export const I18nKeys: Partial<Record<Enums, string>> = {
  ...
  [Enums.P_LAUNCH_GAME_WALLET_TYPE_DISABLE]: "error_message.P_LAUNCH_GAME_WALLET_TYPE_DISABLE",
  ...
  [Enums.P_BANK_CARD_IDENTITY_ALREADY_BINDED_TO_ANOTHER_MEMBER_FOR_BANK]:
    "error_message.P_BANK_CARD_IDENTITY_ALREADY_BINDED_TO_ANOTHER_MEMBER_FOR_BANK",
  [Enums.P_BANK_CARD_IDENTITY_ALREADY_BINDED_TO_ANOTHER_MEMBER_FOR_CRYPTO]:
    "error_message.P_BANK_CARD_IDENTITY_ALREADY_BINDED_TO_ANOTHER_MEMBER_FOR_CRYPTO",
  ...
  [Enums.CORE_SESSION_IDLE_TIMEOUT]: "error_message.CORE_SESSION_IDLE_TIMEOUT",
  ...
  [Enums.P_GET_GAME_HISTORY_FAILED]: "error_message.P_GET_GAME_HISTORY_FAILED",
   ...
   * =========== 利息寶相關 ===========
   */
  [Enums.P_INTEREST_APPLY_ERROR]: "error_message.error_message_400003001"
}

/**
 * 本地後備錯誤文案（依語系，key 為 languageStore 的 locale，如 `zh-tw`、`zh-cn`、`en`）。
 *
 * 用途：當遠端 i18n 平台尚未佈署對應 `I18nKeys` 翻譯時，改用此處文案，確保錯誤提示可正確顯示；
 * 一旦遠端補上翻譯，`useApi` 會優先採用遠端翻譯（見 useApi 的 default 分支）。
 */
export const LocalFallbackMessages: Partial<Record<Enums, Record<string, string>>> = {
  [Enums.P_GET_GAME_HISTORY_FAILED]: {
    en: "This game provider does not support viewing bet details",
    "zh-tw": "該遊戲商不支援查看詳情明細",
    "zh-cn": "该游戏商不支持查看详情明细",
    th: "ผู้ให้บริการเกมนี้ไม่รองรับการดูรายละเอียดการเดิมพัน",
    id: "Penyedia game ini tidak mendukung tampilan detail taruhan",
    vi: "Nhà cung cấp trò chơi này không hỗ trợ xem chi tiết cược",
    jp: "このゲームプロバイダーは明細の詳細表示に対応していません",
    ko: "이 게임 제공업체는 상세 내역 보기를 지원하지 않습니다",
    sp: "Este proveedor de juegos no admite ver los detalles de la apuesta",
    br: "Este provedor de jogos não suporta a visualização de detalhes da aposta",
    bm: "Penyedia permainan ini tidak menyokong paparan butiran pertaruhan",
    bn: "এই গেম প্রদানকারী বাজির বিবরণ দেখা সমর্থন করে না",
    ar: "لا يدعم مزود اللعبة هذا عرض تفاصيل الرهان",
    my: "ဤဂိမ်းဝန်ဆောင်မှုသည် အသေးစိတ်ကြည့်ရှုခြင်းကို မပံ့ပိုးပါ",
    tl: "Hindi sinusuportahan ng game provider na ito ang pagtingin sa detalye ng taya",
    fr: "Ce fournisseur de jeux ne prend pas en charge l'affichage des détails du pari",
    sw: "Mtoa huduma huyu wa mchezo hairuhusu kuona maelezo ya dau",
    hi: "यह गेम प्रदाता दांव विवरण देखने का समर्थन नहीं करता"
  }
}
