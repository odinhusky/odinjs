<template>
  <q-dialog v-model="isShow" transition-show="fade" transition-hide="fade" :transition-duration="'0'" :maximized="true">
    <q-card :class="['full-screen-container', variant && `variant-${variant}`]">
      <div :class="{ 'bg-black-control': isBgBlack }">
        <q-card-section class="h5-vip-header">
          ...
          <div v-if="props.isShowLineCover" class="h5-header-bg-line-cover"></div>
        </q-card-section>
      </div>
      <slot name="body"></slot>
    </q-card>
...
// TODO:Husky 應該要抽變數，但目前變數規則還沒有統一，而且也應該要跟 variants 一樣，判斷不同的背景，替換不同的背景圖
.bg-black-control {
