  ...
  >
    ...
    <template v-slot:after>
      <template v-if="isCash">
        <q-btn round dense flat class="wallet" @click="props.onAction ? props.onAction() : openPopup()">
          <img v-if="$q.platform.is.mobile" :src="commonSvgIcon('money')" />
          <p v-else>{{ $t("common.btn.deposit") }}</p>
        </q-btn>
      </template>
    </template>
  </q-select>
