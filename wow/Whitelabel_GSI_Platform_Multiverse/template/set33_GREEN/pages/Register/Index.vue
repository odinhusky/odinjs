...
useBasicInfoQuery()
const {} = useCommonImg()
const { isPhoneRegisterMode, isRegisterOtpEnabled, defaultCountryCode } = useEnv()
...
async function register() {
  ...
  if (isPhoneRegisterMode.value) {
    ...
    // 分開發送手機號碼和國碼
    const registerSmsData: Partial<Request.RegisterSms> & Pick<Request.register, "fullname"> = {
      register_method: formRegister.value.register_method ?? envInfoStore.envInfo.registerMethod,
      phone: formRegister.value.phone,
      country: formRegister.value.country,
      ...(shouldMirrorRegisterAccountName.value ? { fullname: formRegister.value.fullname } : {}),
    }
    if (isRegisterOtpEnabled.value) { registerSmsData.sms_otp = formRegister.value.sms_otp }
