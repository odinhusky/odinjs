  ...
  >
    ...
    >
      ...
      <div class="gift-card-container">
        ...
        <q-badge class="absolute top-24 right-8 text-lg bg-gold-linear-gradient" rounded :label="giftsList.length" />
        <span class="gift-title">{{ t("promotion.pick_and_win") }}</span>
        <q-img width="200px" :src="claimGiftImg('giftBox.png')" loading="lazy" />
        <q-btn
          class="gift-confirm-btn mt-10 px-6 text-lg bg-gold-linear-gradient"
          unelevated
          dense
          rounded
  ...
  <q-dialog v-model="claimDialog" persistent transition-show="scale" transition-hide="scale" class="claim-gift-dialog">
    <q-card v-if="giftsList.length" class="claim-gift-card p-4 !rounded-xl" style="width: 600px">
      <q-card-section class="flex justify-between items-center">
        <div v-if="giftsList[0].options.length > 1" class="text-bold text-2xl text-white">
          {{ t("promotion.select_currency") }}
        </div>
        <div v-else class="text-bold text-2xl text-white">{{ t("promotion.congratulations") }}</div>
        <q-btn
      ...
      <q-card-actions align="center">
        <q-btn
          class="mt-4 mb-4 !px-8 text-xl bg-gold-linear-gradient"
          unelevated
          dense
          rounded
...
.gift-card-container {
  ...
  .gift-title {
    ...
    background-image: linear-gradient(252.72deg, #f8e8bd 35.55%, #edb266 65.06%);
  }
