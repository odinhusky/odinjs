<template>
  <HeaderTitleBack v-if="isMobile" :titleI18n="bankCardId ? 'common.btn.edit' : 'common.btn.add'" variant="blackGold">
    <q-form class="bank-add-layout h5" :class="{ '!pb-[24rem]': isTelegramMiniApp }" @submit="handleSubmit">
      ...
      <div class="withdraw-container">
        <template v-if="bankCardState.form.payment_type_id === FUND_METHOD_TYPE.Enums.BankTransfer">
          <div class="form-item row input-row p-5">
            ...
            <!-- account name -->
            <div class="row col-12 col-sm-5">
              <div class="form-title label col-12">{{ $t(getBankCardRealNameLabel('member.bank.accountName')) }}</div>
              <div class="input-content col-12">
                <q-input
      ...
      <div class="action-btns row justify-center">
        <q-btn text-color="white" color="primary" label="Confirm" class="submit-btn btn-submit" type="submit" :disable="bankCardRealNameSubmissionBlocked" />
      </div>
    </q-form>
  </HeaderTitleBack>
        ...
        <div class="profile-form column">
          ...
          <q-form class="form-content" @submit="handleSubmit">
            ...
            <div class="action-btns multi-btns row justify-center no-wrap">
              <q-btn :label="$t('common.btn.cancel')" class="cancle-btn btn-reset" @click="goMemberBank" />
              <q-btn :label="$t('common.btn.submit')" class="submit-btn btn-submit" type="submit" :disable="bankCardRealNameSubmissionBlocked" />
            </div>
          </q-form>
        </div>
      </div>
...
onMounted(async () => {
  ...
  if (bankCardId.value) {
    ...
    // getBankCardInfo 會設置 payment_gateway_id，這會觸發 watch 調用 getBankList
    // 如果沒有 payment_gateway_id，watch 也會調用 getBankList(undefined)
    // 所以不需要在這裡再次手動調用 getBankList
  } else {
    // 新增模式：使用共用函數初始化幣別
...
.input-content {
  ...
  :deep(.q-field--error .q-field__marginal),
  :deep(.q-field--error .q-select__dropdown-icon) {
    color: rgb(193, 0, 21) !important;
  }
