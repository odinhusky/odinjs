      ...
      <div v-else class="forget-password-form form-sms">
        <q-form @submit.prevent="forgetPasswordSms">
          <div class="form-content">
            ...
            <div class="action-btns row justify-center">
              <q-btn class="btn-submit forgot-password-btn" :loading="isLoading">
                {{ $t("common.btn.submit") }}
              </q-btn>
            </div>
          </div>
...
import BannerMember from "app/template/okbet_blackGold/components/BannerMember.vue"
const $q = useQuasar()
const Rules = useRule()
const { t } = useI18n()
...
async function forgetPasswordSms() {
  ...
  router.push({
    name: "QuickPass",
    params: {
      account: data.account || smsData.phone,
      token: data.token
    }
  })
}
