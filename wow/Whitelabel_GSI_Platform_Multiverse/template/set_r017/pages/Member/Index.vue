<template>
  ...
  >
    <div
      v-if="!isMembershipManagementRoute"
    ...
    <div id="member-center-layout" class="w-full flex flex-nowrap gap-5 min-h-[39.9375rem] phone:min-h-0">
      <MemberNav v-if="!isMembershipManagementRoute" class="phone:hidden">
        <template #header>
...
import { computed, markRaw, onMounted, provide, reactive, watch } from "vue"
...
const isMembershipManagementRoute = computed(() => route.name === "MembershipManagement")
...
const setMemberNavs = () => {
  memberNavStore.setNavs([
      ...
      show: true },
    {
      ...
      show: true },
    {
      i18nKey: "menu.interest",
      routerName: "memberInterest",
      activeRouteName: ["memberInterest"],
      icon: MEMBER_NAV_ICON_TYPE.Enums.INTEREST,
      show: isCash.value },
    {
      i18nKey: "menu.membershipManagement",
...
watch(isMemberAgent, setMemberNavs, { immediate: true })
