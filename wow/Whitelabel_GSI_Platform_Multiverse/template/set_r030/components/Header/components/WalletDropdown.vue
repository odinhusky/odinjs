<template>
  ...
  >
    ...
    <template v-slot:after>
      <q-btn round dense flat class="wallet" @click="router.push({ name: 'MemberDeposit' })">
        <!-- <img v-if="$q.platform.is.mobile" :src="svgIcon('money')" /> -->
        <img v-if="$q.platform.is.mobile" :src="svgIcon('deposit')" alt="" />
        <p v-else>{{ $t("menu.deposit") }}</p>
      </q-btn>
    </template>
  </q-select>
</template>
