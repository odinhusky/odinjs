<template>
  ...
  <div class="relative cursor-pointer flex justify-center" @click="selectFile">
    <q-avatar size="48px" class="border-2 border-[var(--bg-line-01)]">
      <img :src="getUserAvatar(svgIcon('icon_avatar'))" class="object-cover w-full h-full" />
    </q-avatar>
    <img v-if="!hasAvatar" :src="svgIcon('avatat-edit')" class="absolute bottom-0 right-0 w-4 h-4" />
  </div>
</template>
