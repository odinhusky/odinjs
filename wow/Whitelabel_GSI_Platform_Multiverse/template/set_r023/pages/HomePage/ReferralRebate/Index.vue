...
<!--
  ...
  DateRangePicker 的 dateDark 現在會自動跟隨 Quasar dark mode，
  但背景色仍需用全局樣式對應 set_r023 的主題色。
-->
<style lang="scss">
...
.body--light {
  .q-date {
    background: var(--primary-07); // #f6f7f8 淺灰
    color: var(--secondary-01); // #0d2245 深藍黑

    .q-date__header {
      background: var(--primary-01); // #025be8 藍
      color: #ffffff;
    }
...
// 問題根源：DateRangePicker 把 --q-primary: #1976D2（預設藍）注入到包裝 div，
// Quasar 的 .bg-primary { background: var(--q-primary) !important } 就吃這個藍色。
// 解法：在 .q-date 層級直接重新定義 --q-primary 為紫色，讓 .bg-primary 自動跟著改。
.body--dark {
  .q-date {
    // 覆蓋 --q-primary，讓 Quasar 內部所有用 .bg-primary / var(--q-primary) 的地方都變紫色
