<template>
  <BannerWebInformation />
  <div class="web-contact-content">
    <ul v-if="cmsContactUsList.length > 0" class="row text-center contact-us-header_inner">
      <li
        v-for="(cmsItem, cmsIndex) in cmsContactUsList"
        :key="cmsIndex"
        class="contact_dotted"
        @click="handleEntranceClick({ entrance: cmsItem.Entrance[0] })"
      >
        <q-img :src="cmsItem.Setting.icon_lang[nowLang as LANGUAGE_TYPE.Enums]" class="contact-img" contain />
        <span class="contact-text">{{ cmsItem.Setting.contact_lang[nowLang as LANGUAGE_TYPE.Enums] }}</span>
      </li>
    </ul>
  </div>
</template>

<script setup lang="ts">
import BannerWebInformation from "app/template/set_jokerhill/components/BannerWebInformation.vue"
import { useEntranceHandler } from "app/template/set_jokerhill/composables/useCms"
import { useCms } from "src/common/composables/useCms"
import { useLanguage } from "src/common/composables/useLanguage"
import { LANGUAGE_TYPE } from "src/common/utils/constants"

const {
  cmsContactUsList,
} = useCms()
const { nowLang } = useLanguage()
const { handleEntranceClick } = useEntranceHandler()

</script>

<style lang="sass" scoped>
@import 'src/common/css/_variable.sass'

.web-contact-content
  max-width: 1366px
  padding: 0
  margin: 0 auto
  +iphone-width
    width: 100%
    padding: 0
  .contact-us-header_inner
    padding-top: 100px
    +iphone-width
      padding-top: 30px
    .contact_dotted
      border-right: 1px dotted #6288D2
      display: flex
      flex-direction: column
      justify-content: center
      width: 33%
      align-items: center
      font-size: 18px

      img
        height: 100px
        width: 100%
        vertical-align: middle
        +iphone-width
          height: 50px
      i
        font-size: 81px
        +iphone-width
          font-size: 55px
      a
        color: #6288D2
        font-size: 14px
      span
        @apply mb-4
        +pad-width
          margin: 0 1.25rem 1rem 0.5rem
      +iphone-width
        border-right: none
        flex-direction: row
        justify-content: flex-start
        width: auto
        border-bottom: 1px dotted #6288D2
        margin-bottom: 10px
        padding-bottom: 10px
        padding-left: 25px
    +iphone-width
      flex-direction: column
      margin-bottom: 20px
.contact-img
  @apply max-w-[6.25rem] max-h-[6.25rem] mb-4
  +iphone-width
    @apply max-w-[3.125rem] max-h-[3.125rem]
.contact-text
  @apply mb-4 ml-4
</style>
