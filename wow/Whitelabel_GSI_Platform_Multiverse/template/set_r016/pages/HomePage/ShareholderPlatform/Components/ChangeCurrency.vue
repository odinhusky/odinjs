...
const { shareholderState, teamOverViewState } = useShareholder()
type WalletDropItem = {
  label: string
  value: string
}
...
const changeCurrency = (item: WalletDropItem) => {
  ...
  shareholderState.query.currency_id = Number(item.value)
  teamOverViewState.query.currency_id = Number(item.value)
}

onMounted(() => {
  shareholderState.query.currency_id = Number(walletDropdown.value[0].value)
  teamOverViewState.query.currency_id = Number(walletDropdown.value[0].value)
})
</script>
