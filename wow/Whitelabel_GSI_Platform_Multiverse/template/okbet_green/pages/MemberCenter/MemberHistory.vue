<template>
            ...
            >
              <template v-slot:header>
                <div class="history-card-header">
                  <div class="history-card-header-side">
                    <span class="history-card-main">{{
                      dateformat(history.updated_at ?? "", "YYYY-MM-DD HH:mm:ss")
                    }}</span>
                    <span class="history-card-sub">{{ $t("tableHeader.accountChangeTime") }}</span>
                  </div>
                  <div class="history-card-header-side is-right">
                    <span class="history-card-main">{{ moneyFormat(history.after_balance) }}</span>
                    <span class="history-card-sub">{{ $t("tableHeader.amountAfterChange") }}</span>
                  </div>
                </div>
              </template>
              <q-card class="history-card-body">
                <q-card-section>
  ...
  <div v-else class="pc-history-container">
    <q-form class="w-full min-w-0" @submit="handleSubmit" @reset="handleReset">
      <img :src="memberImg('history.png')" alt="statement" class="img-order" />
      <!-- 類型切換 -->
      <q-tabs v-model="historyState.query.search_type" outside-arrows mobile-arrows class="type-tabs">
        <q-tab v-for="item in searchTabs" :key="item.value" :name="item.value">
          {{ $t(item.label) }}
        </q-tab>
      </q-tabs>
      ...
      .history-mobile-card {
        ...
        .history-card-body {
          ...
          .q-card__section {
            ...
            gap: 1rem;
            padding: 0.5rem 0;
            border-bottom: 1px solid rgba(202, 221, 201, 0.8);
