<template>
  <div v-if="gameTypeState.list" class="game-tab-wrapper">
    <div class="game-tab-content">
      ...
      <q-btn
        v-for="item in sportsHiddenGameTypeList"
        :key="item.id"
        class="game-tab"
...
const sportsHiddenGameTypeList = computed(() => {
  return gameTypeState.list.filter((item): boolean => item.id !== GAME_TYPE.Enums.SPORTBOOK)
})
...
@media (max-width: 767px) {
  ...
  .game-tab-wrapper .game-tab-content {
    width: max-content;
  }
}
</style>
