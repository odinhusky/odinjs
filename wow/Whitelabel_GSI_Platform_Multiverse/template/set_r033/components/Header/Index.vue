<template>
  <div class="header" :class="{ fixed: !props.modelValue }">
    ...
    <div class="header-right">
      <WalletDropdown v-if="isLogin" :onAction="() => router.push({ name: 'MemberDeposit' })" />
      ...
      <div v-if="isUp.phone && isLogin" class="gap-4 flex items-center justify-center">
        <q-btn flat class="menuft">
          <q-icon name="person" style="color: var(--neutral-03)" />
...
import WalletDropdown from "src/common/components/WalletDropdown/Index.vue"
...
.header {
  ...
  background: var(--bg-header, #0c0b0a);
  flex-direction: row;
  padding: 1rem;
