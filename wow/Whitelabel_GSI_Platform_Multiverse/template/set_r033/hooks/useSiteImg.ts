...
export function useSiteImg() {
  ...
  // Referral 頁面（從 okbet 帶過來）
  const mobileBackBtnImg = computed(() => resultImages("mobile-back-btn.png"))
  const mobileExpansionDetailImg = computed(() => resultImages("mobile-expansion-detail.png"))
  const referralBannerImg = computed(() => resultImages("referral-banner.jpg"))
  const referralEditIcon = computed(() => resultImages("referral-edit.png"))
