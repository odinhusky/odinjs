<template>
  <HeaderTitleBack v-if="isMobile" titleI18n="member.profile.myDetails" variant="red">
    <div class="profile-container h5">
      ...
      <div class="info-container logout">
        <q-btn :loading="isLoading" @click="logout" color="primary" text-color="black">{{ $t("common.btn.logout") }}</q-btn>
      </div>
    </div>
            ...
            <template v-if="needCountryCodePhone && dialog.info.column_name === COLUMN_NAME.Enums.PHONE">
              <div class="w-full p-5 flex flex-nowrap gap-2">
                <q-select
                  ...
                  class="w-[45%] pc-profile-input white-input-error custom-border"
                ...
                <q-input
                  ...
                  dense
                  class="dialog-form-input w-[55%] pc-profile-input white-input-error custom-border"
        ...
        >
          <div v-if="item.column_name === COLUMN_NAME.Enums.PHONE" class="input-container flex flex-nowrap gap-2">
            <q-select
              v-if="columnCountryCode"
              v-model="userInfoForm2[columnCountryCode.column_name]"
              class="w-[45%] custom-border white-input-error pc-profile-input"
              :class="{ 'select-placeholder': !userInfoForm2[columnCountryCode.column_name] }"
              :options="columnCountryCode.values"
              emit-value
              map-options
            ...
            >
              <template v-slot:before>
                ...
                <div v-else class="text-base font-bold">
                  {{ item.column_label || item.column_name }}
                </div>
              </template>
            </q-select>
            <q-input
              v-model="userInfoForm2[item.column_name]"
              class="w-[55%] custom-border white-input-error pc-profile-input"
              dense
...
.profile-container.pc {
  ...
  .profile-body {
    ...
    .q-toggle {
      :deep(.q-toggle__label) { color: #ffffffa6; }
    }
...
.edit-dialog {
  .dialog-card {
    ...
    .dialog-header {
      ...
      .btn-cancel { color: $text-sky-gray-color; }
      .title {
        font-weight: bold;
        max-width: 20rem;
        overflow: hidden;
    ...
    .dialog-body {
      .dialog-form-row {
        ...
        padding: 20px;
        height: 134px;
        :deep(.q-field__control) { color: $text-pale-gray-color !important; }
        .dialog-form-title {
          font-weight: 400;
          line-height: 2.87;
