      ...
      <div class="flex toolbar column">
        ...
        <div class="flex items-center justify-end mb-1 toolbarTop" v-else>
          <div class="flex items-center self-center user-wallet">
            <div class="user-info-bar flex items-center">
              ...
              <span class="normal-case no-ripple ml-1">{{ $t("home.welcome") }}</span>
              <q-btn
                flat
                :ripple="false"
                class="px-1 font-bold normal-case user-name text-yellow no-ripple ml-1"
        ...
        <div class="flex toolbarBottom justify-start">
    ...
    <q-scroll-area class="fit">
      ...
      <q-list>
        ...
        <q-expansion-item v-if="isLogin" expand-separator icon="person" :label="$t('menu.myProfile')">
          <q-card>
            <q-list class="text-white bg-dark">
              <template v-for="(menuItem, index) in profileList.slice(0, 6)" :key="index">
                <q-item clickable v-ripple @click="menuLink(menuItem)">
                  <q-item-section avatar> </q-item-section>
                  <q-item-section>
                    {{ $t(`menu.${menuItem.i18n}`) }}
        ...
        <!-- <q-item clickable v-ripple @click="router.push({ name: 'MemberHistory' })">
            <q-item-section avatar>
              <q-img :src="iconSvg('history')" alt="history" spinner-color="white" class="text-white nav-img" />
            </q-item-section>
            <q-item-section>
              {{ $t(`menu.history`) }}
            </q-item-section>
          </q-item>
          <q-item clickable v-ripple @click="router.push({ name: 'MemberOrder' })">
            <q-item-section avatar>
              <q-img :src="iconSvg('rewards')" alt="order" spinner-color="white" class="text-white nav-img" />
            </q-item-section>
            <q-item-section>
              {{ $t(`menu.order`) }}
            </q-item-section>
          </q-item> -->
        <template v-for="(cmsItem, cmsIndex) in menuList" :key="cmsIndex">
        ...
        <template v-for="(menuItem, index) in profileList.slice(6)" :key="index">
          <div v-if="isLogin">
            <q-item clickable v-ripple @click="menuLink(menuItem)">
              <q-item-section avatar>
...
@import "app/template/set33_RED/assets/css/header.sass"

section.header-menu
  width: 100%
  position: relative
  box-shadow: 2px 4px 20px #000000ba
  background: linear-gradient(180deg, rgba(245, 233, 134, 1) 0%, rgba(196, 162, 66, 1) 100%)
  +pad-width
    display: none
  .main-menu
    padding: 0 0px
