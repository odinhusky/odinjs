      ...
      <template v-slot:header>
        ...
        <q-tr>
          ...
          <q-th
            ...
            class="text-center bg-green"
            >返傭金額(含所有下載)</q-th
          >
          <q-th v-if="referralConfigData?.reward_mode !== 'cpa'" rowspan="2" class="text-center bg-green">功能</q-th>
        </q-tr>
        <!-- 第二行子表頭 -->
        <q-tr v-if="referralConfigData?.reward_mode !== 'cpa'">
          <q-th
              ...
              <table>
                <tbody>
                  <tr
                    v-for="(currency, key) in revenueColumns"
                    :key="key"
                    v-show="referralConfigData?.reward_mode !== 'cpa'"
                  >
                    <td>
                  ...
                  <tr v-show="referralConfigData?.reward_mode !== 'cpa'">
...
const baseColumns = [
  {
    name: "billing_date",
    field: "billing_date",
    label: t("menu.settlementDate"),
    align: "center" as const,
    format: (val: unknown) => formatDate(val)
  },
...
const columns = computed(() => {
  // 純 CPA（固定獎金）模式隱藏幣別比例與詳情欄位
  if (referralConfigData.value?.reward_mode === "cpa") {
    return baseColumns
  }
