  ...
  <q-header>
    <div class="header-area">
      <div class="header-area-left">
        <img
          :src="resultImages('header/menu.png')"
          class="menu-btn"
          alt="Menu"
          @click="toggleDrawer"
        />
      ...
      <div class="header-area-center">
        ...
        <div class="center-right-area">
          ...
          <template v-else>
            <q-btn class="login-btn" color="primary" @click="showLoginOrRegister(0)">
              {{ $t("common.btn.login") }}
            </q-btn>
            <q-btn v-if="isCash" class="register-btn" color="primary" @click="showLoginOrRegister(1)">
          ...
          <div v-if="isLogin" class="user-info-modal">
            <template v-for="(item, index) in userInfoModalList" :key="index">
              ...
              >
                <img
                  v-if="!!item.imgUrl"
                  :src="resultImages(`header/${item.imgUrl}`)"
                  :alt="item.label || ''"
                />
                {{ item.label }}
              </div>
...
const showLoginOrRegister = (type: number) => {
  if (!type) {
...
const goToPage = (name: string) => {
  const routeName = MENU.WalletTypeToRouteName[name] || "MemberDeposit"
