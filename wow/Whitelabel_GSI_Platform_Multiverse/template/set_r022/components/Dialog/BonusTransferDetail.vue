<template>
  <q-dialog v-model="dialog" persistent transition-show="fade" transition-hide="fade">
    <q-card class="bonus-transfer-dialog" :class="{ h5: isMobile, 'bonus-transfer-dialog--dark': isDarkMode }">
        ...
        <template v-else-if="bonusTransferStatus">
          ...
          <div class="bonus-condition-list">
            ...
            >
              ...
              <q-btn
                v-if="!condition.passed && condition.action"
                class="bonus-condition-row__action"
                flat
                no-caps
                :ripple="false"
                @click.stop.prevent="goToConditionAction(condition.action)"
              >
                {{ $t("common.btn.go") }}
              </q-btn>
...
type ConditionRouteName = "home" | "memberVip" | "Referral"
...
const conditionRows = computed<BonusConditionRow[]>(() => {
  ...
  return conditions.map((condition, index) => {
    ...
    return {
      ...
      action: conditionConfig?.action || null,
...
const goToConditionAction = async (action: ConditionAction): Promise<void> => {
  closeDialog()
...
.bonus-transfer-dialog {
  ...
  overflow: hidden;
  border-radius: 8px;
  background: var(--dialog-bg-02, #ffffff);
  box-shadow: 0 8px 24px #00000024;
  color: var(--dialog-text-01, #6b7280);
  font-family: "Noto Sans TC", sans-serif;
}

.bonus-transfer-dialog--dark {
  background: var(--neutral-01, #143241);

  .bonus-progress-panel {
    background: linear-gradient(90deg, #0d2533 0%, #215a78 100%);

    &::before {
      background: linear-gradient(
        134.58deg,
        rgba(255, 255, 255, 0.18) -25.17%,
        rgba(45, 126, 255, 0.6) -0.77%,
        rgba(255, 255, 255, 0.18) 67.58%,
        rgba(45, 126, 255, 0.6) 102.59%
      );
    }
  }

  .bonus-condition-row__progress-text {
    color: var(--neutral-05, #8a8a8a);
  }
...
.bonus-transfer-summary {
  ...
  overflow: hidden;
  border-radius: 8px;
  background: var(--neutral-04, #f1f1f1);
}
...
.summary-card__icon {
  ...
  &::before {
    ...
    background: linear-gradient(105.42deg, #bec7dc 0%, #1677ff 100%);
    content: "";
...
.bonus-progress-panel {
  ...
  border-radius: 8px;
  background: linear-gradient(90deg, #f6f7f8 0%, #dfdfdf 100%);
  ...
  &::before {
    ...
    background: linear-gradient(
      134.58deg,
      rgba(190, 199, 220, 0.6) -25.17%,
      rgba(2, 91, 232, 0.6) -0.77%,
      rgba(190, 199, 220, 0.6) 67.58%,
      rgba(2, 91, 232, 0.6) 102.59%
    );
    content: "";
    mask: linear-gradient(#ffffff 0 0) content-box, linear-gradient(#ffffff 0 0);
    mask-composite: exclude;
...
.bonus-transfer-all-btn {
  ...
  border: 0;
  border-radius: 8px;
  background: var(--primary-09, #74aaff);
  color: #ffffff;
  cursor: pointer;
  font-size: 13px;
  font-weight: 800;
...
.bonus-condition-row__icon-mask {
  ...
  width: 20px;
  height: 20px;
  background: var(--icon-04, var(--primary-01, #025be8));
...
.condition-progress-track {
  ...
  height: 7px;
  overflow: hidden;
  border: 1px solid var(--primary-09, #74aaff);
  border-radius: 7px;
  background: #025be814;
}
...
.bonus-transfer-description__title {
  ...
  :deep(.q-icon) {
    font-size: 16px;
    color: var(--card-card-icon-secondary-enabled, #f97316);
  }
}
...
.bonus-transfer-description__list {
  ...
  padding-left: 15px;
  font-size: 14px;
  color: var(--text-text-title, #ffffffbf);
  line-height: 1.35;
}
