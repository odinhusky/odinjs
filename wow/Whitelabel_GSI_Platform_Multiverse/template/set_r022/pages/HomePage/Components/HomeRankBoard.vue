        ...
        <q-tab-panel v-for="(type, typeKey) in rankTypes.types" :key="typeKey" :name="type.value">
          <div class="rank-table">
            <div class="rank-thead">
              <div class="rank-tr">
                <div v-for="(item, key) in visibleHeaders" :key="key" class="rank-th">{{ $t(item.i18nLabel) }}</div>
              </div>
            </div>
            <div v-if="visibleItems.length" class="overflow-hidden">
              <transition-group appear enter-active-class="animated slideInDown" :key="currentIndex">
                <div
                  ...
                  :class="hidePayoutColumn ? 'rank-item--cols-3' : 'rank-item--cols-4'"
                  @click="
                    eventbus.emit(
                      'openBetDetail',
                      true,
...
const hidePayoutColumn = computed(() => isPHBM.value && tab.value === RANK_TYPE.latestBet)
...
const visibleHeaders = computed(() => {
  ...
  return hidePayoutColumn.value ? base : [...base, { i18nLabel: "common.payout" }]
})
...
.rank-board-wrapper {
  ...
  .rank-thead {
    ...
    width: 100%;
    height: 3.125rem;
    background: linear-gradient(to bottom, var(--neutral-01) 0%, var(--neutral-04) 100%);
  ...
  .rank-item {
    @apply grid gap-[.5rem] p-[.5rem] items-center;
