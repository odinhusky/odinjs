<template>
  <HeaderTitleBack v-if="isDown.pc" titleI18n="member.profile.myDetails" variant="blueOrange">
          ...
          >
            <template v-if="item.column_name === COLUMN_NAME.Enums.PHONE">
              <div @click="handleDialogOpen(item)" class="info-row">
                <p class="title">{{ item.column_label || item.column_name }}</p>
                <p class="content">
      ...
      <q-card class="dialog-card">
        <q-form @submit="handleH5Submit" @reset="closeDialog">
          <q-card-section class="dialog-header">
            ...
            <span class="title">{{ dialog.info?.column_label || dialog.info?.column_name }}</span>
            ...
            <template v-if="needCountryCodePhone && dialog.info.column_name === COLUMN_NAME.Enums.PHONE">
              <div class="w-full p-5 flex flex-nowrap gap-2">
                ...
                <q-input
                  ...
                  dense
                  class="dialog-form-input w-[55%] md:w-4/6"
...
const selfExclusionColumn = computed<Response.MemberColumn>(() => {
  return {
    ...
    column_label: t("member.profile.disableAccountDate"),
    lang: { [nowLang.value]: t("member.profile.disableAccountDate") },
    required: false,
    edit: true,
    customize: false,
...
const dateOptions = (date: string) => { return dayjs(date).isAfter(dayjs()) }
...
async function handleH5Submit() {
  if (needCountryCodePhone.value) {
    ...
    Promise.all([setSingleUserInfo(dialog.info.column_name), setSingleUserInfo(COLUMN_NAME.Enums.COUNTRY)]).then(
      (result) => {
        if (result.every((e) => e.status === true)) { dialog.show = false }
      }
    )
    return
  }
...
watch(disableAccount, () => { userInfoForm2.self_exclusion_at = null })
...
.profile-container {
  ...
  :deep(.q-field__marginal) { color: $neutral-01 !important; }

  :deep(.q-field__native) { color: $neutral-01 !important; }

  :deep(.q-input .q-field__control) { color: $neutral-01 !important; }

  :deep(.q-field__control):before { border-bottom-color: $functional-line !important; }

  :deep(.q-field__control):after { border-bottom-color: $functional-line !important; }
}
...
.profile-container.pc {
  ...
  .profile-body {
    ...
    .input-container {
      ...
      &.long { width: 98%; }
      :deep(.q-field__before) { padding-right: 14px; }
      ...
      .select-placeholder {
        :deep(.q-field__inner) {
          .q-field__control {
            .q-field__native {
              span { color: $neutral-01 !important; }
            }
          }
        }
      }
...
.profile-container.h5 {
  ...
  @include phone-width { row-gap: 0.5rem; }
  .avator-container {
    width: 100%;
    height: 12.3125rem;
    position: relative;
    display: inline-grid;
    background: $secondary-card;
    @include phone-width { height: 6.875rem; }
    .title {
      position: absolute;
...
.edit-dialog {
  .dialog-card {
    ...
    .dialog-body {
      ...
      :deep(.q-select__dropdown-icon) { color: $neutral-01 !important; }
      :deep(.q-placeholder) { color: rgba($neutral-01, 0.6) !important; }
      :deep(.q-field--standard .q-field__control:before) { border-bottom: 1px solid $functional-line !important; }
      .dialog-form-row {
        width: 100%;
        padding: 20px;
        height: 134px;
        :deep(.q-field__control) { color: $neutral-01 !important; }
        :deep(.q-field__control):before { border-bottom-color: $functional-line !important; }
        :deep(.q-field__control):after { border-bottom-color: $functional-line !important; }
        .dialog-form-title {
          font-weight: 400;
    ...
    @include phone-width {
      ...
      .dialog-body {
        .dialog-form-row {
          .dialog-form-title { font-size: 0.875rem; }
          .dialog-form-input { font-size: 0.875rem; }
        }
        .select-container {
          height: 12.875rem;
          .select-row {
