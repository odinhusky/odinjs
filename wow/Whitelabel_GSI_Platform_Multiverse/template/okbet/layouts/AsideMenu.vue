<template>
  ...
  <div class="side-menu" :class="{ isClose: !modelValue }">
    ...
    <div class="button-area button-area-bottom">
      ...
      >
        ...
        <template #default>
          <ul v-if="modelValue" class="account-list">
            ...
            >
              <q-btn align="left" flat class="member-center-btn" :to="{ name: 'memberProfile' }">
                <span>{{ $t("menu.memberCenter") }}</span>
                <q-badge
                  v-if="inboxUnreadTotal > 0"
                  class="member-center-unread-badge"
                  color="negative"
                  rounded
      ...
      <!-- <q-btn
        v-if="isLogin"
        class="normal-btn btn-freespin button-title"
        :class="{ 'btn-common-active': route.name === 'FreeSpin' }"
        :to="{ name: 'FreeSpin' }"
      >
        <img :src="memberImg('free_spin_icon.png')" alt="" class="freespin-icon" />
        <span class="btn-title">{{ $t("menu.freeSpin") }}</span>
      </q-btn> -->
...
const { svgIcon, menuIcon, memberImg } = useSiteImg()
