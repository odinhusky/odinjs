    ...
    <q-card class="amount-dialog">
      ...
      <q-card-section class="amount-dialog-content">
        <div class="account">
          ...
          <span class="account-balance text-base font-normal">
            {{ $t("game.balance") }}:
            <span class="account-balance-value">{{ dialogBalance }}</span>
          </span>
        </div>
...
const dialogBalance = computed(() => {
  const balance =
    memberManagementStore.dialogIncreaseItem === 1
      ? memberManagementStore.dialogMemberRemainQuotaAmount
      : memberManagementStore.dialogMemberBalance
