<template>
  ...
  <div class="home-layout">
    ...
    <div class="home-content" id="scrollDom" @scroll="handleInnerScroll()">
      <div class="home-content-cms home-x-spacing" v-if="homeList && homeList?.length">
              ...
              <ul class="game-list long flex">
                <div class="carousel-header" v-if="shouldShowCarouselHeader(cmsItem)">
                  <div class="home-title">
                    ...
                    <div class="title-label-area">
                      <!-- Only show title label if it exists -->
                      <p class="title-label" v-if="getCmsTitle(cmsItem)">
                        {{ getCmsTitle(cmsItem) }}
                      </p>
                      <div class="title-actions" v-if="shouldShowActions(cmsItem)">
      ...
      <div class="download-area relative" :class="{ 'download-area--bf61': isBetfy66Agent }">
        <div class="download-main">
          <div class="article">
            <h1>{{ $t(homeSloganKeys.title) }}</h1>
            <h5>{{ $t(homeSloganKeys.subtitle) }}</h5>
            <p>{{ $t(homeSloganKeys.content) }}</p>
          </div>
        </div>
      </div>

      <div :class="cx('w-full', FLEX_JUSTIFY_CENTER)">
        <Leaderboard v-if="envInfo.leaderboard_enabled" class="rank-board" />
      </div>
    </div>
  </div>
...
import MarqueeList from "app/template/okbet/components/MarqueeList/Index.vue"
...
const { getBreakpoints, shouldDisplayDevice, getGridStyle, cmsHomeList: homeList } = useCms()

const { nowLang } = useLanguage()
const { handleAIHelperRouteEvent } = useAIHelperEvent()

const DEFAULT_HOME_SLOGAN_KEYS = {
  title: "slogan.title1",
  subtitle: "slogan.subtitle1",
  content: "slogan.content1",
} as const

const BF61_HOME_SLOGAN_KEYS = {
  title: "slogan.bf61.title1",
  subtitle: "slogan.bf61.subtitle1",
  content: "slogan.bf61.content1",
} as const

const isBetfy66Agent = computed(() => envInfo.agentCode?.toUpperCase() === "BF61")
const homeSloganKeys = computed(() => (isBetfy66Agent.value ? BF61_HOME_SLOGAN_KEYS : DEFAULT_HOME_SLOGAN_KEYS))
...
.home-layout
  ...
  padding-top: 0
  position: relative
  // MobileNav shows when width <= 1200px (isDown.pc), so add spacing when it is not visible (>= 1201px)
...
// Ensure RankBoard and Footer spacing in H5
+iphone-width
  .rank-board
  ...
  // Ensure Footer appears after RankBoard
  :deep(.footer-wrapper)
