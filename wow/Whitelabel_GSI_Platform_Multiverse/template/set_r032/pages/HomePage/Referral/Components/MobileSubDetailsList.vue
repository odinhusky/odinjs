    ...
    <div class="list-wrapper" v-if="referralStatementDetailData?.list?.length">
      <q-card class="mobile-page-total-card">
        ...
        <q-card-section class="page-total-content">
          ...
          <template v-for="(item, currencyCode) in pageSummary.revenue_total" :key="currencyCode">
            <div v-if="getCurrencyCode(Number(currencyCode))" class="page-total-item">
      ...
      <q-card class="mobile-page-total-card">
        ...
        <q-card-section class="page-total-content">
          ...
          <template v-for="(item, currencyCode) in referralStatementDetailTotalData?.revenue_total" :key="currencyCode">
            <div v-if="getCurrencyCode(Number(currencyCode))" class="page-total-item">
              <span class="page-total-title">{{ getCurrencyCode(Number(currencyCode)) }}</span>
              <span>{{ moneyFormat(item) }}</span>
            </div>
      ...
      <q-card class="list-item" v-for="(item, index) in referralStatementDetailData.list" :key="index">
        <q-card-section class="list-item-section">
          <q-expansion-item class="expansion-wrapper detail">
...
import Pagination from "app/template/set_r032/components/Pagination/Index.vue"
...
@import "app/template/set_r032/assets/css/referral.scss";
