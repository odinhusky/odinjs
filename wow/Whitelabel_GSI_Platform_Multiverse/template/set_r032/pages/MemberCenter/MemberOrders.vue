...
.h5-order-area {
  ...
  // 把 q-card-section 內的 title / text 改成水平排列（左右對齊）
  // 只 scope 在本頁，不影響共用 reports.scss 的其他頁面
  .data-expansion-item :deep(.q-card__section) {
    display: flex;
    align-items: center;
    justify-content: space-between;
    gap: 1rem;

    // 標題不要被 flex 壓縮，永遠保持單行（避免「編號」被擠成「編 / 號」）
    .title {
      white-space: nowrap;
      flex-shrink: 0;
    }

    // text 給 min-width: 0 才能在 flex 容器內正常 word-break / 折行
    // text-align: right 讓多行內容（例如長的 trans_code）也保持靠右對齊
    .text {
      min-width: 0;
      text-align: right;
    }
