          ...
          <li v-for="game in gameRow" :key="game.game_id" class="game-item" @click="handleGameClick(game)">
            <div class="img-container">
              ...
              <q-btn
                ...
                class="btn-favorite is-active hide-hover"
                round
                flat
...
import { SearchGameKey } from "app/template/set_r032/utils/constants/symbols"
...
const { gameTypeState, openGame, getGameImage } = useGame()
...
.game-item {
  ...
  .img-container {
    ...
    .btn-favorite {
      ...
      // 半透明圓形底色（原本是 svg 內建，換成 q-icon 後用 CSS 補回）
  ...
  .game-favorites {
    ...
    .q-icon {
      font-size: 1rem;
      color: var(--primary-11);
      // 用 drop-shadow 沿著 icon 實際輪廓畫光暈（box-shadow 會畫成方形）
      filter: drop-shadow(0 0 5px var(--primary-11));
    }
  }
