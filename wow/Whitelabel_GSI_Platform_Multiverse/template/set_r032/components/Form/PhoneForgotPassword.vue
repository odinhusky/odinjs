...
const { envData, defaultCountryCode } = useEnv()
...
function changeForgotMethod(type: FORGOT_PASS_METHOD.Enums) {
  ...
  formSms.phone = ""
  formSms.sms_otp = ""
  formSms.country_code = defaultCountryCode.value
  formAccount.account = ""
  formAccount.email = ""
}
...
watch(defaultCountryCode, (code) => {
  if (code && !formSms.country_code) {
    formSms.country_code = code
  }
})
...
// dark theme 下給 q-input/q-select 加外光暈（套在實際可見的 .q-field__inner 邊框上）
.form-input {
  @include dark {
    :deep(.q-field__inner) {
