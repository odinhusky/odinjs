<template>
  <div :class="['game-item', { h5: isLargeTablet, pc: !isLargeTablet }]">
    ...
    <div
      v-if="entrance.type === CMS_ENTRANCE_TYPE.Enums.GAME_LINK || entrance.type === CMS_ENTRANCE_TYPE.Enums.CATEGORY_LOBBY"
      class="player-number"
    >
      <q-icon name="person" size="xs" />
      <span>{{ getRandom(1, 500) }}</span>
    </div>
...
const { setDefaultGameImg, gamePlayerImg } = useCommonImg()
const { hotTagImg, newTagImg } = useSiteImg()
const { getRandom } = useCommon()
