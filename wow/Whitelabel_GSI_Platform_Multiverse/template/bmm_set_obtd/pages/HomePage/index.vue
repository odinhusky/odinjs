...
const { useBasicInfoQuery, getUserWalletList, getUserKycStatus, userInfo2 } = useUserInfo()
useBasicInfoQuery()
...
const { x, style } = useDraggable(ClaimGiftButton, {
  initialValue: {
    x: width.value > 1024 ? width.value - 150 : width.value - 70,
    y: height.value > 1024 ? height.value - 200 : height.value - 140,
  },
})
...
watch(x, (val) => {
  x.value = width.value > 1024 ? width.value - 150 : width.value - 70
  console.log(val)
})
