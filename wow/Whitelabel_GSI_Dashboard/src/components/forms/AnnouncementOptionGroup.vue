...
<script lang="ts" setup>
  ...
  import { getAnnouncementMasterList, getAnnouncementAgentList } from "@/api/announcement"
  ...
  const { t } = useI18n()
  onMounted(async () => {
    const { data: adminList } = await getAnnouncementMasterList()
    const { data: agentList } = await getAnnouncementAgentList()
