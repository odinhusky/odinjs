...
<script lang="ts" setup>
  ...
  import { getAgentProductList } from "@/api/report"
  ...
  onMounted(async () => {
    /*const { data } = await getAgentProductList()
    console.log(data)*/
    await queryStore.getProductDropdownList()
    queryStore.productDropdown.forEach((item) => {
      const { label, value } = item
      dropdownData.list.push({ label, value })
    })
    //generateDropdown(queryStore.productDropdown)
  })
</script>
