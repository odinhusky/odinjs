<template>
  ...
  <p v-if="dropdownData.list.length">
    {{ t("query_params.withdraw_status") }}
  </p>
  <querySelect
...
<script lang="ts" setup>
  ...
  import { WITHDRAW_STATUS } from "@/utils/constants"
  ...
  onMounted(() => {
    numberEnumToArray(WITHDRAW_STATUS.Enums).forEach((item) => {
      dropdownData.list.push({
        label: WITHDRAW_STATUS.I18nKeys[item as keyof typeof WITHDRAW_STATUS.I18nKeys] || "common.unknow",
