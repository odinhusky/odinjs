<template>
  ...
  <p v-if="dropdownData.list.length">
    {{ t("query_params.tier_when_depositing") }}
  </p>
  <querySelect
...
<script lang="ts" setup>
  ...
  import { TIER_WHEN_DEPOSITING_TYPE } from "@/utils/constants"
  ...
  onMounted(() => {
    numberEnumToArray(TIER_WHEN_DEPOSITING_TYPE.Enums).forEach((item) => {
      dropdownData.list.push({
        label:
          TIER_WHEN_DEPOSITING_TYPE.I18nKeys[item as keyof typeof TIER_WHEN_DEPOSITING_TYPE.I18nKeys] ||
          "common.unknow",
