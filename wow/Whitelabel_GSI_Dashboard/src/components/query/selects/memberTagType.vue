<template>
  ...
  <p v-if="dropdownData.list.length">
    {{ t("query_params.member_tag_type") }}
  </p>
  <querySelect
...
<script lang="ts" setup>
  ...
  onMounted(() => {
    numberEnumToArray(MEMBER_TAG_TYPE.Enums).forEach((item) => {
      dropdownData.list.push({
        label: MEMBER_TAG_TYPE.I18nKeys[item as keyof typeof MEMBER_TAG_TYPE.I18nKeys] || "common.unknow",
