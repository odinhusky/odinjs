<template>
  <p v-if="dropdownData.list.length">
    {{ t("query_params.monitoring_type") }}
  </p>
  <!-- 偵查類型 -->
  <querySelect
...
<script lang="ts" setup>
  ...
  onMounted(() => {
    numberEnumToArray(MONITORING_TYPE.Enums).forEach((item) => {
      dropdownData.list.push({
        label: MONITORING_TYPE.I18nKeys[item as keyof typeof MONITORING_TYPE.I18nKeys] || "common.unknow",
