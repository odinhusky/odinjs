<template>
  ...
  <p v-if="dropdownData.list.length">
    {{ t("query_params.commission_group") }}
  </p>
  <querySelect
...
<script lang="ts" setup>
  ...
  import { COMMISSION_GROUP } from "@/utils/constants"
  ...
  onMounted(() => {
    numberEnumToArray(COMMISSION_GROUP.Enums).forEach((item) => {
      dropdownData.list.push({
        label: COMMISSION_GROUP.I18nKeys[item as keyof typeof COMMISSION_GROUP.I18nKeys] || "common.unknow",
