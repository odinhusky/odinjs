<template>
  ...
  >
    <!--<template v-slot:option="scope">
      <q-item v-bind="scope.itemProps" clickable v-ripple @click="toggleSelection(scope.opt)">
        <q-checkbox
          v-model="scope.selected"
          :label="$t(scope.opt.label)"
          color="primary"
          :disable="scope.opt.value === -1"
          @click.stop="toggleSelection(scope.opt)"
        />
      </q-item>
    </template>-->
