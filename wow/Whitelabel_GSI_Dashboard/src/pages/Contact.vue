      ...
      <div class="col-lg-4 col-md-4 col-sm-12 col-xs-12">
        <q-card class="no-shadow" bordered>
            ...
            >
              <q-list class="">
                ...
                <span v-for="(contact, index) in contacts_list" :key="index" @click="selected_contact = contact">
                  <contact-item :avatar="contact.avatar" :name="contact.name" :position="contact.position" />
                </span>
              </q-list>
            </q-tab-panel>
            ...
            <q-tab-panel name="favorites" class="q-pa-none" :style="{ height: size['height'] - 80 + 'px !important' }">
              <q-list class="">
                ...
                <span v-for="(favorite, index) in favorites_list" :key="index" @click="selected_contact = favorite">
                  <contact-item :avatar="favorite.avatar" :name="favorite.name" :position="favorite.position" />
                </span>
              </q-list>
            </q-tab-panel>
          </q-tab-panels>
          ...
          <q-tabs v-model="tab" dense class="bg-grey-3" align="justify">
            <q-tab name="all" icon="person" class="text-capitalize" label="All" />
            <q-tab name="favorites" icon="star" class="text-capitalize" label="Favorites" />
          </q-tabs>
        </q-card>
      </div>
      <div class="col-lg-8 q-pl-xs col-md-8 col-sm-12 col-xs-12">
        <q-card class="no-shadow" bordered :style="{ height: size['height'] - 24 + 'px !important' }">
          ...
          <div v-for="(detail, detail_index) in detail_list" :key="detail_index">
            <contact-detail-item
              :icon="detail.icon"
              :text-color="detail.text_color"
              :value="selected_contact[detail['field']]"
              :label="detail.label"
            />

            <q-separator v-if="detail_index != detail_list.length - 1" inset="item" />
          </div>
        </q-card>
      </div>
    </div>
