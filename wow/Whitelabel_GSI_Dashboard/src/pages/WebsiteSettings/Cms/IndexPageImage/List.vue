<template>
  <div class="q-pa-md">
    <div class="row q-mb-md justify-start">
      <q-btn class="btns btn-blue" color="main-color" :to="{ name: 'CmsIndexPageImageAdd' }" v-if="permission.edit">
        ...
        <template #body="props">
          <q-tr>
            ...
            <q-td key="actions" :props="props">
              <q-btn flat fab-mini color="blue" :to="{ name: 'CmsIndexPageImageEdit', params: { id: props.row.id } }">
                {{ $t("btn.edit") }}
              </q-btn>
...
<script lang="ts" setup>
  ...
  function handleSortInput(row: Response.CmsItem) {
    const params: Request.UpdateCmsItemSort = {
      ...
      title: row.title,
      type: CMS_TYPE.Enums.INDEXPAGEIMAGE
    }
  ...
  onMounted(() => {
    handleGetCmsList(CMS_TYPE.Enums.INDEXPAGEIMAGE)
  })
</script>
