<template>
  <div class="q-pa-md">
    <div class="row q-mb-md justify-start">
      <q-btn class="btns btn-blue" color="main-color" :to="{ name: 'GcashHomeAdd' }" v-if="permission.edit">
        <q-icon class="q-mr-xs" size="xs" name="add" />
        {{ $t("btn.add") }}
      </q-btn>
      <q-toggle v-model="isShowPopUp" color="primary" :label="$t('btn.pop_up_switch')" />
    </div>
        ...
        <template #body="props">
          <q-tr>
            ...
            <q-td key="actions" :props="props">
              <q-btn flat fab-mini color="blue" :to="{ name: 'GcashHomeEdit', params: { id: props.row.id } }">
                {{ $t("btn.edit") }}
              </q-btn>
...
<script lang="ts" setup>
  ...
  function handleSortInput(row: Response.CmsItem) {
    //如果有鎖定的資料 禁止輸入0跟1
    const hasEditable = cmsList.value.some((item) => item.is_editable)
    if (hasEditable) {
      if (Number(row.sort) === 0 || Number(row.sort) === 1) {
        row.sort = 2
      }
    }
  ...
  const isShowPopUp = computed({
    get: () => cmsList?.value[0]?.is_popup || false,
    set: async (value: boolean) => {
      await handleUpdateCmsItemPopup({
        type: CMS_TYPE.Enums.GCASHHOME,
        popup: value
      })
      await handleGetCmsList(CMS_TYPE.Enums.GCASHHOME)
    }
  })

  onMounted(() => {
    handleGetCmsList(CMS_TYPE.Enums.GCASHHOME)
  })
</script>
