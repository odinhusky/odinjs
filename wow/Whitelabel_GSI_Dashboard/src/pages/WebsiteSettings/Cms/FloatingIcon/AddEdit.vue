      ...
      <div class="q-pa-xl bg-white">
        <q-form @submit="handleSubmit">
          <q-card flat>
            ...
            <q-card-section class="row items-center q-gutter-md">
              <!-- icon -->
              <div v-for="item in Object.keys(cmsForm.setting.icon_lang)" :key="item">
                <span>{{ item }}</span>
              ...
              <!-- 開啟方式 -->
              <div v-if="!isCustomLink">
                <span>{{ $t("common.redirect_method") }}</span>
                <q-select
                  v-model="cmsForm.setting.opening_method"
            ...
            <q-card-section>
              ...
              <div class="row items-center q-gutter-md">
                <div v-for="(entrance, entranceIndex) in cmsForm.entrance" :key="`entrance-${entranceIndex}`">
                  <EntranceLink v-model="entrance.type" :entrance="entrance" :type="CMS_TYPE.Enums.FLOATING_ICON" />
                </div>
              </div>
              <div class="row items-center q-gutter-md q-mt-md" v-if="isPopLink">
                <Editor :model-value="cmsForm.setting.pop_up_content" @update:model-value="handelEditorTitle" />
              </div>
            </q-card-section>
...
<script lang="ts" setup>
  ...
  import { CMS_TYPE, LANGUAGE_TYPE, CMS_ENTRANCE_TYPE, CMS_INTERNALPAGE } from "src/utils/constants"
  ...
  const Rules = useRule()
  const isCustomLink = ref(false)
  const isPopLink = ref(false)
  const {
    isLoading,
    cmsForm,
    displayLoginList,
  ...
  const backRouteName = "CmsFloatingIconList"
  ...
  onMounted(() => {
    if (isEditMode.value && cmsId.value) {
      ...
      initCmsForm(CMS_TYPE.Enums.FLOATING_ICON)
      addCmsFormEntrance()
    }
  })
  watch(
    () => cmsForm.value.entrance[0],
    (newVal) => {
      if (newVal) {
        isCustomLink.value = newVal.type === CMS_ENTRANCE_TYPE.Enums.CUSTOM_LINK
        isPopLink.value =
          newVal.type === CMS_ENTRANCE_TYPE.Enums.INTERNAL_PAGE &&
          newVal.payload?.link_id === CMS_INTERNALPAGE.Enums.INTERNAL_POP
      }
    },
    { deep: true }
  )
  const handelEditorTitle = (value: string) => {
    cmsForm.value.setting.pop_up_content = value
  }
</script>
