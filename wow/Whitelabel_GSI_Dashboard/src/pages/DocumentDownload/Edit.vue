<template>
  <div class="q-pa-md">
    <q-card class="q-mx-auto editWrapper q-px-lg q-py-md">
      <q-form>
        <q-card-section>
          <div class="text-h5 text-bold text-dark">{{ $t("common.editorial_document") }}</div>
        </q-card-section>
        <q-card-section>
          <q-separator />
        </q-card-section>
...
<script lang="ts" setup>
  ...
  import { getDocumentDownloadDetail, updateDocumentDownloadDetail } from "@/api/documentDownload"
  ...
  const { search, spinShow, isSuccess, tableData } = useSearch(getDocumentDownloadDetail)
  onMounted(() => {
    ...
    Promise.all([search({ id: parseInt(id) })])
      .then(() => {
        ...
        form.file_type = tableData.value.file_type
        form.target = tableData.value.target_front_end
        form.title = tableData.value.title
        form.desc = tableData.value.desc
        form.target_audience = tableData.value.target_agent
        dateTimeSelector.from = genTimeFormat(tableData.value.file_start_time, "yyyy-MM-dd") as any
        dateTimeSelector.to = genTimeFormat(tableData.value.file_end_time, "yyyy-MM-dd") as any
        form.enabled = tableData.value.enabled
        form.file = tableData.value.file_path
  ...
  async function onSubmit() {
    ...
    const { code, msg } = await updateDocumentDownloadDetail(form)
    if (code !== 0) {
      $q.notify({
        type: "negative",
        message: msg,
