<template>
  ...
  <div class="q-pa-md">
    <q-card class="q-mx-auto editWrapper q-px-lg q-py-md">
      <q-form>
        <q-card-section>
          <div class="text-h5 text-bold text-dark">{{ $t("common.add_notice") }}</div>
        </q-card-section>
        <q-card-section>
          <q-separator />
        </q-card-section>
...
<script lang="ts" setup>
  ...
  import { addAnnouncementDetail } from "@/api/announcement"
  ...
  const typeDropdownList = genEnumToDropdown(DISPLAY_OBJECT_TYPE.Enums, DISPLAY_OBJECT_TYPE.I18nKeys).map((e) => {
    e.label = t(e.label)
    return e
  })

  const objectDropdownList = genEnumToDropdown(ANNOUNCEMENT_TYPE.Enums, ANNOUNCEMENT_TYPE.I18nKeys).map((e) => {
    e.label = t(e.label)
    return e
  })
  ...
  async function onSubmit() {
    ...
    const { code, msg } = await addAnnouncementDetail(form)
    if (code === 0) {
      $q.notify({
        type: "positive",
