          ...
          >
            <template #body="props">
              <q-tr>
                ...
                <q-td key="dispatched_count" :props="props">
                  {{ props.row.dispatched_count || 0 }}
                </q-td>
...
<script lang="ts" setup>
  ...
  import { getInterestApplicationAuditing } from "@/api/interest"
  ...
  let { search, tableData, totalSize } = useSearch(getInterestApplicationAuditing)
  const { genTimeFormat } = useCommon()
  let catchQueryForm: Request.GetInterestApplicationAuditing
  ...
  const mockSearch = async (queryForm: any) => {
    ...
    const mockResponse = mockData.getInterestApplicationAuditing
