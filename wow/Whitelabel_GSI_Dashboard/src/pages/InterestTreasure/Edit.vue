<template>
  <div class="q-pa-md">
    <SubPage :action-label-i18n-key="'btn.edit'" :custom-back-func="onBackTo" :custom-title="form.name">
    ...
    <q-card class="bg-white py-6">
      ...
      <q-form v-else @submit="handleSubmit">
        <!-- 基本信息 -->
        <q-card-section>
        ...
        <q-card-section>
          <div class="row q-col-gutter-md">
            <div class="col-6">
              ...
              <DateTimePicker
                class="edit-input disabled-field"
                label=""
            ...
            <div v-for="(plan, index) in form.plans" :key="index" class="">
              <div class="col col-3 q-pa-md" style="background: #fcf8ff; border-radius: 10px">
                <div class="q-mb-sm">
                  ...
                  <q-input v-model="plan.principal" type="number" outlined dense readonly bg-color="grey-2" />
                </div>
                <div class="q-mb-sm">
                  <span
                  ...
                  >
                  <q-input v-model.number="plan.days" type="number" outlined dense readonly bg-color="grey-2" />
                </div>
                <div class="q-mb-sm">
                  <span
...
<script setup lang="ts">
  ...
  import { getInterestActivityDetail, updateInterestActivity } from "@/api/interest"
  ...
  const form = reactive<{
    ...
    plans: Array<Request.AddInterestActivityPlan & { id?: number }>
  }>({
    name: "",
    info: [],
    currency_id: 0,
  ...
  const increaseAuditRate = () => changeAuditRate(1)
  const decreaseAuditRate = () => changeAuditRate(-1)
  ...
  const updateInterestTreasureBanner = async (value: string, item: Request.AddInterestActivityInfo) => {
    ...
    try {
      ...
      if (value.startsWith("data:image")) {
        ...
        item.image = value
        // 如果是 objectKey，转换为完整 URL 用于预览
        previewImages.value[item.lang] = getImageFullPath(value)
      }
    } catch (error) {
      $q.loading.hide()
      if (item.lang) {
  ...
  // 将 objectKey 转换为完整的图片 URL
  const getImageFullPath = (img?: string): string => {
    if (!img) return ""
  ...
  const loadActivityDetail = async () => {
    ...
    try {
      ...
      const response = await getInterestActivityDetail(id)

      if (response.code === 0 && response.data) {
        const data = response.data as unknown as Response.GetInterestActivityDetailData
        ...
        sourceStartTime.value = data.start_time
        sourceEndTime.value = data.end_time
        form.is_auto_dispatch = data.is_auto_dispatch
        form.audit_rate = data.audit_rate
        form.maximum_interest_limit = data.maximum_interest_limit || 0

        // 填充多语言信息
        if (data.info && data.info.length > 0) {
          form.info = data.info.map((item) => ({
            lang: item.lang || "",
            title: item.title || "",
            image: item.image || ""
          }))
          // 初始化预览图片 - 将 objectKey 转换为完整 URL
          data.info.forEach((item) => {
            if (item.lang && item.image) {
              previewImages.value[item.lang] = getImageFullPath(item.image)
            }
          })
        } else {
          // 如果没有 info 数据，初始化多语言信息
        ...
        if (data.plans && data.plans.length > 0) {
          form.plans = data.plans.map((plan) => ({
            id: plan.id,
            days: plan.days,
        ...
        // RFC3339 初始值與時區切換都統一從原始 API 值重算 DatePicker 顯示。
        syncDateTimeSelector()
      } else {
        $q.notify({
          type: "negative",
          message: response.msg || t("message.load_failed"),
          position: "top"
        })
        router.push({ name: "InterestTreasureList" })
      }
  ...
  const validatePlans = (): boolean => {
    const validPlans = form.plans.filter((plan) => plan.id !== undefined)
    if (validPlans.length === 0) {
      $q.notify({
        type: "negative",
        message: `${t("query_params.event_plan")} - ${t("error_msg.amount_cannot_be_0")}`,
        position: "top",
        timeout: 1000
      })
      return false
  ...
  const handleSubmit = async () => {
    ...
    try {
      // 构建更新 payload - 只包含 API 需要的字段
      const payload: Request.UpdateInterestActivity = {
        name: form.name,
        // 转换 info 为符合 UpdateInterestActivityInfo 的格式
        info: form.info.map((item) => ({
          image: item.image,
          lang: item.lang,
          title: item.title
        })),
        // is_auto_dispatch 值保持不变（1=手動, 2=自動）
        is_auto_dispatch: form.is_auto_dispatch,
        audit_rate: form.audit_rate,
        maximum_interest_limit: form.maximum_interest_limit,
        // plans 只发送已存在的（有 id 的），且只包含 id 和 interest_rate
        plans: form.plans
      ...
      const { code, msg } = await updateInterestActivity(activityId.value, payload)
      ...
      if (code === 0) {
        ...
        $q.notify({
          type: "negative",
          message: msg || t("message.edit_failed"),
          position: "top"
        })
      }
    } catch (error) {
