...
<script lang="ts" setup>
  ...
  let { search, tableData, isSuccess } = useSearch(getAgentMemberCommissionSettingListSubordinateDetail)
  onMounted(async () => {
  ...
  function submit() {
    const { commission_id, account } = route.params
    let payload = {
  ...
  watch(
    ...
    (newAccount, oldAccount) => {
      if (newAccount !== oldAccount && route.name === "AgentMemberCommissionSettingNestedDetail") {
        submit()
      }
    }
  )
