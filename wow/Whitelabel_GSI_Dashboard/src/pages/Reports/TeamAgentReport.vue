            ...
            <template #body="props">
              ...
              <q-tr>
                <q-td key="member_account" :props="props" class="expand-cell">
                  ...
                  />
                  <span
                    :class="{ 'text-blue cursor-pointer': props.row.member_count > 1 }"
...
<script lang="ts" setup>
  ...
  import type { GetMemberOverviewPersonalItem, MemberOverviewItemDetail } from "@/api/response.type"
  ...
  const onSubmit = async (queryForm: GetMemberOverviewList) => {
    ...
    // 日期區間不可大於31天
    if (Number(queryForm.end) - Number(queryForm.start) > 31 * 24 * 60 * 60 * 1000) {
      return $q.notify({
        type: "negative",
        message: t("common.date_range_cannot_be_greater_than_thirty_days"),
        position: "top",
        timeout: 2000
      })
    }
    ...
    teamSummaryExpanded.value = false
  ...
  const goDetail = async (item: MemberOverviewItemDetail) => {
    if (item.member_count <= 1) return

    store.getCurrencyList()

    // 保存完整的 route.query，只更新需要的參數
    router.push({
      name: "TeamAgentReportDetail",
      params: {
        memberAccount: String(route.query.agentAccount)
      },
      query: {
        ...route.query, // 保留所有原始 query 參數
        currency: item.currency_id,
        agentAccount: item.member_account
      }
    })
  }
  ...
  const teamSummaryExpanded = ref(false)
  ...
  async function toggleSelfDataExpand() {
    ...
    teamSummaryExpanded.value = newState
    if (!newState) return
