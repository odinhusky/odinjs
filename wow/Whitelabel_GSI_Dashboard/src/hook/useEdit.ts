...
export function useEdit(dialogFormInitData: any) {
  ...
  const cloneDialogForm = (formData) => (dialogForm.value = JSON.parse(JSON.stringify(formData)))

  const insertDialogFormItem = (attr, formData) => (dialogForm.value[attr] = formData)

  const getInfo = async (row: any, infoFunction: Function | null, fixAttr: Array<string> = []) => {
    ...
    await (infoFunction as Function)({ id: row.id }).then((res: any) => {
      dialogFormLoading.value = false
      if (res.data instanceof Array) {
        dialogForm.value.sequenceList = res.data
      } else {
        dialogForm.value = { ...res.data, id: row.id }
      }
  ...
  const updateInfo = async (title: string, updateFunction: Function) => {
    return await updateFunction(dialogForm.value.id, dialogForm.value)
  }

  const deleteInfo = async (deleteFunction: Function) => {
