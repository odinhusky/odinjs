...
import { tabLabelFormatter } from "@/utils/site"
...
export function useGoRouter() {
  ...
  const addTab = (event: Event, item: any) => {
    ...
    if (!tabInfo.isMenu) {
      store.commit("SET_ACTIVE_TABINFO", item.params ? item.params : {})
    }
    if (tabData.find((tab: any) => tab.name === tabInfo.name)) {
      return
    }
  ...
  const goLogin = async () => {
    await logout().then((res) => {
      ...
      localStorage.removeItem("userInfoStroe")
