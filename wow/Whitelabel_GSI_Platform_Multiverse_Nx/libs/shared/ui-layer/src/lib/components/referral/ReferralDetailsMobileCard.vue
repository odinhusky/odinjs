...
const formatRange = computed(() => {
  const start = props.row.billing_start?.slice(0, 16).replace("T", " ") || "-"
  const end = props.row.billing_end?.slice(0, 16).replace("T", " ") || "-"
  return `${start} - ${end}`
})
...
const handleView = () => {
  emit("view", {
    statementId: props.row.statement_id,
    range: { start: props.row.billing_start, end: props.row.billing_end }
  })
}
...
<template>
  ...
  >
    <BasePlainBtn :class-obj="{ button: headerButtonClass }" @click="toggleExpanded">
      <div class="min-w-0">
        <div class="truncate text-base leading-6 font-bold">{{ formatDate(props.row.billing_date) }}</div>
    ...
    <div v-if="expanded" class="border-t border-[var(--border-border-primary)] px-4 pb-4">
      <div class="flex flex-col py-2">
        <div
          class="flex items-center justify-between gap-3 border-b border-[var(--border-border-primary)] py-2"
        >
          <span class="text-sm leading-5 text-[var(--card-card-subtitle-primary-enabled)]">
            {{ $t("menu.settlementRange") }}
          </span>
          <span class="text-right text-xs leading-5 font-bold text-[var(--card-card-title-primary-enabled)]">
            {{ formatRange }}
          </span>
        </div>
        <div
      ...
      <BaseBtn theme="primary" category="outline" size="md" class="h-10 w-full !rounded-lg" @click="handleView">
        {{ $t("menu.details") }}
      </BaseBtn>
    </div>
  </article>
