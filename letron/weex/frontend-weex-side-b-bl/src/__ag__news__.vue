<template>
    <div class="__ag__news__" :class="[ipx?'ipx':'']" @swipe="handleSwipe">
		...
		<div class="__ag__news-tabs">
			<div class="__ag__news-tabs-item__" v-for="item in tabList" :key="item.id" @click="change(item)">
                <text class="__ag__tabs-text" :class="[tab == item.id?'tab-select-text':'']">{{item.name}}</text>
            </div>
		</div>
		<slider class="__ag__slider__" interval="10" offset-x-accuracy="200" auto-play="false" infinite="false" @change="changeHandler" :index="tab">
			<div class="__ag__news-content__" v-for="item in tabList" :key="item.id">
				<scroller class="__ag__uplivecontent__" :show-scrollbar="false">
					...
					<div class="__ag__newslist__" v-if="list.length>0">
						<ag-news-item v-for="(item,index) in list" :item="item" :index="index" :length="list.length" :isindex="false" :key="item.id"></ag-news-item>
					</div>
					<div class="__ag__newsnotlist__" v-if="list.length<=0" @click="change({id:0})">
	...
	export default {
		...
		data() {
            return {
				...
				tabList: [{id:0,name:'全部'},{id:1,name:'篮球'},{id:2,name:'足球'},{id:3,name:'其他'}],
				tab: 0,
				sliderIndex: 0,
