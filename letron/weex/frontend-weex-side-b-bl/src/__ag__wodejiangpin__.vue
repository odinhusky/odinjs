<template>
  <div class="__ag__jiangpin-container__">
    ...
    <scroller class="__ag__prize-record__" show-scrollbar="false" :style="{height:`${realHeight}px`}" v-if="prizeList.length>0" >
    ...
    <div class="__ag__show-nopic__" v-if="showPic">
      <div class="norecord-box">
        <image :src="__ag__url__('static/shijiebei/norecord3.png')" class="no-record-img" />
    ...
    <!-- 转接小助手弹窗 -->
    <ag-popup class="__ag__lives_popup__" v-if="assistantModal" :isAnimate="assistantModal">
      <div class="luck-modal">
        ...
        <div class="assistant-tips">
          <text class="agiconfont assistant-icon" ref="loading" v-if="env.code == 'xg'">&#xe631;</text>
				  <text class="agiconfont assistant-icon" ref="loading" v-else>&#xe7f1;</text>
...
export default {
  ...
  data(){
    return{
      ...
      loading: false,
      showPic:false,
      realHeight:(weex.config.env.deviceHeight / weex.config.env.deviceWidth) * 750 - 198,
  ...
  methods:{
    ...
    toGet(){
     ...
     listMyPrizes(){
					...
					__ag__sportApi__.pageAwardResult().then(res=>{
