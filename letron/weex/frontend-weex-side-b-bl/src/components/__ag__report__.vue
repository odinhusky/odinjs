    ...
    <div class="__ag__report__" v-if="isreport1">
        <div class="__ag__reporttab__">
            <div class="__ag__reporttitle__">
                ...
                <text class="__ag__reporttitletext__ agiconfont report-icon" @click="onclose">&#xe64b;</text>
            </div>
            <div class="__ag__reportbody__">
                <div class="__ag__reportcontent__" v-for="(item,index) in reportList" @click="__ag__ontabreport__(index+1)" :key="'rt'+index">
                    <div class="__ag__resportcellon__" v-if="tab!=index+1"></div>
                    <text class="agiconfont  __ag__resportcellonclass__ " v-if="tab ==index+1">&#xe682;</text>
    ...
    export default {
        ...
        methods:{
            ...
            __ag__onReport__(){
                ...
                __ag__sportApi__.editRoomReport({roomId:rid,type:this.tab}).then(resp=>{
                    // this.$emit('onreport')
                    this.isreport1 = false
                    this.tab = 0
                     this.$emit('onclose')
					util.message("我们会在24小时内处理，超管确认违规后对内容进行相应处理！")
				}).catch(res=>{
                    util.message(util.toString(res))
                })
			},
        }
    }
