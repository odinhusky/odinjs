	...
	<div class="__ag__video-control__">
		...
		<div class="__ag__video-content__" :style="{display: isDisplay}">
			<div class="__ag__video-start__" :class="{'__ag__video-start-mantle__': isDisplay}">
				<span v-if="isPlays" @click="clickPause" class="agiconfont icon-pause __ag__play-icon__"></span>
				<span v-else @click="clickStart" class="agiconfont icon-play __ag__pause-icon__"></span>
			</div>
			<div class="__ag__video-bar__">
				<div class="__ag__bar-lf__">
					...
					<div class="__ag__danmu-item__" v-if="openDanmu">
						<span v-if="isDanmu" @click.stop="clickDanmu" class="agiconfont icon-chat-open __ag__danmu-icon__"></span>
						<span v-else @click.stop="clickDanmu" class="agiconfont icon-chat-close __ag__danmu-icon__"></span>
						<span style="font-size: 13px;">弹幕</span>
					</div>
				</div>
				<div class="__ag__bar-rg__">
					<span v-if="!isMute" @click.stop="clickMute" class="agiconfont icon-mute1 __ag__video-mutes__"></span>
					<span v-else @click.stop="clickMute" class="agiconfont icon-mute2 __ag__video-mutes__"></span>
					<div class="__ag__video-hd__" @click.stop="clickQuality"><span>{{qualityText}}</span></div>
					<span v-if="!isFullscreens" @click.stop="clickFullscreen" class="agiconfont icon-fullscreen1 __ag__video-fullscreen__"></span>
					<span v-else @click.stop="exitFullscreen" class="agiconfont icon-fullscreen-exit __ag__video-fullscreen__"></span>
				</div>
			</div>
		</div>
	</div>
