...
const thisFileName = "__ag__chatMsgNew__"
  ...
  methods: {
    ...
    loadimg(e, n) {
      if (e.success) {
        ...
        const firstId = len > 0 ? this.a__ag__messageList__[0].id : ""
        const scrollToId = Number(firstId) > Number(lastId) ? firstId : lastId

        // # 由於整個陣列有被 reverse 過，所以最後一筆不一定是顯示在畫面上的第一筆，透過 Id 的大小來判斷是不是真的是最新的一筆留言
        // - 文字的部分則本來就在反轉的陣列中，所以不用調整

        if (!this.a__ag__showscroll__) {
          this.__ag__goscroll__(scrollToId)
        }
      }
    },
    closePopup() {
