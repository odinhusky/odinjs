<template>
 <!-- v-if="!(type==6 &&( status==0 || status==2))" -->
	<div class="__ag__task-item-container__">
		...
		<div class="__ag__task-content-box__" :class="[border==1?'no-border':'']">
			<div class="__ag__task-text-wc__">
				<div class="__ag__task-amount-text__">
					<text class="__ag__task-title-wc__">{{title}} (</text>
					<text class="__ag__task-amount__" v-if="activityType!=1">{{amount>everyAmount?everyAmount:amount}}</text>
					<text class="__ag__task-amount__" v-else>{{done!=0 ?1:0}}</text>
					<text class="__ag__task-everyAmount__">/{{everyAmount}})</text>
				</div>
				<text class="__ag__task-description-wc__">{{des}}</text>
			</div>

			<div class="__ag__task-button__" :class="[done==0?'':done==1?'toget':'done']" @click="toClick">
				<text v-if="done===0"  class="__ag__task-button-text__" >去完成</text>
				<text v-if="done===1"  class="__ag__task-button-text__" :class="[done==1?'__ag__toget__':'']" >领取</text>
				<text v-if="done===2" class="__ag__task-button-text__" :class="[done==2?'__ag__toget__':'']">已领取</text>
			</div>
		</div>
...
<script>
	...
	export default {
		...
		methods: {
			toClick(){
			 let	obj={id:this.postId,daily:this.daily,status:this.done,type:this.activityType}
				this.$emit('click',obj)
			},
		}
	}
...
.__ag__icon-content__{
	...
	width:94px;
	height: 94px;
	background-color: rgba(236,27,80,0.2);
