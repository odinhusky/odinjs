<template>
  ...
  >
    ...
    <scroller
      ...
      :class="[
        keyboardSize == '0px' && ipx ? 'ipx' : '',
        listFaq && listFaq.length ? '__ag__msgchatscroller_faq__' : '',
        listFaq && listFaq.length && ipx
          ? '__ag__msgchatscroller_faq_ipx__'
          : '',
      ]"
    ...
    >
      ...
      >
        ...
        >
          ...
          >
            ...
            <!-- 加密圖片类型 -->
            <async-image
              v-if="n.contentType == 6"
              @clickImg="clickImg"
              :imgStyle="imageStyle(n)"
              :src="__ag__url__(n.content)"
              @load="loadimg($event, n)"
            ></async-image>
      ...
      <div
        ...
        v-if="a__ag__showscroll__ && !a__isScroll__ && tab == 1"
        @click="
          a__ag__showscroll__ = false
          __ag__goscrollBottom__(messageList.length)
        "
      >
    ...
    <div
      class="__ag__msgsend__"
      :class="[ipx ? '__ag__msgsend_ipx__' : '']"
      v-if="hasFriend"
    >
      <div class="__ag__msgsend_faq__" v-if="listFaq && listFaq.length">
        <scroller
          scroll-direction="horizontal"
          :show-scrollbar="false"
          class="__ag__faq_scroll__"
        >
          <div
            class="__ag__faq_item__"
            v-for="item in listFaq"
            :key="item.id"
            @click="__ag__faq__(item)"
          >
            <text class="__ag__faq_text__">{{ item.question }}</text>
          </div>
        </scroller>
      </div>
      <div class="__ag__msgsend_content__">
        <div
        ...
        <div
          class="__ag__livefs__"
          v-if="a__ag__content__"
          @click="__ag__send__"
        >
...
const thisFileName = "__ag__chatList__"
export default {
  ...
  data() {
    return {
      ...
      a__ag__content__: "",
      a__ag__pageMsgForm__: { size: 50, page: 1, friendId: 0, toId: null },
  ...
  created() {
    ...
    imgsuccess.onmessage = function (event) {
      ...
      const param = get(event, "data.param", null)

      util.message("上传成功")
      that.__ag__imgSend__(data, param)
    }
  ...
  methods: {
    ...
    keyboard(event) {
      ...
      if (
        ...
        if (!this.isHeigBoard) {
          let r =
            this.a__ag__messageList__[this.a__ag__messageList__.length - 1]
          this.__ag__goscroll__(r.id)
        }
    ...
    clickImg(img, contentType) {
      ...
      this.imgPopupUrl = this.__ag__url__(img)
    },
    ...
    async __ag__onmsg__(msg) {
      ...
      for (let i in msg) {
        ...
        if (r.messageType == 3 && this.type == 3) {
          if (r.senderId == this.uid && this.tab == 1) {
            let c = r
            c.avatar = c.senderAvator
            await __ag__sport_api__
    ...
    // - 上傳完 uploadBase64 的圖片之後會回傳該路徑，打editMag API 的用意是為了將 儲存在後端的資料跟回傳回來的路徑做連結，並且告知 contentType 是什麼
    // ^ 3: 一般圖片，非加密處理過後
    // ^ 6: 加密圖片，個人聊天室以及直撥聊天室的圖片上傳都屬於此類
    async __ag__imgSend__(data, param) {
      if (!data) return

      // 檢查送過來的結果是不是本頁發送出去的圖片上傳請求，如果不是則不送出
      // 如果是本頁送出的請求，則繼續發送 editMsg 或是 editGroupMsg
      const target = get(param, "target", "")
      if (!target || target !== thisFileName) return
    ...
    async prevMsg() {
      if (this.type == 3) {
        let f = { friendId: this.uid, size: 50 }
    ...
    __ag__faq__(item) {
      let sendForm = {}
      if (item.autoType) {
        sendForm.tid = this.uid
        sendForm.content = item.question
        sendForm.faqId = item.id
        this.a__ag__content__ = item.question
        this.__ag__faqSend__(sendForm)
        return
      }
      ...
      sendForm.refContent = ""
      sendForm.content = item.question
      sendForm.contentType = 1
      sendForm.tid = this.uid
      this.__ag__faqSend__(sendForm)
    },
    __ag__faqSend__(sendForm) {
      __ag__sport_api__
        .editMsg(sendForm)
        .then((resp) => {
          let r = resp.data
