<template>
	<div class="__ag__matchlist__" v-if="listSchedule3 && listSchedule3.length>0">
		<scroller  scroll-direction="horizontal" :show-scrollbar="false" class="__ag__matchscroll__" >
			<ag-matchitem v-for="(item) in listSchedule3" :key="item.id" :teamitem="item" :userMatchMap="userMatchMap" :user="user" :switchWorldcup="switchWorldcup"></ag-matchitem>
	...
	export default {
		...
		computed:{
			...
			listSchedule3() {
				...
				// 正在直播和申请未上播的比赛
				let allLiveAndScheduledMatches = this.__ag__formattedMatchList__(allLiveMatches,allRequestMatches);
				//从已预约的比赛 帅选直播中的比赛
				let userSubscribedLiveMatch = userMatchList.filter((match) => match.joinMap && match.joinMap.sche && match.joinMap.sche.islive === 1);
				// 从正在直播和申请未上播的比赛 帅选直播中的比赛
				const liveFormattedMatchesList = allLiveAndScheduledMatches.filter((match) => match.islive === 1);
				// 从正在直播和申请未上播的比赛 帅选未直播的比赛
				const notLiveScheduledMatchList = allLiveAndScheduledMatches.filter((match) => match.islive === 0);
				// 从所有直播中的比赛和已预约正在直播的比赛筛选出同一场的比赛
				const liveMatchUserSubscribed = liveFormattedMatchesList.filter((m1) =>userSubscribedLiveMatch.some((m2) => m1.uid.toString() === m2.joinMap.sche.uid));
				// 筛选已推荐的比赛
				const backendRecommendedMatches = allHotMatchs.filter((m) => m.joinMap.hot_match.status == 1);
				// 从已推荐的比赛筛选正在直播的比赛
				const backendRecommendedMatches_live = backendRecommendedMatches.filter((m) => m.islive === 1);
				// 筛选出同一个主播直播中的比赛
				const formattedBackendRecommendedMatches_live = liveFormattedMatchesList.filter((m1) =>backendRecommendedMatches_live.some((m2) => m2.uid.split(",").indexOf(m1.uid.toString()) !== -1)).filter((m3) => !liveMatchUserSubscribed.some((m4) => m3.uid === m4.uid))
				// 从已推荐的比赛筛选未直播的比赛
				const backendRecommendedMatches_not_live = backendRecommendedMatches.filter((m) => m.islive !== 1);
				// 从预约未上播和已推荐未上播的比赛筛选同一场比赛
				const formattedBackendRecommendedMatches_not_live_with_streamer = notLiveScheduledMatchList.filter((m1) => backendRecommendedMatches_not_live.some((m2) => m1.scheduleId.toString() === m2.id.toString()));
				// 从已推荐的比赛筛选未直播的比赛筛选同一场比赛
				const formattedBackendRecommendedMatches_not_live_without_streamer = backendRecommendedMatches_not_live.filter((m1) =>!formattedBackendRecommendedMatches_not_live_with_streamer.some((m2) => m1.id === m2.id));
				// 筛选一场比赛多个主播的比赛
				const allOtherLiveMatches = liveFormattedMatchesList.filter((m1) => !liveMatchUserSubscribed.some((m2) => m1.uid === m2.uid)).filter((m1) =>!formattedBackendRecommendedMatches_live.some((m2) => m1.uid === m2.uid));
				// 筛选预约未直播的比赛
				const userSubscribedNotLiveMatch = userMatchList.filter((match) => match.joinMap && match.joinMap.sche && match.joinMap.sche.islive === 0);
				// 从所有直播中的比赛和已预约正在直播的比赛筛选出同一场的比赛排序
				const sortedUserSubscribedLiveMatches = this.getSortedMatchList(liveMatchUserSubscribed);
				// 筛选出同一个主播直播中的比赛排序
				const sortedBackendRecommendLiveMatches = this.getSortedMatchList(formattedBackendRecommendedMatches_live);
				// 筛选一场比赛多个主播的比赛排序
				const sortedOtherLiveMatches =this.getSortedMatchList(allOtherLiveMatches);
				// 筛选预约未直播的比赛排序
				const sortedNotLiveMatchUserSubscribed = this.getSortedMatchList(this.xxformattedMatchList(userSubscribedNotLiveMatch));
				// 从已推荐的比赛筛选未直播的比赛排序
				const sortedNotLiveBackendRecommendedMatches = this.getSortedMatchList(this.getSortedMatchList(this.__ag__formattedMatchList__([], backendRecommendedMatches_not_live)));
				// 从预约未上播和已推荐未上播的比赛筛选同一场比赛排序
				const sortedNotLiveBackendRecommendedMatchesWithStreamer = this.getSortedMatchList(formattedBackendRecommendedMatches_not_live_with_streamer);
				// 从已推荐的比赛筛选未直播的比赛筛选同一场比赛排序
				const sortedNotLiveBackendRecommendedMatchesWithoutStreamer = this.getSortedMatchList(this.__ag__formattedMatchList__([],formattedBackendRecommendedMatches_not_live_without_streamer));
				// 筛选出所有未开始的比赛
				const allOtherNotLiveMatchList = notLiveScheduledMatchList.filter((m1) =>!sortedNotLiveBackendRecommendedMatches.some((m2) => m1.id === m2.id)).filter((m3) =>!sortedNotLiveMatchUserSubscribed.some((m4) => m3.id === m4.id));
				// 未开始的比赛排序
				const sortedAllOtherNotLiveMatches = this.getSortedMatchList(allOtherNotLiveMatchList);
				...
				return finalShowMatchList.length > 12 ? finalShowMatchList.slice(0, 12) : finalShowMatchList;
			},
			listSchedule1() {
			...
			listSchedule2() {
				...
				let list = this.listSchedule.sort((a, b)=>{
					if(!!a.uid == !!b.uid){
		...
		methods:{
			getSortedMatchList(matchList) {
				...
				if (matchList && matchList.length > 0) {
					...
					sortedList = matchList.sort((m1, m2) => m2.lives.length - m1.lives.length);

					// 2. if number of streamer same, then sort by match begin time
					sortedList.sort((m1, m2) => {
						if (m1.lives.length === m2.lives.length) {
							return (util.parseDate(m1.beginTime).getTime() - util.parseDate(m2.beginTime).getTime());
						}
					});
			...
			__ag__formattedMatchList__(liveMatchesList, requesetMatchesList) {
				...
				if (liveMatchesList && liveMatchesList.length) {
					for (let i in liveMatchesList) {
						...
						const obj = {
							title: liveObj.title,
							isRecommended: liveObj && liveObj.joinMap && liveObj.joinMap.hot_match && liveObj.joinMap.hot_match.status,
				...
				if (requesetMatchesList && requesetMatchesList.length) {
					for (let i = 0; i < [...requesetMatchesList, ...[]].length; i++) {
						...
						const object = {
							title: item.title,
							isRecommended: item && item.joinMap && item.joinMap.hot_match && item.joinMap.hot_match.status,
							beginTime: item.beginTime,
							fid: item.fid,
