	...
	<div class="__ag__datalive__">
		...
		<div class="__ag__livechathead__"  @swipe="handleSwipe">
			<div class="__ag__livechattab__" @click="changeHandler({index:0})">
				<text class="__ag__livechattabtext__" :class="[tab==0?'selected':'']">战绩</text>
			</div>
			<div class="__ag__livechattab__" @click="changeHandler({index:1})">
				<text :class="[tab==1?'selected':'']" class="__ag__livechattabtext__">赛况</text>
			</div>
			<div class="__ag__livechattab__" @click="changeHandler({index:2})">
				<text :class="[tab==2?'selected':'']" class="__ag__livechattabtext__">阵容</text>
			</div>
			<div class="__ag__livechattab__" @click="changeHandler({index:3})">
				<text :class="[tab==3?'selected':'']" class="__ag__livechattabtext__">推荐</text>
			</div>
		</div>
		<slider class="__ag__datalivecontent__" interval="10" offset-x-accuracy="200" auto-play="false" infinite="false" @change="changeHandler" :index="tab"  @swipe="handleSwipe">
			<scroller class="__ag__datalivechatcontent__" :class="[ipx?'__ag__datalivecontentipx__':'']" show-scrollbar="false" v-for="item in tabList" :key="item.id">
				<!-- 战绩 -->
				<ag-record v-if="item.id==0" :info="info" :tab="tab" :incomeLive="dataincomeLive" 
				:scheduleClass="info.scheduleClass" :status="info.status" :matchId='sid'>
				</ag-record>

				<!-- 赛况 -->
				<ag-race v-if="item.id==1" :tab="tab" :incomeLive="dataincomeLive" 
				:scheduleClass="info.scheduleClass" :matchId='sid'></ag-race>

				<!-- 阵容 -->
				<ag-lineup v-if="item.id==2" :matchId='sid' :tab="tab" :scheduleClass="info.scheduleClass" :status="info.status" 
				:incomeLive="dataincomeLive"></ag-lineup>

				<!-- 推荐 -->
				<div v-if="item.id==3" class="__ag__dataliverecd__">
					<div class="__ag__dataliverecddiv__" v-if="recommedNewList && recommedNewList.length>0">
						<ag-recommed-new-item class="__ag__dataliverecditem__" v-for="item in recommedNewList" 
						:item="item" :key="item.id" @jumpRecommed="jumpRecommed" :user="a__ag__user__"></ag-recommed-new-item>
					</div>
	...
	export default {
		...
		data() {
			return {
				info:{},
				tab:0,
				tabList: [{id:0,name:'战绩'},{id:1,name:'赛况'},{id:2,name:'阵容'},{id:3,name:'推荐'}],
				sid:'',
				dataincomeLive:{},
				a__ag__user__:{},
			}
		...
		methods:{
			...
			async __ag__loadData__(){
				...
				try {
					...
					if(this.a__ag__listMatchScheduleById__.data && this.a__ag__listMatchScheduleById__.data.length>0){
						this.info = this.a__ag__listMatchScheduleById__.data[0]
						this.dataincomeLive = sport.hasIncomeLive(this.a__ag__listMatchScheduleById__.data,true)
					}
				} catch (error) {
