<template>
    <div class="__ag__denmu_main__">
        <div class="__ag__denmu_list__" :class="['__ag__denmu-'+index+'__']" v-for="(item,index) in danmuList">
            <div class="__ag__denmu_item__" :class="[li.stopDan?'move-danmu': 'move-danmu-no']" v-for="li in item.list" :style="{left: li.left}">
                <div class="__ag__avator_item__" v-if="li.senderType == 1">
                    <image class="__ag__avator_img__" mode="widthFix" :src="__ag__url__(li.senderAvator)"></image>
                    <text class="__ag__avator_text__">主播</text>
                </div>
                <text v-if="li.senderType == 2" class="__ag__assistant_item__">助理</text>
                <text  class="__ag__denmu_text__" :class="[li.senderType == 1 ? '__ag__anthor_text__':'',li.senderType == 2 ? '__ag__assistant_text__':'']" @click.stop="__ag__click__(li.link)">{{li.content}}</text>
            </div>
        </div>
    </div>
</template>
