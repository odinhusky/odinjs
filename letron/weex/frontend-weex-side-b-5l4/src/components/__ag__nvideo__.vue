<template>
  <div class="__ag__nvideo-main__" @click="__ag__click__" v-if="src">
    ...
    <div class="__ag__video_error__" v-if="isVideoError" :style="errortyle">
      <text class="agiconfont __video_error__" @click="onFefresh"
        >&#xe66c;</text
      >
    </div>
    ...
    >
      ...
      >
        ...
        <div class="__ag__control_rg_image__" @click.stop="__ag__onShare__">
          <image
            class="__ag__rg_image__"
            :src="__ag__url__('static/share.png')"
            mode="aspectFit"
          ></image>
        </div>
      </div>
    ...
    >
      ...
      <image class="__video_loading_image__" :src="__ag__url__(line)"></image>
    </div>
...
export default {
  ...
  methods: {
    ...
    __ag__getShareUrl__() {
      api
        .listMetaData({ label: "shareiOSUrl" })
        .then((resp) => {
    ...
    changeHideBar() {
      ...
      let cy = this.isHideBar ? -160 : 0
      landscape = util.isLandscape()
      if (landscape) {
        cy = this.isHideBar ? -160 : 0
      }
