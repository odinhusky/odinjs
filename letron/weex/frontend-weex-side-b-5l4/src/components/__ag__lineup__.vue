<template>
    <div class="__ag__lineup__">
		<ag-recordtitle v-if="status == 2 && scheduleClass == 4" name="本场最佳"></ag-recordtitle>
		<div class="__ag__bczj__" v-if="status == 2 && scheduleClass == 4 && bestList && bestList.length">
			<ag-lineupbestgame v-for="(item,index) in bestList" :item="item" :key="'b'+index"></ag-lineupbestgame>
		</div>
		<div v-if="status == 2 && scheduleClass == 4 && (!bestList || !bestList.length)" class="__ag__not__">
			<text class="__ag__nottext__">暂无信息</text>
		</div>
        <ag-teamlineupList :scheduleClass="scheduleClass" :item="incomeLive" :lineupList="lineupList"></ag-teamlineupList>
		<div class="__ag__lineupcontent__" v-if="(status == 0 &&  scheduleClass == 4) || (status == 2 && scheduleClass == 5) || (status == 0 && scheduleClass == 5)">
			<ag-recordtitle name="首发阵容"></ag-recordtitle>
			<ag-teamname :incomeLive="incomeLive" :tips="tipText" ></ag-teamname>
			<ag-linupplayerlist :incomeLive="incomeLive" :list="startingList" :scheduleClass="scheduleClass" :isStarting="true"></ag-linupplayerlist>
			<ag-recordtitle name="替补阵容"></ag-recordtitle>
			<ag-teamname :incomeLive="incomeLive" :tips="tipText" ></ag-teamname>
			<ag-linupplayerlist :incomeLive="incomeLive" :list="substituteList" :scheduleClass="scheduleClass" :isStarting="true"></ag-linupplayerlist>
		</div>
		<div class="__ag__lineupcontent__" v-if="status == 2 && scheduleClass == 4">
			<ag-teamrecord :isDefeat="false" :teamName="incomeLive.teamNamea" :image="incomeLive.teamFlaga" :isTeamName="true"></ag-teamrecord>
			<ag-teamtitle></ag-teamtitle>
			<div class="__ag__teamitem__" v-if="homePlayerList && homePlayerList.length">
				<ag-teamscoreitem v-for="(item,index) in homePlayerList" :item='item' :key="'l'+ index" ></ag-teamscoreitem>
			</div>
			<div v-else class="__ag__not__">
				<text class="__ag__nottext__">暂无信息</text>
			</div>
			<ag-teamrecord :isDefeat="false" :teamName="incomeLive.teamNameb" :image="incomeLive.teamNameb" 
			:isTeamName="true"></ag-teamrecord>
			<ag-teamtitle></ag-teamtitle>
			<div class="__ag__teamitem__" v-if="awayPlayerList && awayPlayerList.length">
				<ag-teamscoreitem v-for="(item,index) in awayPlayerList" :item='item' :key="'l'+ index" ></ag-teamscoreitem>
			</div>
			<div v-else class="__ag__not__">
				<text class="__ag__nottext__">暂无信息</text>
			</div>
		...
		<ag-teamname :incomeLive="incomeLive" :tips="tipText" ></ag-teamname>
        <ag-linupplayerlist :incomeLive="incomeLive" v-if="list2 && list2.length" :list="list2" :scheduleClass="scheduleClass" :isStarting="true"></ag-linupplayerlist>
		<div v-else class="__ag__not__">
			<text class="__ag__nottext__">暂无信息</text>
		</div>
	</div>
    ...
    export default {
		...
		methods:{
			async getListMatchLineup() {
				try {
					...
					l.forEach(r => {
						...
						if (r.positionId){
							r.position = r.positionId >=0 && r.positionId <= 5 ? ['守门员','后卫','后腰','中场','前腰','前锋'][r.positionId] : '-'
						}
					...
					let h = l.filter(r => {
						...
						return r.lineupType == 0 ||  r.lineupType == 1 || r.lineupType == 'home' || r.lineupType == 'away'
					})
					this.lineupList = h
					...
					let a = l.filter(r => {
						...
						return r.lineupType == 2 ||  r.lineupType == 3 || r.lineupType == 'homeBackup' || r.lineupType == 'awayBackup'
					})
					let hsl = []
					let hst = []
					let asl = []
			...
			async getListPlayerTechnic() {
				try {
					let resp = await this.__ag__listPlayerTechnic__({matchId: this.matchId})
					let l = resp.data
					if (!l && !l.length) {
						return
					}
					l.forEach(r => {
						...
						r.rebound = parseInt(r.offensiveRebound) + parseInt(r.defensiveRebound)
					})
					let hp = l.filter(r => {
						if (!r) {
							return false
					...
					for(let i in this.homePlayerList){
						...
						if (i == 1) {
							let homeBestAssist = this.homePlayerList.sort(this.sortBest('assist'))
						...
						if (i == 2) {
							let homeBestRebound = this.homePlayerList.sort(this.sortBest('rebound'))
					...
					for(let i in this.awayPlayerList){
						...
						if(i == 1){
							let awayBestAssist = this.awayPlayerList.sort(this.sortBest('assist'))
						...
						if(i == 2){
							let awayBestRebound = this.awayPlayerList.sort(this.sortBest('rebound'))
