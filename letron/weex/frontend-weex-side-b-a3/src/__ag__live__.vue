<template>
  ...
  >
    ...
    >
      ...
      >
        ...
        <div class="__ag__livegz__" v-if="item.id == 0">
          ...
          <div
            class="live-luck"
            v-if="switchWorldcup && a__ag__user__ && a__ag__user__.userType < 3"
          >
            <image
              class="luck-jiang"
              :src="handleImgPath('jiang.png')"
              @click="onLuckDraw"
            ></image>

            <div class="luck-fk" @click="onLuckDraw" v-if="playStatus">
              <image class="fk-image" :src="handleImgPath('fka.png')"></image>
              <text class="fk-text" v-if="!isEndDuration">已观看</text>
              <text class="fk-text" v-if="!isEndDuration"
                >{{ liveDuration }}分钟</text
              >
              <text class="fk-text" v-else>点击抽奖</text>
            </div>
          </div>
        </div>
        ...
        <div class="__ag__livelt__" v-if="item.id == 2">
          ...
          <ag-chat-new-msg-list
            ref="chatlist"
            v-if="!ismsgtalkto && env.brand == 'bl'"
            @keyboard="keyboard"
    ...
    <div class="__ag__socketerror__" v-if="issocket">
      <text
        class="__ag__socketerrortext__"
        :class="[env.brand == 'bl' ? '__ag__socketerrortext_bl__' : '']"
    ...
    <ag-popup
      ...
      v-if="signOutLive"
      :isAnimate="signOutLive"
    >
      <div class="index-modal">
        <image class="modal-bg" :src="handleImgPath('bg.png')"></image>
        <text class="__receive__text">再观看{{ watchDuration }}分钟后</text>
        <text class="__receive__text">即可参加抽奖</text>
        <image
          style="width: 430px; height: 426px"
          :src="handleImgPath('zhong.png')"
        ></image>
        <div class="__ag__lives_btn__">
          <text class="index-btn" @click.stop="signOutLive = false"
            >继续观看</text
          >
          <text class="index-btn" @click.stop="onBack">狠心离开</text>
        </div>
      </div>
      <image
...
import chatNewMsgList from "./components/__ag__chatMsgNew__.vue"
...
// = Theme Color
import { primary } from "./style/themes/bbg.ts"

const dom = weex.requireModule("dom")
const animation = weex.requireModule("animation")
let durationTimer = false
let danmuIndex = 0
export default {
  components: {
    "ag-nvideo": nvideo,
    ...
    "ag-chat-new-msg-list": chatNewMsgList,
    "ag-notlive": notlive,
    "ag-ipx": ipx,
    "ag-emotion": emotion,
  ...
  computed: {
    ...
    liveDuration() {
      let s = this.secondTime % 60

      if (s < 10) {
        s = "0" + s
      }
      let sec = Math.floor(this.secondTime / 60)
      if (sec < 10) {
        sec = "0" + sec
      }
      let second = sec + ":" + s
  ...
  methods: {
    ...
    landscape(e) {
      ...
      console.log("globalEvent2222", orientation)
      if (orientation == 1) {
        this.isFull = false
      } else {
        this.isFull = true
    ...
    changeFull(event) {
      ...
      if (v) {
        dom.getComponentRect(v, function (ret) {
          console.log("qwe==", util.toString(weex.document))
    ...
    onkeyboardSize() {
      if (this.keyboardSize != "0px") {
        ...
        console.log("zbmsgchat", zbmsgchat)
    ...
    newmsg(msg) {
      ...
      if (r) {
        ...
        if (
          ...
          // console.log('如果是当前聊天对象', this.talking.type, msg.messageType, id, this.talking.targetId)
        } else {
          //未读数+1
          r.unread++
        }
    ...
    onSocket(data) {
      this.issocket = !data
      console.log("socket===", util.toString(data))
    },
    ...
    getWatchSecondKey() {
      let uid = (this.a__ag__user__ && this.a__ag__user__.id) || 0
      let date = "sec" + util.dateFormat(util.getDate()) + uid
      return date
    },

    async apiWatchTime() {
      let date = this.getWatchSecondKey()
      this.secondTime = util.getItem(date) || 0

      if (this.secondTime > 0) {
        return
      }
      let today = util.dateFormat(util.getDate())

      try {
        let resp = await api.pageUserActivity()
        if (resp.data && resp.data.list && resp.data.list.length) {
          let list = resp.data.list.filter(
            (r) =>
              r.type == 3 &&
              r.joinMap &&
              r.joinMap.stat &&
              r.joinMap.stat.date == today
          )

          if (list && list.length) {
            let item = list.pop()
            let amount =
              item.joinMap && item.joinMap.stat && item.joinMap.stat.amount
            console.log("b---", amount)
            if (amount > 0) {
              this.secondTime = amount * 60
              util.setItem(date, this.secondTime)
              return
            }
          }
        }
    ...
    calculateTime() {
      ...
      durationTimer = setInterval(() => {
        ...
        if (this.secondTime == 1800) {
          util.message("已观看30分钟，前去任务中心领取一次抽奖次数把~")
        } else if (this.secondTime == 3600) {
          util.message("已观看60分钟，前去任务中心领取一次抽奖次数把~")
        } else if (this.secondTime == 5400) {
          util.message("已观看90分钟，前去任务中心领取一次抽奖次数把~")
        } else if (this.secondTime == 7200) {
          util.message("已观看120分钟，前去任务中心领取一次抽奖次数把~")
        } else if (this.secondTime == 9000) {
          util.message("已观看150分钟，前去任务中心领取一次抽奖次数把~")
        }
      }, 1000)
    },
    onBack() {
      this.signOutLive = false
      util.pop()
    },
    // 退出直播间
    backLive() {
      let m = Math.floor(this.secondTime / 60)
      this.signOutLive = m < 150
      if (m < 30) {
        this.watchDuration = 30 - m
      } else if (m < 60) {
        this.watchDuration = 60 - m
      } else if (m < 90) {
        this.watchDuration = 90 - m
      } else if (m < 120) {
        this.watchDuration = 120 - m
      } else if (m < 150) {
        this.watchDuration = 150 - m
      }
    },
  },
}
...
.fk-text {
  ...
  letter-spacing: 2px;
  color: @white;
  text-shadow: 0.5px 0.5px 0px @red25;
}
.__ag__lives_popup__ {
  width: 750px;
