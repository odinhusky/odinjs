...
sport.goPolicy = (t)=>{
	util.getPush("__ag__web__", {
    url: `${env.download}assets/static/privacy.html`,
    title: "Privacy",
  })
}
sport.goAgreement = (t)=>{
	util.getPush("__ag__web__", {
    url: `${env.download}assets/static/eula.html`,
    title: "Service",
  })
}
...
sport.__ag__infoUser_editGuest__=()=>{
	...
	if(token){
		__ag__sport_api__.infoUser(info).then(sport.__ag__onlogin__).catch(res=>{
			console.log('guest==1',util.toString(res))
