<template>
  <div class="__ag__matchmain__">
    ...
    <!-- 斷網後沒有數據的狀態下拉重新整理 -->
    <scroller
      v-if="a__ag__newslist__.length == 0"
      class="class__scroller"
    ...
    >
      ...
      ></refresh>
      <div class="empty__content">下拉更新</div>
    </scroller>

    <!-- 比賽分類 -->
    <div
      class="__ag__matchClassification__"
      v-if="a__ag__newslist__.length > 0"
    >
      <ag-match-classification
        :list="a__ag__newslist__"
        :scheduleClassId="a__ag__scheduleClass__"
        @changeClass="__ag__changeClass__"
      ></ag-match-classification>
    </div>

    <loading
      class="__loading_main__"
      :class="[ipx ? 'loading-ipx' : '', loadinging ? 'show' : 'hide']"
    >
      <loading-indicator class="loading_text" ref="loading"></loading-indicator>
    </loading>

    <div
      class="__ag__sche-body__"
      :class="[
        ipx ? 'matchbody-ipx' : '',
        ipx && loadinging ? 'matchbody-load-ipx' : '',
        !ipx && loadinging ? 'matchbody-unload-ipx' : '',
      ]"
    >
      <slider
      ...
      >
        <ag-schedule-list
          v-for="item in a__ag__newslist__"
          :key="item.id"
          :scheduleItem="item"
          :ref="'sch' + (item.id + 1)"
          :userMatchMap="userMatchMap"
          :currentClass="a__ag__scheduleClass__"
        ></ag-schedule-list>
      </slider>
      <div
        class="__ag__schedule-bottom__"
        v-if="sliderIndex"
        :class="[ipx ? 'schedule-bottom-ipx' : '']"
        @click="__ag__onsliderIndex__"
      >
        <text class="__ag__schedule-bottom-text__">回到当前賽程</text>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  computed: {
    a__ag__newslist__() {
      if (
        !this.a__ag__listMatchScheduleClassResp__ ||
        !this.a__ag__listMatchScheduleClassResp__.data
      ) {
        return []
      }
  ...
  methods: {
    async __ag__changeClass__(item) {
      await this.__ag__loadData__()
    ...
    __ag__onNewslist__(list) {
      ...
      l.push({
        ...
        prevList: [],
        list: [],
        prevForm: { done: "0", page, size, hot: 1, time },
        nextForm: { done: "0", page, size, hot: 1, time },
        form: { done: "0", time, hot: 1, page: 1, size },
      })
      l.push({
        id: 0,
        name: "全部",
        prevList: [],
        list: [],
        prevForm: { done: "0", page, size, time },
        nextForm: { done: "0", page, size, time },
        form: { done: "0", time, page: 1, size },
      })
      ...
      for (let i in list) {
        ...
        l.push({
          ...
          prevList: [],
          list: [],
          prevForm: { done: "0", time, page, size, scheduleClass: r.id },
          nextForm: { done: "0", time, page, size, scheduleClass: r.id },
          form: { done: "0", page: 1, size, time, scheduleClass: r.id },
        })
        index++
      }
      return l
...
.ag-ipx {
  background-image: linear-gradient(to top, @gray21, @gray24);
}
