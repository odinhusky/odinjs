<template>
  <div
    class="__ag__uplist__"
    v-if="sortedStreamerList && sortedStreamerList.length > 0"
  >
    <scroller
      scroll-direction="horizontal"
      :show-scrollbar="false"
      class="__ag__uplistscroll__"
    >
      <div class="__ag__uplistscroll__">
        <div
          v-for="item in sortedStreamerList"
          :key="'p' + item.id"
          class="__ag__uplistitem__"
          @click="__ag__clickTap__(item)"
        >
          <ag-anchor-item
            :listFollowMap="a__ag__listFollowMap__"
            :item="item"
            @cancelFollow="__ag__cancelFollow__"
            @followTap="__ag__followTap__"
            :isindex="true"
            :showAction="false"
            @onClick="__ag__clickTap__(item)"
          ></ag-anchor-item>
        </div>
      </div>
    </scroller>
...
export default {
  ...
  computed: {
    list2() {
      if (
        !this.a__ag__pageUp__ ||
        !this.a__ag__pageUp__.data ||
        !this.a__ag__pageUp__.data.list
      ) {
        return []
      }
    ...
    sortedStreamerList() {
      ...
      // 已订阅的主播列表
      const userSubscribedStreamerIdList = Object.keys(
      ...
      // 从所有主播列表筛选已推荐的主播
      const backendRecommendedStreamerList = allStreamerList.filter(
        (ss) =>
          ss.joinMap && ss.joinMap.hot_up && ss.joinMap.hot_up.status === 1
      )
      // 从所有正在直播的主播筛选已订阅的主播
      const liveSubscribedStreamerList = allLiveStreamerList.filter((m1) =>
        userSubscribedStreamerList.some((m2) => m1.id === m2.id)
      )
      // 从所有正在直播的主播筛选已推荐的主播
      const liveBackendRecommendedStreamerList = allLiveStreamerList
        .filter((m1) =>
          backendRecommendedStreamerList.some((m2) => m1.id === m2.id)
        )
        .filter(
          (m3) => !liveSubscribedStreamerList.some((m4) => m3.id === m4.id)
        )
      // 未推荐、未订阅的主播
      const otherLiveStreamerList = allLiveStreamerList.filter(
        (m1) =>
          ![
      ...
      // 从预约未上播的主播筛选已订阅的主播
      const notLiveSubscribedStreamerList = [
        ...allRequeseStreamerList,
        ...defaultL,
      ].filter((m1) => userSubscribedStreamerList.some((m2) => m1.id === m2.id))
      // 从预约未上播的主播筛选已推荐的主播
      const notLiveBackendRecommendedStreamerList = [
        ...allRequeseStreamerList,
        ...defaultL,
      ].filter((m1) =>
        backendRecommendedStreamerList.some((m2) => m1.id === m2.id)
      ...
      const notSubAndRecommendedScheduleStreamerList =
        allRequeseStreamerList.filter(
          (m1) =>
            ![
      ...
      const otherNotLiveStreamerLlist = defaultL.filter(
        (m1) =>
          ![
            ...notLiveSubscribedStreamerList,
            ...notLiveBackendRecommendedStreamerList,
  ...
  methods: {
    ...
    __ag__clickTap__(item) {
      ...
      if (!islive) {
        let scheduleUsers = this.requestList.find(
          (user) => user.joinMap.u.id === item.id
        )
        if (!!scheduleUsers) {
