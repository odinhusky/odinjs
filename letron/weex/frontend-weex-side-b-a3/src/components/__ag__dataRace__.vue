<template>
  <div class="__ag__race_main__">
    ...
    <ag-data-race-satistics
      :listMatchDetail="listMatchDetail"
    ></ag-data-race-satistics>
  </div>
</template>
    ...
    async __ag__handleListMatchDetail__() {
      ...
      try {
        ...
        if (resp.success) {
          ...
          if (this.scheduleClass == 4) {
            let homeFast = this.getNumber(
              parseInt(r.homeFast || 0),
              parseInt(r.awayFast || 0)
            )
            let awayFast = this.getNumber(
              parseInt(r.awayFast || 0),
              parseInt(r.homeFast || 0)
            )

            let homeInside = this.getNumber(
              parseInt(r.homeInside || 0),
              parseInt(r.awayInside || 0)
            )
            let awayInside = this.getNumber(
              parseInt(r.awayInside || 0),
              parseInt(r.homeInside || 0)
            )

            let homeExceed = this.getNumber(
              parseInt(r.homeExceed || 0),
              parseInt(r.awayExceed || 0)
            )
            let awayExceed = this.getNumber(
              parseInt(r.awayExceed || 0),
              parseInt(r.homeExceed || 0)
            )

            let homeTotalmis = this.getNumber(
              parseInt(r.homeTotalmis || 0),
              parseInt(r.awayTotalmis || 0)
            )
            let awayTotalmis = this.getNumber(
              parseInt(r.awayTotalmis || 0),
              parseInt(r.homeTotalmis || 0)
            )
