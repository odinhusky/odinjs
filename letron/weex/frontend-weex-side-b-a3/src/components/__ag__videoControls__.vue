    ...
    >
      <div class="__controls-lf__">
        <div class="__controls-direct__" v-if="!isFull">
          ...
          <text
            class="agiconfont __line-status__"
            v-if="isPlay"
          ...
          <text
            class="agiconfont __line-status__"
            v-if="!isPlay && state != 'fail'"
            @click.stop="clickStart"
            >&#xe66c;</text
          >
          <text
            class="agiconfont __line-refresh__"
            ref="__video_refresh__"
            @click="clickRefresh"
            >&#xe631;</text
          >
        </div>
        ...
        <div class="__controls-direct__" v-if="isFull && !ivx">
          <text
            class="agiconfont __line-status__"
            v-if="isDanmus"
            @click.stop="isDanmus = false"
            >&#xe69b;</text
          >
          <text
            class="agiconfont __line-status__"
            v-else
            @click.stop="isDanmus = true"
            >&#xe69a;</text
          >
          <input
            ref="inputref"
            class="__controls_input__"
            v-if="isDanmus"
            :style="keyboardInput"
            placeholder="说点什么吧"
            v-model="content"
      ...
      <div class="__controls-rg__">
        ...
        <div class="__ag__dan_mu__" v-if="showDanmu">
          <image
            class="__ag__danmu_image__"
            v-if="isDanmu"
            :src="handleImgPath(danmuImg)"
            mode="aspectFit"
            @click.stop="__ag_changeDanmu__('0')"
          ></image>
          <image
            class="__ag__danmu_image__"
            v-else
            :src="handleImgPath('static/danmu-2.png')"
            mode="aspectFit"
            @click.stop="__ag_changeDanmu__('1')"
          ></image>
        </div>
        <div class="__controls-limpid__" @click.stop="__ag__changeLimpid__">
          <text class="__control-limpid-text__" ref="__limpid__">{{
            isLimpids
          }}</text>
        </div>
        <text
          class="agiconfont __control-full__"
          v-if="!isFullscreen"
          @click.stop="clickFullscreen('1')"
          >&#xe63e;</text
        >
        <text
          class="agiconfont __control-full__"
          v-else
          @click.stop="clickFullscreen('0')"
          >&#xe63f;</text
        >
      </div>
    </div>
