<template>
  <div class="swiper" v-if="swipersFinal && swipersFinal.length">
    ...
    >
      <div
        class="swiper"
        v-for="(item, index) in swipersFinal"
        :key="index"
        @click="onurl(item)"
      >
        <image
        ...
        <div class="swiper-item" v-if="!item.thumb">
          ...
          <div class="__ag__match_rg__">
            <image
              v-if="item.teamFlagb"
              :src="__ag__url__(item.teamFlagb)"
              class="__ag__match_image__"
            ></image>
            <div class="__ag__match_item__" v-else>
              <text class="__ag__match_text__">{{
                item.teamNameb && item.teamNameb.charAt(0)
              }}</text>
            </div>
          </div>
        </div>
      ...
      <indicator
        v-if="swipersFinal && swipersFinal.length > 0"
        class="indicator"
      ></indicator>
    </slider>
  </div>
...
export default {
  ...
  methods: {
    ...
    onurl(item) {
      if (item.url && item.url.indexOf("/pages/choujiang") != -1) {
      ...
      if (item.type == 4) {
        let live = this.userLiveList.find((l) => item.articleId == l.scheduleId)
        if (live && live.uid) {
          util.getPush("__ag__live__", { uid: live.uid })
          return
        }
        let data = {
          id: item.articleId,
