<template>
  <div class="__ag__match_lives_main__" @click.stop="">
    <div class="__lives_author__">
      <ag-user-img
        class="__author_img__"
        :avatar="upitem.avatar"
        :name="upitem.userNicename"
      ></ag-user-img>
    </div>
    <text class="__lives_name__">{{ upitem.userNicename }}</text>
    <text class="__lives_tips__" v-if="upitem.islive == 1"
      >该主播正在直播其他比赛， 是否去看看？</text
    >
    <text class="__lives_tips__" v-else
      >该主播不在家，是否去看看其他主播？</text
    >
    ...
    <div class="__lives_content__" v-if="lives">
      <div class="__broadcast_item__">
        <ag-user-img
          class="__broadcast_img__"
          :avatar="
            liveitem.joinMap &&
            liveitem.joinMap.schedule &&
            liveitem.joinMap.schedule.teamFlaga
          "
          :name="
            liveitem.joinMap &&
            liveitem.joinMap.schedule &&
            liveitem.joinMap.schedule.teamNamea
          "
        ></ag-user-img>
        <text class="__broadcast_name__">{{
          liveitem.joinMap &&
          liveitem.joinMap.schedule &&
          liveitem.joinMap.schedule.teamNamea
        }}</text>
      ...
      <div class="__broadcast_item__">
        <ag-user-img
          ...
          :avatar="
            liveitem.joinMap &&
            liveitem.joinMap.schedule &&
            liveitem.joinMap.schedule.teamFlagb
          "
          :name="
            liveitem.joinMap &&
            liveitem.joinMap.schedule &&
            liveitem.joinMap.schedule.teamNameb
          "
        ></ag-user-img>
        <text class="__broadcast_name__">{{
          liveitem.joinMap &&
    ...
    <div class="__lives_content__" v-else>
      <div class="__broadcast_lf__">
        <ag-user-img
          class="__author_img__ __author_lives_img__"
          :avatar="liveitem.avatar"
          :name="liveitem.upName"
        ></ag-user-img>
        <image class="__zbz_img__" :src="handleImgPath('zbz.gif')"></image>
      </div>
      <div class="__broadcast_rg__">
        <text class="__tema_name__">{{ liveitem.upName }}</text>
        <text class="__broadcast_tema__"
          >{{
            liveitem.joinMap &&
            liveitem.joinMap.schedule &&
            liveitem.joinMap.schedule.teamNamea
