<template>
  <div class="__ag__schedule-list__">
    <div
      class="__world_banner__"
      v-if="scheduleItem.name == '世界杯'"
      @click="__ag__goWorld__"
    >
      <image class="__world_image__" :src="handleImgPath('sjb.png')"></image>
    </div>
    <!-- 赛事、赛果 -->
    <div
      class="__ag__match-screen-container__"
      :class="[scheduleItem.name == '世界杯' ? 'world_screen' : '']"
    >
      <ag-matchscreen
        @onscreen="__ag__onscreen__"
        :ishot="a__ag__scheduleItem__.id == 1"
        :screen="a__ag__screen__"
        :datetime="a__ag__datetime__"
      ></ag-matchscreen>
    </div>
    <!-- :class="[a__ag__loadingMore__?'__ag__sche-body-scroll-more__':'']"  -->
    <scroller
      class="__ag__sche-body-scroll__"
      :class="[scheduleItem.name == '世界杯' ? 'world_body' : '']"
      :show-scrollbar="false"
      @scroll="__ag__onscroll__"
      @loadmore="__ag__loadmore__"
    >
      <refresh
      ...
      >
        <loading-indicator
          v-if="a__ag__freshText__ != '没有更多数据'"
          class="__ag__loading__"
        ></loading-indicator>
        <text class="__ag__refreshtext__">{{ a__ag__freshText__ }}</text>
      </refresh>
      <div
        v-for="(date, index) in ag__dateList__"
        :key="date"
        @appear="
          (e) => {
            onappear(e, date)
      ...
      >
        <div
          :id="'date-' + date"
          class="__ag__match-date__"
          :class="[
            a__ag__scheduleItem__.id != 1 && env.brand == 'bl'
              ? '__ag__match_date__'
              : '',
            a__ag__scheduleItem__.id != 1 && env.brand == '5'
              ? '__ag__match_date_5__'
              : '',
          ]"
        >
          <text class="__ag__datetext__">{{ util.formatDate(date) }}</text>
          <text class="__ag__datetext__" style="margin-left: 8px">{{
            util.formatWeeks(date)
          }}</text>
        </div>
        <div
          class="__ag__matchrequestitem__"
          v-if="a__ag__scheduleItem__.id == 1"
        >
          <ag-matchrequestitem
            v-for="item in a__ag__listByDate__[date]"
            :key="item.id"
            :id="'v' + item.id"
            :dateItem="item"
      ...
      <div
        class="loadingbox"
        v-if="
          a__ag__loadingMore__ && !platform && a__ag__scheduleItem__.id != 1
        "
      >
        <loading
          class="loadingmore"
          :display="a__ag__loadingMore__ ? 'show' : 'hide'"
        >
          <loading-indicator
            v-if="a__ag__loadMoreText__ != '没有更多数据'"
            class="__ag__loading__"
          ></loading-indicator>
      ...
      <loading
        class="loadingmore-android"
        v-if="platform && a__ag__scheduleItem__.id != 1"
        @loading="onloading"
        :display="a__ag__loadingMore__ ? 'show' : 'hide'"
      >
        <loading-indicator
...
export default {
  ...
  methods: {
    ...
    async __ag__loadData__() {
      ...
      if (
        this.a__ag__scheduleItem__ &&
        this.a__ag__scheduleItem__.id == 1 &&
        this.scheduleItem &&
        this.scheduleItem.id == 1
      ) {
        try {
      ...
      if (
        ...
        try {
          await this.__ag__formPageSchedule__(
            this.a__ag__scheduleItem__.nextForm
          )
        } catch (err) {
          this.a__ag__refreshing__ = false
