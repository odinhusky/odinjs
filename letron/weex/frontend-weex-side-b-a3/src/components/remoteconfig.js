...
async function remoteConfig(rcurl1, rcurl2, rcurl3) {
  try {
    ...
    // console.log("@@ DomainRequest resp1", resp1, typeof resp1);
    if(resp1 !== '') {
      return resp1;
    } else {
      const resp2 = await getDomain(rcurl2)
      // console.log("@@ DomainRequest resp2", resp2, typeof resp2)
      if (resp2 !== "") {
        return resp2
      } else {
        const resp3 = await getDomain(rcurl3)
        // console.log("@@ DomainRequest resp3", resp3, typeof resp3)
        return resp3
      }
    }
  } catch(e) {
