<template>
  <div
    class="__ag__match-screen__"
    :class="[
      !ishot && env.brand == 'bl' ? '__ag__match_date__' : '',
      !ishot && env.brand == '5' ? '__ag__match_date_5__' : '',
    ]"
  >
    ...
    <div class="__ag__screen-content__" v-if="!ishot">
      <div
        ...
        :class="[
          env.brand == 'bl' ? '__ag__screen-body_bl__' : '',
          screen == 0 ? '__ag__select__' : '',
        ]"
        @click="onscreen(0)"
      >
        <text
...
.__ag__match-screen__ {
  ...
  padding-right: 16wx;
  height: 40wx;
  background-color: @primaryRaceBg;
  display: flex;
  flex-direction: row;
