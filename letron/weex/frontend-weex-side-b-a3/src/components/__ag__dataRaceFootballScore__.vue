    ...
    <div class="__football_rg__" ref="football">
      <div class="__rg_top__">
        <text
          ...
          class="__rg_time__"
          :class="[
            index == 0 ? '__relative_left_' : '',
            index == 2 ? '__relative_center_' : '',
            index == 3 ? '__aling_center_' : '',
            index > 3 ? '__aling_right_' : '',
            index == 6 ? '__relative_right_' : '',
          ]"
          >{{ item }}’</text
        >
      </div>
      <div class="__team_content__">
        <div class="__team_box__">
          <div
            class="__team_item__"
            v-for="(item, index) in teamLista"
            :key="'a' + index"
            :class="[
              index == 0 ? '__border_lf__' : '',
              index >= 4 ? '__padding_right__' : '',
            ]"
          >
            <div
              class="__team_event__"
              v-for="(t, k) in item.list"
              :key="'td' + k"
              :style="{ left: t.p + 'px' }"
            >
              <text v-if="t.k == 1 && t.name" class="agiconfont __event_zuqiu__"
                >&#xea78;</text
              >
              <text
                v-if="t.name && (t.k == 2 || t.k == 9 || t.k == 3)"
                class="__event_brand__"
                :class="[
                  t.k == 2 || t.k == 9
                    ? '__event_brand_red__'
              ...
              <image
                v-if="t.k == 7 && t.name"
                style="width: 18wx; height: 18wx"
                :src="handleImgPath('strokes.png')"
                resize="contain"
              ></image>
            </div>
          </div>
        ...
        <div
          class="__midfield_box__"
          v-if="
            (incomeLive && incomeLive.status == 2) ||
            (incomeLive.status == 1 && percent > 0)
          "
          :style="{ left: wleft + 'px', right: wleft + 'px' }"
        >
          <text class="__midfield_text__"></text>
        </div>
      </div>
    </div>
  </div>
...
export default {
  ...
  computed: {
    percent() {
      ...
      if (
        this.incomeLive.status == 3 ||
        this.incomeLive.status == 4 ||
        this.incomeLive.status == 5 ||
        this.incomeLive.status == 6
      ) {
        p = 0
      }
