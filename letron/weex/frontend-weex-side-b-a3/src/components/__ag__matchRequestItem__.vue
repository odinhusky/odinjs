<template>
  ...
  <div class="__ag__matchrequestitem__">
    ...
    <div class="__ag__matchup__" v-if="dateItem.userlist && dateItem.userlist.length > 0">
      <div
      ...
      >
        <div class="__ag__matchupimgdiv__">
          ...
          ></ag-userimg>
          <text v-if="item.islive == 1" class="__ag__matchupimgtext__">直播中</text>
        </div>
  ...
  methods: {
    async onMakeAppointment() {
      ...
      try {
        ...
        if (resp.success) {
          this.loading = false;
          this.$set(this.userMatchMap, resp.data.scheduleId, resp.data.scheduleId);
    ...
    onlive(userlist, item, islive) {
      let uid = item.uid;
      console.log("u==", util.toString(item));
      if (islive) {
        let params = {
          uid: uid,
        };
