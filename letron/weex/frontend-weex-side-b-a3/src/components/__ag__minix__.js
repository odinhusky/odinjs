	...
	export default {
		...
		computed: {
			...
			image() {
				if (env.brand == "bl" || env.brand == "5" || env.brand == "bbg") {
          return ""
        }
				return 'linear-gradient(to top, #d6d6d6, #c0c0c0)'
			},
			bg() {
				if (env.brand == "bl") {
          return "#FCC400"
		...
		methods: {
			...
			__ag__requestList__(livelist,requestList){
				...
				console.log('map===',util.toString(map))
			...
			__ag__groupByDate__(list){
				...
				console.log("aaa===",util.toString(this.a__ag__listByDate__))
