  ...
  >
    ...
    >
              ...
              <div v-for="(ji, index) in n.jsonContent" :key="'jc' + n.id + '-' + index">
                ...
                <div class="__ag__msgusertext_text__" v-if="ji.type == 'text'">
                  ...
                  <div
                    ...
                    v-if="(n.type == 1 || n.type == 4) && n.refContent && n.refName"
                  ...
                  >
                    <div class="__ag__ref_content__">
                      ...
                      ></image>
                      <text class="__ag__ref_text__" v-else>{{ n.refContent }}</text>
                    </div>
                    <text class="__ag__ref_content_text__">{{ ji.content }}</text>
                  </div>
                </div>
      ...
      <div
        ...
        v-if="a__ag__showscroll__ && !a__isScroll__"
        @click="
...
export default {
  ...
  methods: {
    ...
    async __ag__send__() {
      ...
      if (type == 2) {
        sendForm.groupId = id;
        // console.log("@@ sendForm", JSON.stringify(sendForm));
    ...
    __ag__addMessage__(msg, prev) {
      ...
      if (prev) {
        l.unshift(msg);
        this.$nextTick(() => {
          this.__ag__goLastscroll__(this.lastId);
        });
      } else {
        l.push(msg);
      }
    ...
    async __ag__loadData__() {
      ...
      try {
        if (this.type == "2") {
          ...
          try {
            let data = await this.__ag__loadGroupMsg__(this.a__ag__pageMsgGroup__);
            let list = data && data.list;
            if (list && list.length > 0) {
              list.reverse().forEach((r) => {
                this.__ag__addMessage__(r);
            ...
            if (userdata && userdata.data && userdata.data.length > 0) {
              ...
              for (let i in userlist) {
                ...
                let u = {
                  ...
                  userNicename: r.joinMap && r.joinMap.u && r.joinMap.u.userNicename,
                };
    ...
    async prevMsg() {
      if (this.type == 3) {
        ...
        if (this.a__ag__messageList__.length > 0) {
          ...
          this.lastId = this.a__ag__messageList__[0].id;
          try {
            let data = await this.__ag__loadMsg__(f);
            if (data.total == 0) {
              util.message("没有更多消息");
