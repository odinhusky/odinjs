  ...
  >
    ...
    <div class="__ag__indexup__">
      ...
      <div class="__ag__indexagup__">
        <ag-uplist :forUpListData="forUpListData"></ag-uplist>
      </div>
    </div>
    ...
    <div class="__ag__indexhbcontent__">
      ...
      <div class="__ag__livenew__">
        <ag-newslist :list="newList" :isindex="true"></ag-newslist>
      </div>
    </div>
  </scroller>
</template>
...
export default {
  ...
  computed: {
    ...
    forUpListData() {
      ...
      let resetList = this.a__ag__pageUp__.data.list

      resetList.sort((a, b) => b.score - a.score)

      resetList.sort((a, b) => {
        if (a.joinMap.live.scheduleId && !b.joinMap.live.scheduleId) {
          return -1
        } else if (!a.joinMap.live.scheduleId && b.joinMap.live.scheduleId) {
          return 1
        } else {
          return 0
        }
      ...
      if (resetList && resetList.length > 20) {
        return resetList.slice(0, 20)
      }
      return resetList
    },
    newList() {
      let l = this.newsArticle
      if (l && l.length > 5) {
        return l.slice(0, 5)
      }
      return l
