    ...
    >
      ...
      >
        <div :id="'date-' + date" class="__ag__match-date__">
          <text class="__ag__datetext__">{{
            a__ag__util__.formatDate(date)
          }}</text>
          <text class="__ag__datetext__" style="margin-left: 8px">{{
            a__ag__util__.formatWeeks(date)
          }}</text>
        </div>
...
import __ag__sportApi__ from "@/components/__ag__sport_api__"
export default {
  components: {
  ...
  mounted() {
    this.__ag__listRequestSchedule__()
  ...
  methods: {
    checkItemFunc(i) {
      let resultInfo = JSON.parse(JSON.stringify(i))
      const checkData = this.listRequestSchedule.data || []

      if (checkData.length > 0) {
        checkData.forEach((item) => {
          if (item.teamIdb === resultInfo.teamIdb) {
            if (!resultInfo.lives) {
              resultInfo.lives = []
            }
            resultInfo.lives.push({
              fromListRequestSchedule: true,
              id: item.id,
              avatar: item.joinMap.u.avatar,
              upName: item.joinMap.u.userNicename,
              joinMap: {
                user: {
                  userNicename: item.joinMap.u.userNicename,
                },
              },
            })
          }
    ...
    async __ag__listRequestSchedule__() {
      try {
        let r = await __ag__sportApi__.listRequestSchedule()
        this.listRequestSchedule = r
        return r
      } catch (error) {
        return error
      }
    ...
    async onrefresh() {
      console.log("@@ 下拉釋放", this.a__ag__screen__)
    ...
    async __ag__loadData__() {
      ...
      await this.__ag__listRequestSchedule__()
    },
