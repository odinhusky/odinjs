<template>
  <scroller
    ...
    v-if="(resp.data.list && resp.data.list.length > 0) || sysnotices"
  >
    ...
    >
      <div class="__ag__messagelivechat__" v-if="item.type == 4">
        <text class="__ag__anthors_text__" v-if="item.senderId == uid"
          >主播</text
        >
        <text class="__ag__messagelivename__"
          >{{ item.joinMap.u.userNicename }}：</text
        >
    ...
    <div
      ...
      @click="__ag__goscroll__(resp.data.list.length)"
    >
