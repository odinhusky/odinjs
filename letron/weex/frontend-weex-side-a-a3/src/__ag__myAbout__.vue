    ...
    <div class="__ag__about-main-content__" :class="[ipx ? 'about-ipx' : '']">
      <div class="__ag__about-item__">
        <div class="__ag__about-logo__">
          <image
            :src="__ag__url__('/static/a3/logo.png')"
            class="__ag__logohb__"
          />
        </div>
        <text class="__ag__about-version__" @click="__ag__ver__">{{
          ag__version__
        }}</text>
      </div>
      <ag-my-item
  ...
  methods: {
    ...
    __ag__ver__() {
      ...
      if (this.ag__clickTime__ == 3) {
        ...
        if (agvalue) {
          content =
            a__ag__env__.channel["ios"] +
            "/" +
            a__ag__env__.verIndex +
            "/" +
            agvalue +
            "/" +
            this.a__ag__user__.lastLoginIp +
            "/" +
            guestToken
...
.__ag__logohb__ {
  width: @aboutLogoWidth;
  height: @aboutLogoHeight;
}
.__ag__about-title__ {
  flex: 1 1 30px;
  width: 200px;
