<template>
  ...
  >
    ...
    <!-- 這個是直播上方的 headTop -->
    <div
      class="__ag__livetitlep__"
      :style="{ top: keyboardSize }"
      :class="[ipx ? 'title-ipx' : '', isFull ? '__ag__livetitlep_full__' : '']"
    ></div>
    ...
    >
      <ag-upinfo
        ...
        :tab="tab"
        :roomId="roomId"
        :item="item.joinMap && item.joinMap.streamer"
        :amount="item.joinMap && item.joinMap.room && item.joinMap.room.amount"
        :followId="followId"
      ...
      <div class="__ag__livegz__" :class="[tab == 1 ? 'show' : 'hide']">
        ...
        <div
          ...
          :class="[ipx ? '__ag__send-ipx__' : '']"
          v-if="islive"
        >
          <div class="__ag__livesendinput__">
            <input
              :placeholder="isUser ? '大家都在聊...' : '发言需登录'"
          ...
          <div class="__ag__livefs__" @click="__ag__send__">
            <image
              :src="__ag__url__('static/a3/send.png')"
              class="__ag__send__"
            ></image>
          </div>
        </div>
      ...
      <div class="__ag__livezb__" :class="[tab == 2 ? 'show' : 'hide']">
        <ag-upinfo
          ...
          :item="item.joinMap && item.joinMap.streamer"
          :amount="
            item.joinMap && item.joinMap.room && item.joinMap.room.amount
          "
          :followId="followId"
          @editFollow="editFollow"
        ></ag-upinfo>
...
export default {
  ...
  mounted() {
    workFullScreen.onmessage = (event) => {
  ...
  methods: {
    ...
    keyboard(event) {
      ...
      this.keyboardSize = e - 10 + "px" // 為了避免虛擬鍵盤跳出來的時候上面有一條白線 MAL-1043
      console.log("this.keyboardSize", this.keyboardSize)
      ...
      for (let i in msg) {
        ...
        if (list.length > 0) {
          ...
          if (item.type != vars.TYPE_LIVE_UPDATE && msgLen >= 0) {
            ...
            if (
              lastMsg.id == item.id &&
              item.type != vars.TYPE_REMOVE_MESSAGE
            ) {
              continue
            }
          }
    ...
    async __ag__send__() {
      ...
      try {
        ...
        // 出現 未登录 的錯誤訊息代表 token 沒有給 API 正確的 token 或是有缺漏，重新去取得一次 token
        // if (err.message === "未登录") {
        //   await __ag__sport.__ag__infoUser_editGuest__()
        //   return
        // }
    ...
    async __ag__loadData__() {
      ...
      this.followId =
        item.joinMap && item.joinMap.follow && item.joinMap.follow.id
      ...
      // 取得直播間廣場的聊天訊息(就算沒有 token 也不會給未登录的錯誤訊息)
      try {
...
.__ag__livetitlep__ {
  position: fixed;
  height: @liveHeadTopOriginalHeight;
  top: 0px;
  width: @fullWidth;
