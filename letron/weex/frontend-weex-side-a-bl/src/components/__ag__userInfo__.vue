	...
	<div class="__ag__userInfo-main__">
		<div class="__ag__userInfo-item__" >
			<div class="__ag__userInfo-item-image__" v-if="info && info.userType < 3">
				<image v-if="info && info.avatar" class="__ag__avatar__" mode="aspectFill" :src="__ag__url__(info.avatar)"></image>
				<text class="__ag__avatar_text__" v-else>{{info && info.userNicename && info.userNicename.charAt(0)}}</text>
			</div>
			<div class="__ag__userInfo-item-image__" v-else>
				<image class="__ag__avatar-user__" mode="aspectFill" :src="__ag__url__('/static/5l5-notuser.png')"></image>
			</div>

			<div class="__ag__userInfo-item-name__" v-if="info && info.userType<3" @click="__ag__modifyUserInfo__">
				<text class="__ag__userInfo-item-user-ame__">{{info && info.userNicename}}</text>
				<text class="__ag__userInfo-item-account__">登录账号: {{info && info.userLogin}}</text>
			</div>
		...
		<div class="__ag__userInfo-btn__"  v-if="info && info.userType<3"   @click="__ag__modifyUserInfo__">
			<image :src="__ag__url__('/static/bl/edit.png')" class="__ag__editimg__"></image>
		</div>
		<div class="__ag__userInfo-item-name__" v-else>
			<div class="__ag__userInfo-btntext__ __ag__btn_orther__"  @click="__ag__goLogin__" >
