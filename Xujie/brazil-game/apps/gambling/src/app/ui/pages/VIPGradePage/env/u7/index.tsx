...
import VipGradePage from "./VIPGradePage";
...
const VIPGradePage = (props: IVIPGradePageProps) => {
  ...
  const allLevelInfoWithBonus = props.allLevelInfo.map((info) => {
    const currentLevelSignInConfigData = props.allSignInConfig?.find(
      (config) => {
        return (
          config.identifier.split("::")[2].replace("V", "") === `${info.level}`
        );
      }
    );
    const currentLevelSignInConfig = JSON.parse(
      currentLevelSignInConfigData?.value || "[]"
    );
    ...
    return {
      ...
      upRewardAmount:
        info.level < 20
          ? `R$ ${formatLocaleMoney(info.upRewardAmout / 100)}`
          : `${
              jackpotMap[info.level.toString()]
            } Ou numerário de valor equivalente`,
      withdrawAmountLimitDayString: `R$ ${formatLocaleMoney(
        info.withdrawAmountLimitDay / 100
      )}`,
    };
  });
  ...
  // }, device)
  return (
    <VipGradePage allLevelInfoWithBonus={allLevelInfoWithBonus} {...props} />
  );
};
