  ...
  return (
    <div className="overflow-x-auto text-white text-center rounded-xl" >
      <table className="table table-zebra w-full">
        ...
        <thead>
          <tr>
            {props.isProxy && <th className='p-4 border-r border-[rgba(255,255,255,0.2)] text-[var(--secondary-assistant)]'>Dividendos</th>}
            <th className='p-4 border-r border-[rgba(255,255,255,0.2)]'>Data</th>
            {props.type === "1" && <th className='p-4 border-r border-[rgba(255,255,255,0.2)]'>Usuário De Recarga</th>}
        ...
        <tbody>
          {props.records !== undefined && props.records?.length > 0 ? props.records?.map((s: any, index: number) => {
            ...
            return (
              <tr key={index}>
                {props.isProxy && (
                  <td className='p-4 border-r border-[rgba(255,255,255,0.2)] text-[var(--secondary-assistant)]'>{s.dividendos}</td>
                )}
            ...
            <tr>
              <td colSpan={props.type === "1" ? (props.isProxy ? 7 : 6) : (props.isProxy ? 6 : 5)}>
                <div className="flex flex-col items-center p-12 bg-[var(--white-20)]">
