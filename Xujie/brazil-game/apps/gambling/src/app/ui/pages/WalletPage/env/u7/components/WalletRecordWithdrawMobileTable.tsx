...
export const WalletRecordWithdrawMobileTable = ({
  ...
  return datasource.length === 0 && showNoData ? (
    <BaseNoData />
  ) : (
    <div className={cx("w-full overflow-y-auto")}>
      <div className={cx("max-h-[500px]")}>
        {datasource.map((data) => (
          <div
            className={cx(
              "bg-linear-4-main relative text-xs rounded-lg mb-3 font-normal shadow-[0px_0px_8px_0px_#FFFFFF40_inset]"
            )}
          >
            ...
            {columns.map((col) => (
              <div className={cx("flex gap-2 items-center justify-between p-2")}>
                <h5
                  className={cx("text-[var(--transparent-white-70)]", {
                    "text-[var(--text-highlight)]":
                      col.dataIndex === "pay_serial_no",
                  })}
                >
                  {col.title}
                </h5>
