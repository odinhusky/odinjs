...
export const MobileTotalTable = (props: IMobileTotalTable) => {
  ...
  return (
    <div className={"flex flex-col rounded-2xl pb-2 text-[#ffffff] text-left mt-2"}>
      ...
      <div className="py-3 bg-[var(--table-varient)] text-white text-center px-2">
        {props.type === "1" && level1RechargeData.isShowDividends && (
          <div className={"flex flex-row justify-end"}>
            <span className={"text-xs text-[var(--state-success)]"}>{level1RechargeData.dividendsText}</span>
          </div>
        )}
        {props.type === "1" && level1RechargeData.isAvgAmountShow && (
            <div className={"flex flex-row justify-end"}>
              <span className={"text-xs text-[var(--state-success)]"}>{level1RechargeData.avgAmountText}</span>
            </div>
        )}
