    ...
    if (data?.length > 0) {
      return (
        <div
          ...
          // )}
          className={cx(
            "grid grid-flow-row-dense grid-cols-3 gap-3"
          )}
        >
          {data &&
      ...
      return (
        <div className="h-[150px] flex flex-col flex-1 justify-center items-center">
          <NoData className="w-[80px] h-[80px] mobile:w-40 mobile:h-40 tablet:w-[200px] tablet:h-[200px]" />
          <div className="text-sm mobile:text-base tablet:text-2xl text-[var(--grayscale-80)] font-medium">
            Nada aqui
          </div>
        </div>
      );
