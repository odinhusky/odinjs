...
export const PageTemplate = (props: IPageTemplate) => {
  ...
  const { isLogin, uILoading, inNativeApp } = useSelector(
  ...
  const HeaderHeight = useMemo(() => {
    if (isShowHeader) {
      return isDesktop ? 56 : isTablet ? 72 : 64
    } else {
      return isMobile ? 48 : 100
    }
  ...
  return (
    ...
    >
      {uILoading.isLoading && (
        <BaseLoadingOverlay
          className={
            "z-[9999] viewport-fixed top-0 left-0 right-0 bottom-0 bg-main"
          }
      ...
      {isShowHeader && (
        <Header
          className={cx(
            HeaderZIndex,
            "viewport-fixed top-0 left-0 right-0 w-full"
          )}
          isLogin={isLogin}
