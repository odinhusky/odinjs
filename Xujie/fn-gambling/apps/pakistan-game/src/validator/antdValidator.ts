...
export const KYCValidator = {
  walletAccount: (value: string, t: TFunction) => {
    const walletAccountRegex = /^\d{9}$/;
    if (!value) {
      return Promise.reject(t('toast_field_cannot_be_empty'));
    }

    if (!walletAccountRegex.test(value)) {
      return Promise.reject(t('toast_wallet_numbers_restrict'));
    }

    return Promise.resolve();
  },
  bankAccountNumber: (value: string, t: TFunction) => {
    if (!value) {
      return Promise.reject(t('toast_field_cannot_be_empty'));
    }
    return Promise.resolve();
  },
  bankName: (value: string, t: TFunction) => {
    if (!value) {
      return Promise.reject(t('toast_field_cannot_be_empty'));
    }
    return Promise.resolve();
  },
  cnicCode: (value: string, t: TFunction) => {
    const cnicRegex = /^\d{13}$/;
    if (!value) {
      return Promise.reject(t('toast_field_cannot_be_empty'));
    }
    if (!cnicRegex.test(value)) {
      return Promise.reject(t('toast_cnic_code_restrict', { limit: 13 }));
    }
    return Promise.resolve();
  },
