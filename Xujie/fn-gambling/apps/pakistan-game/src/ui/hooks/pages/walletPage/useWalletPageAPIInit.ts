...
export const useWalletPageAPIInit = () => {
  ...
  const setWithdrawBankCards = useWalletPageWithdrawContentStore(
    (state) => state.setWithdrawBankCards
  );
  ...
  useDeepEffect(() => {
    if (!isEmpty(savedBankList)) {
      setWithdrawBankCards(savedBankList);
      const bankCard = savedBankList[0];
