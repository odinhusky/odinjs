      ...
      return (
        <>
          ...
          {value > 0 && (
            <div
              className={cx(
                'ml-[8px] font-normal flex h-3 w-5 mobile:h-4 mobile:w-9 rounded bgi-[var(--base-2-main)] justify-center items-center',
            ...
            {rowData.map((colData: number, colIndex: number) => {
              return (
                <div
                  key={colIndex}
                  className={cx(
                    'flex w-full justify-center items-center break-all',
                    {
                      'bgi-text-[var(--state-warn-main)]':
                        !isVipIcon && colIndex === 3,
                      'bgi-text-[var(--grayscale-10)]':
                        isVipIcon && colIndex === 1,
                    }
                  )}
