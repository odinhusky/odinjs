...
export const GameList = (props: Mode2GameListConfig) => {
  ...
  const collapseInitList = props!.list.slice(0, 3);
  const [collapseList, setCollapseList] = useState(collapseInitList);

  const toggleCollapse = () => {
    setCollapseList(isCollapse ? collapseInitList : props!.list);
          ...
          return (
            <SwiperSlide
              ...
              className={cx('relative cursor-pointer', {
                'py-[2px] mobile:py-2 px-2 mobile:px-4 rounded-lg box-border bgi-[var(--grayscale-20)]':
                  isSupplierGameList,
                'bgi-[var(--grayscale-00)]': isMobile && isSupplierGameList,
              })}
              style={{
  ...
  /**
   * 目前为止，只有PK[V4]的mobile端有该样式
   * 可折叠游戏列表
   */
  const renderSupplier = () => {
    return (
      <div className={cx('grid gap-1', gridContainerClass)}>
        {collapseList.map((item: GameListItemResult, index: number) =>
          renderGameItemComponent(item, props, index)
        )}
      </div>
    );
  ...
  const renderGameList = (): JSX.Element => {
    if (isMobile && isSupplierGameList) {
  ...
  useEffect(() => {
    setCollapseList(collapseInitList);
  }, [props!.list]);

  return (
    <div
      className={cx('mb-4 mobile:mb-5', {
        'p-2 box-border rounded-lg bgi-[var(--grayscale-15)]':
          isMobile && isSupplierGameList,
      })}
    >
      <GameSectionHeader
        title={props.tabName}
        ...
        swiperRef={swiperRef}
        lineClassName={isSupplierGameList ? 'bgi-[var(--base-2-main)]' : ''}
        isSupplierGame={isSupplierGameList}
