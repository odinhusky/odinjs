...
const WithdrawInfoSummary = memo(() => {
  ...
  const blockClass =
    'bgi-[var(--linear-8-main)] bgi-text-[var(--grayscale-00)] p-3 mobile:py-3 mobile:px-6 tablet:p-6 rounded-xl';
  const moneyTextClass = 'font-medium text-base mobile:text-xl tablet:text-3xl';
  const labelTextClass = 'font-medium text-sm mobile:text-lg';

  const Progress = () => {
    return (
      <div
        className={cx(
          'px-3 py-2.5 mt-1 box-border bgi-[var(--grayscale-10)] rounded-xl',
          'flex items-center justify-between'
        )}
      >
        <div
          className={cx(
            'w-full h-3 flex-1 relative',
            'bgi-[var(--transparent-gray-10)] rounded'
          )}
        >
          <div
            className="absolute left-0 top-0 h-full rounded bgi-[var(--linear-8-main)]"
            style={{
          ...
          >
            <div
              ...
              style={{
                background:
                  'linear-gradient(135deg, var(--transparent-white-20) 25%, transparent 25%, transparent 50%, var(--transparent-white-20) 50%, var(--transparent-white-20) 75%, transparent 75%, transparent)',
                backgroundSize: '1rem 1rem',
              }}
            ></div>
          </div>
        </div>
        <div className="ml-4 text-sm font-medium bgi-text-[var(--grayscale-100)]">
          {formatNumber(withdrawProgress, true, 2)}%
        </div>
      </div>
    );
  };
  ...
  return (
    <div className={cx(blockClass, 'my-4')}>
      <div className={cx('flex')}>
        <div className={cx(FLEX_ITEMS_CENTER, 'flex-col flex-1')}>
          <div className="flex items-end">
            <Icon
              className={'h-6 w-6 mr-1'}
              name="ic_rs"
              // src={getImgUrl(EResourceLevel.V, 'ic_rs')}
            />
            <div className="">
        ...
        <div className="w-[1px] h-11 bgi-[var(--grayscale-30)]"></div>
        <div className={cx(FLEX_ITEMS_CENTER, 'flex-col flex-1')}>
          <div className={cx('flex items-end')}>
            <Icon className={'h-6 w-6 mr-1'} name="ic_rs" />
            <div>
              <div className={labelTextClass}>
      ...
      <div className="mt-3 py-2 px-3 rounded-xl bgi-[var(--grayscale-00)]">
        <div className={cx(FLEX_ITEMS_CENTER, 'justify-between')}>
          <div className={cx(labelTextClass, 'bgi-text-[var(--grayscale-70)]')}>
    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="flex flex-col gap-2 font-normal text-sm mobile:text-base">
        ...
        <div className={'flex justify-between'}>
          ...
          <div
            className="value text-xs font-medium cursor-pointer bgi-text-[var(--base-2-main)] underline decoration-[#0F66C1]"
            onClick={() => {
              handleWalletPageBaseClick({
      ...
      {/* 分割線 */}
      <div className={cx('', 'h-px w-full bgi-[var(--grayscale-30)]')} />
...
const WithdrawAmountInput = memo(() => {
  ...
  return (
    <div className={cx('h-auto flex flex-col gap-4 relative')}>
      <div className="flex items-center gap-3">
        <img
          src={getImgUrl(EResourceLevel.V, `vip_level_${withdrawVipLevel}`)}
          alt="icon-vip"
          className="w-[72px] h-[60px] absolute -top-12 left-1/2 -translate-x-1/2"
        />
        <p className="w-full text-wrap text-base font-medium pt-6 box-border flex justify-between">
          <span>
            {t('wallet_withdraw_daily_withdrawal_limit_daily_withdrawal_limit')}
            :
          </span>
          <span>{formatMoney(dailyWithdrawLimit, true)}</span>
        </p>
      </div>

      <div className={cx('', 'h-px w-full bgi-[var(--grayscale-30)]')} />
      <div className={cx('flex flex-col gap-1')}>
        <div className={cx('text-wrap text-base font-medium')}>
          <div>
            {t('wallet_withdraw_daily_withdrawal_limit_single_withdrawal')}
          </div>
          <div className="bgi-text-[--grayscale-70]">
        ...
        >
          <Input
            ...
            styles={{
              containerDiv: '!py-2 !rounded-lg !bgi-[var(--grayscale-00)]',
              inputBox: '!bgi-text-[var(--grayscale-50)]',
              input: '!placeholder-red !px-0',
            }}
            onChange={(e) => {
              handleWalletPageClick({
...
const WithdrawPasswordInput = memo(() => {
  ...
  return (
    <div className="flex flex-col gap-2">
      <div className={cx('')}>
        {t('wallet_withdraw_input_title_withdrawal_password')}
      </div>
      ...
      >
        <Input
          ...
          placeholder={{
            // TODO Evan i18n [PK]
            i18nKey: 'Enter the withdrawal password',
          }}
          styles={{
            containerDiv: '!py-2 !rounded-lg !bgi-[var(--grayscale-00)]',
            inputBox: '!bgi-text-[var(--grayscale-50)]',
...
const WithdrawInputForm = memo((form: FormInstance) => {
  ...
  const BLOCK_CLASS_NAME =
    'bgi-[var(--grayscale-10)] tablet:bgi-[var(--grayscale-15)] rounded p-3 rounded-xl p-3 mobile:px-6';
  return (
    <Form
      form={form}
      className={cx(
        ...
        'gap-3 tablet:gap-y-0 tablet:gap-x-6',
    ...
    >
      <div className={cx(BLOCK_CLASS_NAME, '')}>
        <WithdrawAmountInput />
      </div>
      ...
      <div className={cx(BLOCK_CLASS_NAME, 'tablet:mt-3')}>
...
export const WithdrawContent = () => {
  ...
  return (
    <div>
      ...
      <div
        className={cx(
          'bg-shadow-[var(--navigation-shadow-up)] bgi-[var(--grayscale-00)]',
          'px-4 pt-[50px] pb-10',
          '-ml-4 w-screen rounded-t-[20px]',
