...
export const WithdrawContent = () => {
  ...
  const vipLevel = useWalletPageWithdrawContentStore((state) => state.vipLevel);
  const totalBalance = useWalletPageWithdrawContentStore(
    (state) => state.totalBalance
  );
  const lockAssets = useWalletPageWithdrawContentStore(
    (state) => state.lockAssets
  );
  const dailyWithdrawLimit = useWalletPageWithdrawContentStore(
    (state) => state.dailyWithdrawLimit
  );
  const withdrawableProgress = useWalletPageWithdrawContentStore(
    (state) => state.withdrawableProgress
  );
  ...
  const renderWithdrawAmountInputBlock = () => {
    ...
    const [labelString1, labelString2] = t(
      ...
      {
        minLimit: formatMoney(withdrawLimitStr[0]),
        maxLimit: formatMoney(withdrawLimitStr[1]),
      }
    ).split('\n');
          ...
          <div className="mobile:flex mobile:flex-nowrap">
            <p className={cx(textClass, 'text-wrap')}>
              <Trans
                ...
                values={{
                  dailyWithdrawLimit: formatMoney(dailyWithdrawLimit, true),
                }}
                components={{
  ...
  return (
    ...
    >
      ...
      <div className={cx('mt-6')}>
        <AffixBottomWrapper hasBottomNav affixContainerClass="pb-5">
          <BasePrimaryBtn
            className={cx(
              'h-10',
              'text-sm font-medium',
