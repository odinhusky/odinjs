...
export const KYCValidator = {
  bankAccount: (value: string, t: TFunction) => {
    if (!value) {
      // return Promise.reject('Please input bank account'); // TODO Evan 可優化
  ...
  ifscCode: (value: string, t: TFunction) => {
    if (!value) {
      // return Promise.reject('Please input IFSC code'); // TODO Evan 可優化
      return Promise.reject(t('toast_field_cannot_be_empty'));
    }
...
export const FullOrderDetailValidator = {
  confirmCode: (value: string, t: TFunction, minLimit: number) => {
    if (!value) {
      return Promise.reject(
        t('balance_record_deposit_record_receipt_not_filled_toast')
      );
    }

    if (value.length < minLimit) {
      return Promise.reject(
        t('balance_record_deposit_record_receipt_input_placeholde')
      );
    }
    return Promise.resolve();
  },
