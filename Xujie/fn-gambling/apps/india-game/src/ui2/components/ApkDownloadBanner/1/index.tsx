    ...
    shouldShowDownloadBanner && (
      <div className="relative overflow-hidden flex items-center justify-between h-10 w-full mobile:h-[64px] tablet:rounded-[8px] bgi-[var(--linear-1)] tablet:mb-5 py-1.5 pr-2 pl-8 mobile:py-2 mobile:pr-6 mobile:pl-10 tablet:py-3 tablet:pr-4 tablet:mt-8">
        <div className="flex flex-col font-medium bgi-text-[var(--base-2-main)] text-xs mobile:text-base ">
          <span className={cx('bgi-text-[var(--linear-2)]')}>{appName}</span>
        ...
        <BaseSecondaryBtn
          className={cx(
            ...
            'bgi-[var(--linear-2)]',
            // 為了讓 V1 的 Download 按鈕 hover 的時候背景色不動
            `${
              import.meta.env['VITE_V_VERSION'] === 'v1'
                ? 'hover:bgi-[var(--linear-2)] active:bgi-[var(--linear-2)]'
                : ''
            }`
          )}
          ...
          children={
            <>
              <div
                className={cx(
                  'border-box',
                  'w-4 h-4 mobile:w-6 mobile:h-6',
                  'rounded',
                  // 'border border-[var(--grayscale-00)]', // icon 本身已經自帶邊框
                  FLEX_CENTER,
                  'mr-1 mobile:mr-2'
                )}
              >
                <Icon
                  className="h-3 w-3 mobile:h-5 mobile:w-5"
                  name="ic_download"
