...
// 先加載 Fallback 圖片，如果是 GameLandscapeItem 則是別張長型的
const gameItemFallbackImg = getImgUrl(
  EResourceLevel.V,
...
export const GameItem = memo(
  forwardRef<HTMLDivElement, GameItemProps>((props, ref) => {
    ...
    const { onEnterGame, isShowName } = useGameItemBase();
    ...
    const handleEnterGameClick = () => {
      ...
      if (!item.isMaintain) {
        if (
          item.enterGameType !== EnterGameType.DIRECTORY &&
          ...
          <div className={cx('relative', props.imageClassName)}>
            ...
            {isLoadError ? (
              ...
              <LazyImage
                ...
                className={cx('absolute rounded-lg w-full h-full object-cover')}
                ref={containerRef}
                loadingFallback={gameItemFallbackImg}
                onError={() => {
