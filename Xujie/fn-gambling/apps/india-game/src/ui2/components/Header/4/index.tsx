...
import useMobileExclusiveHeaderOverride from './useMobileExclusiveHeaderOverride';
...
const GameLogo = memo(() => {
  ...
  return (
    <p className="text-base font-bold leading-none bgi-text-[var(--grayscale-100)]">
      {activeManufacturer}
    </p>
  );
});
...
const HeadRight = memo(
  () => {
    ...
    switch (config.type) {
      ...
      default: //首頁
        return (
          <div className="flex gap-2">{config?.render && config.render()}</div>
        );
    }
  },
  (prevProps, nextProps) => {
...
export const Header = memo(
  forwardRef<HTMLDivElement, Partial<IConfig>>(
    (props: Partial<IConfig>, ref: Ref<HTMLDivElement>) => {
      ...
      //  有背景圖，必須壓底色
      const rootHeaderStyle = config.headerBgImg
        ? {
            background: config.headerBgColor || `var(--background-header)`,
            backgroundColor: `var(--background-header)`,
          }
        : {};

      //  有背景圖，控制背景圖位置，寬度，並且跟隨 scroll 位置
      const headerStyle =
        config.headerBgImg && !config.headerBgColor
          ? {
      ...
      return (
        <header
          ref={ref}
          className={cx('header-content', 'w-full fixed z-40')}
          style={rootHeaderStyle}
        >
          {config.type === EHeaderType.Null ? (
            <></>
          ) : (
            <>
              <div className={cx(headerBgColor)} style={headerStyle}>
                <ApkDownloadBanner />
              </div>
