...
const SocialIcon = memo((props: SocialIconProps) => {
  return (
    ...
    >
      ...
      {/*  src={*/}
      {/*    props.isClickable*/}
      {/*      ? getImgUrl(EResourceLevel.ICONS, `${props.icon}_default`)*/}
      {/*      : props.icon*/}
      {/*  }*/}
      {/*  alt={props.icon}*/}
      {/*/>*/}
...
export const SocialList = (props: SocialListProps) => {
  ...
  const clickableIconMapping: Record<string, string> = {
    '18+': 'icon_18_plus',
  ...
  const socialList = useMemo(() => {
    ...
    console.log('@@@===> usageScenariosList', usageScenariosList);

    return list.map((item) => ({
      ...item,
      icon: isClickable
        ? iconMapping[item.label.toLowerCase()] || item.icon
        : clickableIconMapping[item.label.toLowerCase()] || item.icon,
    }));
