...
const TotalRewardTab = () => {
  ...
  return (
    ...
    >
      <div className="mt-3 bgi-[var(--base-2-variant6)] px-7 py-1.6">
        <p className="flex items-center gap-2 bgi-text-[var(--base-2-variant2)]">
          <span>{t('earn_my_rewards_withdrwa_total_rewards')}:</span>
          <span className="bgi-text-[var(--base-1-main)]">{totalRewards}</span>
        </p>
      </div>
    </AffixHeaderBottomWrapper>
  );
          ...
          >
            ...
            <div className="flex flex-1 flex-col justify-center">
              ...
              <p className="text-base bgi-text-[var(--base-2-variant2)]">
                {formatDate(item.createTime ?? 0, 'YYYY-MM-DD HH:mm')}
              </p>
            </div>
            <div className="bgi-text-[var(--base-1-main)]">
              {' '}
              {formatMoney({ value: item.rewards, includeDecimal: true })}
            </div>
          </div>
        );
      })}
