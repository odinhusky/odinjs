...
import { handleInvitePageStatisticsDetailCustomerServiceClick } from '@mode2/action/actionTypes';
...
export const StaticsSpecialDetail = () => {
  ...
  const rewardColumns: ITableColumn<{
    ...
    },
    {
      ...
      dataIndex: 'betAmount',
      render: (v) => {
        return formatMoney({ value: v.betAmount });
      },
    },
    {
    ...
    },
    {
      ...
      dataIndex: 'bonus',
      render: (v) => {
        return formatMoney({ value: v.bonus });
      },
    },
  ];
  ...
  const weeklyColumns: ITableColumn<{
      ...
      render: (v) => {
        return (
          <span className="bgi-text-[var(--state-success-main)]">
            {' '}
            {formatMoney({ value: v.currentLevel })}{' '}
          </span>
        );
      },
    },
    ...
    },
    {
      ...
      render: (v) => {
        return (
          <span className="bgi-text-[var(--linear-2)]">
            {' '}
            {formatMoney({ value: v.lastWeek })}{' '}
          </span>
        );
      },
    },
  ...
  return (
    ...
    >
      ...
      <div
        className={cx(
          ' text-xs mobile:text-sm bgi-text-[var(--grayscale-70)] my-2',
          'text-center'
        )}
      >
        ...
        {isMobile ? <br /> : null}
        <a
          className="ml-1 bgi-text-[var(--state-warn-main)] underline underline-offset-8 cursor-pointer"
          onClick={() => {
            handleInvitePageClick({
              actionName: handleInvitePageStatisticsDetailCustomerServiceClick,
            });
          }}
        >
