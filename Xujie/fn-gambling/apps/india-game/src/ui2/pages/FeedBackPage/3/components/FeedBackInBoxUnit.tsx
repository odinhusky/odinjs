...
>(({ data }, ref) => {
  ...
  return (
    ...
    >
      ...
      >
        {data.isRead ? (
          <Icon
            ...
            name="ic_inbox_notify"
            className="w-full"
          />
        ) : (
          <>
            <Icon name="ic_customer_support" className="w-full" />
            <RedDot className={'absolute top-1 right-0.5 w-1.5 h-1.5'} />
          </>
        )}
      </div>
      ...
      {/* 內文 */}
      <div
        className={cx('flex-1', 'truncate ...', 'text-xs mobile:text-base', {
          'bgi-text-[var(--grayscale-100)]': !data.isRead,
          'bgi-text-[var(--transparent-white-60)]': data.isRead,
        })}
      >
        {data.title}
      </div>
