...
export const OrderDetailModal = () => {
  ...
  const getUTRValue = () => {
    ...
    const state = orderDetail.status;
    const UTRState = orderDetail.UTRState;
    const UTRCode = orderDetail.UTRCode;

    // TODO i18n
    // 只有訂單狀態進程中且UTRCode不為空時顯示UTRCode
    if (state === RechargeRecordStatus.PROCESSING) {
      if (UTRCode) return UTRCode;
      if (UTRState === RechargeReceiptState.UNCOMPLETED) {
        return t('balance_record_deposit_record_receipt_page_title');
      }
          ...
          <div className="flex flex-col gap-1.5">
            ...
            {curSwitchContentTabId === WalletPageTabType.DEPOSIT ? (
              <div className={cx(itemClassName)}>
                <div className={cx(labelClassName)}>
                  <span>UTR</span>
                  <span className="text-xs">(Transaction ID / Ref No.)</span>:
                </div>

                {'UTRCode' in orderDetail && orderDetail.UTRCode ? (
                  <div
                    className={cx(valueClassName, 'flex items-center gap-0.5')}
                    onClick={() => {
                      handleRecordPageClick({
                        actionName: handleRecordPageCopyOrderNumberClick,
                        payload: {
                          text: orderDetail.UTRCode!,
                        },
                      });
                    }}
                  >
                    <span>{orderDetail.UTRCode}</span>
                    <Icon name="ic_copy_2" className="w-4 h-4 cursor-pointer" />
                  </div>
                ) : (
                  <div
                    className={cx('text-base font-medium ', {
                      'bgi-text-[var(--grayscale-100)] no-underline':
                        !isEmpty(orderDetail.UTRCode),
                      'bgi-text-[var(--base-1-main)] underline':
                        orderDetail.status === RechargeRecordStatus.PROCESSING,
                    })}
                    onClick={(e) => {
                      e.stopPropagation();
                      if (
                        ...
                        handleRecordPageClick({
                          ...
                          payload: { value: orderDetail.orderNumber },
                        });
                      }
                    }}
                  >
