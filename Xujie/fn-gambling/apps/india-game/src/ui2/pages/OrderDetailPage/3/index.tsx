...
const getRechargeUTRData = (data: RechargeRecordItemResult) => {
  const baseMap = {
    pending: {
      iconName: 'ic_pending_outline_3',
      className:
        'bgi-text-[var(--state-warning-main)] underline decoration-[var(--state-warning-variant2)]',
      text: 'UTR Number',
    },
    success: {
      iconName: 'ic_success_outline',
      className:
        'bgi-text-[var(--state-success-main)] underline decoration-[var(--state-success-main)]',
      text: 'UTR Number',
    },
    fail: {
      iconName: 'ic_fail_outline',
      className:
        'bgi-text-[var(--state-error-main)] underline decoration-[var(--state-error-main)]',
      text: 'UTR Number',
    },
    uncompleted: {
      iconName: '',
      className:
        'bgi-text-[var(--grayscale-100)] underline decoration-[var(--grayscale-100)]',
  ...
  // 訂單狀態 進行中
  if (data.status === RechargeRecordStatus.PROCESSING) {
    switch (data.UTRState) {
      case RechargeReceiptState.PROCESSING:
        return baseMap.pending;
      case RechargeReceiptState.COMPLETED:
        return baseMap.success;
      case RechargeReceiptState.REJECTED:
        return baseMap.fail;
      default:
        return baseMap.uncompleted;
    }
  }
        ...
        {orderList.length > 0 &&
          orderList.map((item, index) => {
            return (
              ...
              >
                <div
                  className={cx('w-28 max-w-1/4', FLEX_ITEMS_CENTER, 'gap-4')}
                >
                  <Icon
                ...
                {curSwitchContentTabId === WalletPageTabType.DEPOSIT ? (
                  <div className="flex flex-col gap-1.5">
                    ...
                    <div className="flex items-center gap-1">
                      <div className="flex items-center gap-1 bgi-text-[var(--base-2-variant1)]">
                        <span>{t('deposit_history_order_id')}:</span>
                        <span className="bgi-border-b-[var(--base-2-variant1)] bgi-text-[var(--base-2-variant1)]">
                          {item.orderNumber}
                        </span>
                      </div>
                      <Icon
                        name="ic_copy_2"
                        className="w-4 h-4 pb-0.5"
                        onClick={(e) => {
                          e.stopPropagation();
                    ...
                    <div
                      ...
                      onClick={(e) => {
                        if (item.status === RechargeRecordStatus.FAIL) {
                    ...
                    >
                      {getRechargeUTRData(item).iconName ? (
                        <Icon
                          name={getRechargeUTRData(item).iconName}
                          className="w-3 h-3"
                        />
                      ) : null}
                      <div className={cx(getRechargeUTRData(item).className)}>
                        {getRechargeUTRData(item).text}
                      </div>
                    </div>
                  </div>
                ) : (
                ...
                <div className="w-20 max-w-1/5 flex flex-col gap-1.5 text-right">
