...
import useWebviewHeaderOverride from './useWebviewHeaderOverride';
import WebViewPageHeader from './components/WebViewPageHeader';
...
export const WebviewPage = () => {
  ...
  useEffect(() => {
    ...
    if (enterGameData && enterGameData?.launchType === LaunchType.REDIRECT) {
      sdkUtils.openBrowser(enterGameData.url);
      return;
    }
  ...
  return (
    <div
      className={cx( 'relative -mx-4 -mb-4', {
        'h-[100vh]': !isDesktop,
        'h-[calc(100vh-68px)] mt-[68px]': isDesktop
    ...
    >
      ...
      {isShowRechargeContent && (
        <div
          className={cx('absolute z-[5] h-full w-full top-0 left-0', {
            'top-[68px]': isShowRechargeContent && isWebViewPageHeaderShow && !isDesktop,
          })}
        >
