...
//           height={64}
//           numbers={`${formatNumber(rewardNum, true)}`}
//         />
//       </div>
...
export const PinduoduoFreeDrawModal = () => {
  ...
  const boxStyle = 'max-w-[160px] max-h-[190px] cursor-pointer';
  ...
  const getNewRandomBox = (prevBox: number) => {
    let newBox;
    do {
      newBox = Math.floor(Math.random() * 4) + 1;
    } while (newBox === prevBox);
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      setFingerIndex((prev) => getNewRandomBox(prev));
    }, intervalTime);
    return () => clearInterval(interval);
  }, [intervalTime]);

  return pinduoduoFreeDrawModalVisible ? (
    <Modal className="!bgi-[var(--transparent-gray-90)]">
      <div
        id="pinduoduoFreeDrawElement"
        className={cx(
          'relative w-full h-full max-w-[372px]',
          modalAnim
            ? modalAnim
            : 'animate__animated animate__fadeIn animate__faster'
        )}
      >
        ...
        {/*  alt=""*/}
        {/*/>*/}
        <div className="relative w-[372px] h-[372px] top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2">
          <div className="absolute flex flex-col w-full top-0 -translate-y-[150%] justify-center items-center">
            <img
              className="h-12"
                ...
                <div key={index} className="relative">
                  <img
                    ...
                    src={getImgUrl(
                      EResourceLevel.V,
                      selectBoxIndex === seat
                        ? 'invitation_wheel_gift_box_active_light'
                  ...
                  {selectBoxIndex === seat ? (
                    <img
                      ...
                      src={getImgUrl(
                        EResourceLevel.V,
                        'invitation_wheel_gift_box_active',
                        '.webp'
                      )}
                    />
                  ) : null}
                  ...
                  <div
                    className={
                      'text-white text-sm text-center bgi-text-[var(--base-2-variant1)]'
                    }
                  >
                    {selectBoxIndex === seat && isRewardGetSuccess >= 0
                      ? `${formatMoney({
                          value: rewardNum,
                          includeDecimal: true,
                        })}`
                  ...
                  {selectBoxIndex === 0 && isRewardGetSuccess < 0 ? (
                    <img
                      className={cx(
                        'absolute w-[72px] h-[72px] bottom-8 right-3 pointer-events-none',
                        {
          ...
          <div className="text-base text-center mt-[56px] bgi-text-[var(--grayscale-100)] font-semibold drop-shadow-[0px_0px_10px_0px_#E36318]">
