...
interface BonusBannerImageProps extends ParsingAnnouncementResult {
  closePop?: () => void;
}
...
const BonusBannerImage = (props: BonusBannerImageProps) => {
  ...
  const flipNumbersHeight = isMobile ? 48 : isTablet ? 60 : 68;
  return (
    <div
    ...
    >
      ...
      <div className="relative">
        ...
        />
        {props.rewardAmount !== undefined && props.rewardAmount > 0 ? (
          <div
            className={cx(
              'absolute',
              'top-[96px] mobile:top-[120px] tablet:top-[135.75px]',
              'left-[18px] mobile:left-[22.5px] tablet:left-[25.48px]'
            )}
          >
            <AnimationFlipNumbers
              height={flipNumbersHeight}
              numbers={`${formatMoney({
                value: props.rewardAmount,
                showCurrency: false,
              })}`}
              resType={FlipNumbersResType.NUMBER}
            />
          </div>
        ) : null}
      </div>
        ...
        <>
          ...
          >
            ...
            <BaseCacheImg
              className="absolute bottom-0 left-0"
              src={getImgUrl(EResourceLevel.V, 'home_popup_invite_bottom')}
              imgName="home_popup_invite_bottom"
              alt="bottom img"
            />
          </div>
