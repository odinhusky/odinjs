...
const BoxWrapper = ({
  ...
  return (
    <div className={cx('w-full overflow-hidden', className)}>{children}</div>
  );
};
...
export const WeakTipsModal = ({
  ...
  const isArticle = typeof content === 'string';
  const ContentWrapper = isArticle ? ArticleWrapper : BoxWrapper;
  ...
  return isShow ? (
    <BaseModal className="!bgi-[var(--transparent-gray-90)]">
      ...
      >
        ...
        <ContentWrapper
          className={cx({ 'text-start': contentPosition === 'start' })}
        >
          {content}
        </ContentWrapper>
        {/*<article*/}
