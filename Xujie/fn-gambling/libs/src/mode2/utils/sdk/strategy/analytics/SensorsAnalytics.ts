...
> = {
  ...
  //   if (!this.extra.isSupportSensorsSata()) {
  //     return;
  //   }
  //   sensors.init({
  ...
  extra: {
    // registerPropertyPlugin(): void {
    //   try {
    //     (sensors as any)?.registerPropertyPlugin({
    ...
    //         data.properties[SensorsEventLabel.CURRENT] =
    //           currentAttribution || '';
    ...
    registerPropertyPlugin(): void {},
    isSupportSensorsSata(): boolean {
      return import.meta.env['VITE_ENABLE_SENSORS_DATA'] === true;
    },
  },
};
